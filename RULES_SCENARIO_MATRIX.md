...
| **COVERED** | Backend vs sandbox decision‑move surface for line_processing (process_line / choose_line_reward)     | `tests/unit/Backend_vs_Sandbox.CaptureAndTerritoryParity.test.ts` (line_processing getValidMoves parity)                                  | Backend + Sandbox    | Confirms `GameEngine.getValidMoves` and `ClientSandboxEngine.getValidLineProcessingMovesForCurrentPlayer` expose identical line order/reward Moves for the square8 combined scenario `Rules_11_2_12_2_Q7_Q20_overlength_line_then_single_cell_region_square8` from `tests/scenarios/rulesMatrix.ts`. |
...
| **COVERED** | Backend vs sandbox decision‑move surface for territory_processing (process_territory_region + elimination) | `tests/unit/Backend_vs_Sandbox.CaptureAndTerritoryParity.test.ts` (Q23 mini‑region + combined line+territory parity)                 | Backend + Sandbox    | Anchors directly on the Q23 mini‑region and combined line+territory RulesMatrix scenarios: `Rules_12_2_Q23_mini_region_square8_numeric_invariant` and `Rules_11_2_12_2_Q7_Q20_overlength_line_then_single_cell_region_square8`, ensuring both hosts expose matching `process_territory_region` / `eliminate_rings_from_stack` Moves and reach identical numeric outcomes. |
