...
>
> - Canonical rules spec [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md) – especially RR-CANON-R070–R072, R100, R170–R173 and the ANM cluster R2xx.
> - Invariants & parity framework [`INVARIANTS_AND_PARITY_FRAMEWORK.md`](INVARIANTS_AND_PARITY_FRAMEWORK.md) – `INV-ACTIVE-NO-MOVES`, `INV-PHASE-CONSISTENCY`, `INV-TERMINATION`, `PARITY-TS-PY-ACTIVE-NO-MOVES`.
> - Implementation mapping [`RULES_IMPLEMENTATION_MAPPING.md`](RULES_IMPLEMENTATION_MAPPING.md) and rules engine architecture [`RULES_ENGINE_ARCHITECTURE.md`](../architecture/RULES_ENGINE_ARCHITECTURE.md).
> - Python strict-invariant and parity tests under [`ai-service/tests/invariants`](../../ai-service/tests/invariants/test_active_no_moves_movement_forced_elimination_regression.py) and [`ai-service/tests/parity`](../../ai-service/tests/parity/test_active_no_moves_line_processing_regression.py).
>
...
- **Global legal action (for ANM):** For a state with `gameStatus == ACTIVE` and current player P, the union of:
  - Phase-local interactive moves currently exposed to P (placements, movements, captures, line decisions, territory decisions, victory acknowledgements) as defined in [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md).
  - Any legal **ring placements** P could make under [`RR-CANON-R080`–`R082`](../../RULES_CANONICAL_SPEC.md) given the current board and `ringsInHand[P]`, even when `currentPhase` is not `ring_placement`.
  - Any legal **forced-elimination** actions for P under [`RR-CANON-R100`](../../RULES_CANONICAL_SPEC.md) when P controls at least one stack but has no placements, movements, or captures.
  - Any legal **recovery actions** for P under [`RR-CANON-R110`–`R115`](../../RULES_CANONICAL_SPEC.md) when P is recovery-eligible (controls no stacks, owns at least one marker, and has at least one buried ring). Recovery eligibility is independent of rings in hand; when `ringsInHand[P] > 0`, P may reach `movement` by voluntarily recording `skip_placement` and then attempt recovery.
- **Real action (for last-player-standing):** As in [`RR-CANON-R172`](../../RULES_CANONICAL_SPEC.md): a ring placement, non-capture movement, or overtaking capture on the player's own turn. **Recovery actions do NOT count as real actions** for LPS purposes - this creates strategic tension where players with rings in hand have a "survival budget." Forced eliminations also do **not** count as real actions.
...
- **Shape:**
  ...
  - No legal ring placements for P under [`RR-CANON-R080`–`R082`](../../RULES_CANONICAL_SPEC.md) (either `ringsInHand[P] == 0` or all placements are illegal by no-dead-placement).
  - Forced-elimination moves exist for P per [`RR-CANON-R100`](../../RULES_CANONICAL_SPEC.md).
...
- **Expected canonical behaviour:**
  ...
  - Engines must surface a forced-elimination decision to the acting agent (rules intent: the player or AI chooses which eligible stack to eliminate); host-level auto-selection via a deterministic tie-breaker is **not** considered rules-canonical under [`RR-CANON-R206`](../../RULES_CANONICAL_SPEC.md) and is tracked as a temporary implementation compromise in [`KNOWN_ISSUES.md` P0.1](../../KNOWN_ISSUES.md).
  - `currentPhase` after the elimination should follow [`RR-CANON-R070`–`R072`](../../RULES_CANONICAL_SPEC.md) exactly; no ACTIVE / MOVEMENT state with P and no global actions may persist.
- **Implementations / tests:**
  - Python invariant regression [`test_active_no_moves_movement_forced_elimination_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_forced_elimination_regression.py).
  - TS orchestrator soaks and `ACTIVE_NO_MOVES` checks in [`scripts/run-orchestrator-soak.ts`](../../scripts/run-orchestrator-soak.ts).
- **RR-CANON mapping:**
...
- **Shape:**
  ...
  - Current player P has at least one legal **ring placement** under [`RR-CANON-R080`–`R082`](../../RULES_CANONICAL_SPEC.md) (rings in hand and valid no-dead-placement targets).
  - `get_valid_moves(state, P)` (movement + capture) returns `[]`.
...
- **Implementations / tests:**
  - Python invariant regression [`test_active_no_moves_movement_placements_only_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_placements_only_regression.py).
  - TS short soaks and plateau diagnostics described in [`INVARIANTS_AND_PARITY_FRAMEWORK.md`](INVARIANTS_AND_PARITY_FRAMEWORK.md).
- **RR-CANON mapping:**
...
- **Implementations / tests:**
  - Python regression [`test_active_no_moves_movement_fully_eliminated_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_movement_fully_eliminated_regression.py).
  - Phase-consistency discussion in [`INVARIANTS_AND_PARITY_FRAMEWORK.md`](INVARIANTS_AND_PARITY_FRAMEWORK.md).
- **RR-CANON mapping:**
...
- **Expected canonical behaviour:**
  ...
  - If no further territory decisions remain for P, the engine must either:
    - Apply forced elimination for P if required by [`RR-CANON-R072`–`R100`](../../RULES_CANONICAL_SPEC.md), or
...
- **Implementations / tests:**
  - Python regression [`test_active_no_moves_territory_processing_regression.py`](../../ai-service/tests/invariants/test_active_no_moves_territory_processing_regression.py).
  - Territory mutator incident analysis [`docs/incidents/INCIDENT_TERRITORY_MUTATOR_DIVERGENCE.md`](../incidents/INCIDENT_TERRITORY_MUTATOR_DIVERGENCE.md).
- **RR-CANON mapping:**
...
- **Implementations / tests:**
  - Python parity regression [`test_active_no_moves_line_processing_regression.py`](../../ai-service/tests/parity/test_active_no_moves_line_processing_regression.py).
  - TS multi-phase orchestrator tests [`tests/scenarios/Orchestrator.Sandbox.multiPhase.test.ts`](../../tests/scenarios/Orchestrator.Sandbox.multiPhase.test.ts) and line scenarios [`tests/unit/GameEngine.lines.scenarios.test.ts`](../../tests/unit/GameEngine.lines.scenarios.test.ts).
- **RR-CANON mapping:**
...
- **Behaviour:**
  - Engines convert all rings in hand to eliminated rings for scoring, then apply the stalemate tiebreak ladder as in [`RR-CANON-R173`](../../RULES_CANONICAL_SPEC.md):
...
- **Implementations / tests:**
  - Victory logic in [`TypeScript.VictoryAggregate.evaluateVictory`](../../src/shared/engine/aggregates/VictoryAggregate.ts).
  - Stalemate scenarios in [`tests/unit/GameEngine.victory.scenarios.test.ts`](../../tests/unit/GameEngine.victory.scenarios.test.ts) and Python parity tests.
- **RR-CANON mapping:**
...
- **Expected canonical behaviour (RR-CANON):**
  - Per [`RR-CANON-R172`](../../RULES_CANONICAL_SPEC.md) and clarifications in [`COMPLETE_RULES.md`]COMPLETE_RULES.md:1376) §13.3, P should win by Last Player Standing once the “exclusive real-action” condition has held for **three consecutive full rounds** (P takes at least one real action on each of their turns; all others have none).
...
- **Current implementation status:**
  - TS and Python engines now implement explicit early LPS detection via the shared LPS tracker:
    - Shared TS helpers in [`src/shared/engine/lpsTracking.ts`](../../src/shared/engine/lpsTracking.ts) (`updateLpsTracking`, `evaluateLpsVictory`).
...
- **Expected canonical behaviour:**
  ...
  - Colour-based concepts like Territory representation (`ActiveColors` in [`RR-CANON-R142`](../../RULES_CANONICAL_SPEC.md)) still account for buried rings; players who are temporarily eliminated (no turn-material) may still contribute a colour to region eligibility even when they are not currently acting.
...
- **Implementations / tests:**
  - TS and Python turn-rotation logic as described in [`RULES_ENGINE_ARCHITECTURE.md`](../architecture/RULES_ENGINE_ARCHITECTURE.md).
  - Python invariants and self-play soaks under `INV-ACTIVE-NO-MOVES` and `INV-PHASE-CONSISTENCY` in [`INVARIANTS_AND_PARITY_FRAMEWORK.md`](INVARIANTS_AND_PARITY_FRAMEWORK.md).
- **RR-CANON mapping:**
...
- **Expected canonical behaviour:**
  - This state is **not** ANM: P has a legal **recovery action** per [`RR-CANON-R110`–`R115`](../../RULES_CANONICAL_SPEC.md).
...
- `INV-ACTIVE-NO-MOVES` in [`INVARIANTS_AND_PARITY_FRAMEWORK.md`](INVARIANTS_AND_PARITY_FRAMEWORK.md) treats any ACTIVE state where `currentPlayer` has turn-material but no global legal action (per §1) as an invariant violation, except for the terminal global-stalemate shape in ANM-SCEN-06.
- `INV-PHASE-CONSISTENCY` in [`INVARIANTS_AND_PARITY_FRAMEWORK.md`](INVARIANTS_AND_PARITY_FRAMEWORK.md) ensures that composite phases (`line_processing`, `territory_processing`, `chain_capture`) never strand the current player in ANM states; scenarios ANM-SCEN-04 and ANM-SCEN-05 are representative.
- `INV-TERMINATION` and the S-invariant [`RR-CANON-R191`](../../RULES_CANONICAL_SPEC.md) rely on forced elimination and Territory/line processing always increasing eliminated rings or Territory; repeated ANM patterns must always consume some finite resource (e.g. caps) so that play cannot continue forever.
...
- Host-level decision-phase, move-clock, and reconnect-window timeout behaviour (including canonical millisecond units and advisory HUD countdown semantics) is specified in [`P18.3-1_DECISION_LIFECYCLE_SPEC.md`](../archive/assessments/P18.3-1_DECISION_LIFECYCLE_SPEC.md) and summarised for the client in [`CURRENT_RULES_STATE.md`](CURRENT_RULES_STATE.md).
