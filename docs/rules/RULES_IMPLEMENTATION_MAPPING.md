...
- **TypeScript / Node**
  ...
  - Shared types and configs in [`TypeScript.types`](../../src/shared/types/game.ts).
...
- **Python**
  ...
  - Training env and dataset generators in [`Python.env`](../../ai-service/app/training/env.py) and siblings.
...
- Prometheus-style metrics for TS↔Python parity in [`TypeScript.rulesParityMetrics`](../../src/server/utils/rulesParityMetrics.ts).
...
- **Primary** rules semantics live in the **shared TypeScript engine** under `src/shared/engine/**`, especially:
  - Board and marker/ring operations in [`TypeScript.core`](../../src/shared/engine/core.ts).
  - Movement and capture in [`TypeScript.movementLogic`](../../src/shared/engine/movementLogic.ts) and [`TypeScript.captureLogic`](../../src/shared/engine/captureLogic.ts).
  - Lines and rewards in [`TypeScript.lineDetection`](../../src/shared/engine/lineDetection.ts) and [`TypeScript.lineDecisionHelpers`](../../src/shared/engine/lineDecisionHelpers.ts).
  - Territory disconnection and processing in [`TypeScript.territoryDetection`](../../src/shared/engine/territoryDetection.ts), [`TypeScript.territoryProcessing`](../../src/shared/engine/territoryProcessing.ts), and [`TypeScript.territoryDecisionHelpers`](../../src/shared/engine/territoryDecisionHelpers.ts).
  - Turn/phase and forced elimination in [`TypeScript.turnLogic`](../../src/shared/engine/turnLogic.ts).
  - Victory and S‑invariant bookkeeping in [`TypeScript.VictoryAggregate`](../../src/shared/engine/aggregates/VictoryAggregate.ts) and [`TypeScript.computeProgressSnapshot()`](../../src/shared/engine/core.ts).
- The **server** packages these helpers into a networked game implementation:
  - Stateful game loop in [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts).
  - Stateless rules interface in [`TypeScript.RuleEngine`](../../src/server/game/RuleEngine.ts).
  - Board geometry and invariants in [`TypeScript.BoardManager`](../../src/server/game/BoardManager.ts).
  - Turn/phase progression and forced elimination in [`TypeScript.advanceGameForCurrentPlayer`](../../src/server/game/turn/TurnEngine.ts).
- The **client sandbox** exposes the same semantics locally:
  - Canonical sandbox harness in [`TypeScript.ClientSandboxEngine`](../../src/client/sandbox/ClientSandboxEngine.ts).
...
- The **Python rules service** is intended to be semantically equivalent to the TS shared engine, with systematic parity checks and S‑invariant comparison via:
  - [`Python.game_engine`](../../ai-service/app/game_engine/__init__.py) (high-level engine).
  - [`TypeScript.PythonRulesClient`](../../src/server/services/PythonRulesClient.ts) and [`TypeScript.RulesBackendFacade`](../../src/server/game/RulesBackendFacade.ts).
...
- **Server entry & game lifecycle**
  ...
  - Game orchestration in [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) and [`TypeScript.GameSession`](../../src/server/game/GameSession.ts).
  - Turn engine and forced elimination in [`TypeScript.advanceGameForCurrentPlayer`](../../src/server/game/turn/TurnEngine.ts).
- **Shared core rules / engine**
  - Core board ops and S‑invariant in [`TypeScript.core`](../../src/shared/engine/core.ts).
  - Movement, capture, lines, territory, victory, and turn state in the shared engine modules listed above.
- **Client sandbox & rules UI**
  - Sandbox entry at `/sandbox` is driven from [`TypeScript.GamePage`](../../src/client/pages/GamePage.tsx) using [`TypeScript.ClientSandboxEngine`](../../src/client/sandbox/ClientSandboxEngine.ts).
  - Board interactions and visualisation via [`TypeScript.BoardView`](../../src/client/components/BoardView.tsx) and related HUD components.
- **AI / training**
  - Self-play and environment for RL in [`Python.RingRiftEnv`](../../ai-service/app/training/env.py).
  - Dataset generators for moves and territory in [`Python.generate_data`](../../ai-service/app/training/generate_data.py) and [`Python.generate_territory_dataset`](../../ai-service/app/training/generate_territory_dataset.py).
...
The core game models live in [`TypeScript.types`](../../src/shared/types/game.ts) and directly encode many entity and invariant rules:
...
- **Primary implementation**
  - Board configs and geometry parameters in [`TypeScript.types`](../../src/shared/types/game.ts) (`BoardType`, `BOARD_CONFIGS`).
  - Adjacency and distance helpers in [`TypeScript.core`](../../src/shared/engine/core.ts) (`calculateDistance`, `getPathPositions`, movement/line/territory ray-walks).
  - BoardManager neighbour utilities in [`TypeScript.BoardManager`](../../src/server/game/BoardManager.ts) (`getHexagonalNeighbors`, `getMooreNeighbors`, `getVonNeumannNeighbors`, `getAdjacentPositions`).
  - Sandbox board-validity helpers in [`TypeScript.ClientSandboxEngine.isValidPosition`](../../src/client/sandbox/ClientSandboxEngine.ts) and [`TypeScript.sandboxTerritory.getTerritoryNeighbors`](../../src/client/sandbox/sandboxTerritory.ts).
- **Supporting / tests**
...
- **Primary implementation**
  - Stack structure and metadata in [`TypeScript.types`](../../src/shared/types/game.ts) (`RingStack`, `stackHeight`, `capHeight`, `controllingPlayer`).
  - Cap calculations in [`TypeScript.calculateCapHeight()`](../../src/shared/engine/core.ts).
  - Placement and stack building in:
    - Backend placement paths inside [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) near `applyMove()`.
    - Sandbox placement in [`TypeScript.ClientSandboxEngine.tryPlaceRings`](../../src/client/sandbox/ClientSandboxEngine.ts).
  - Overtaking stack mutation in:
    - Shared capture helpers in [`TypeScript.enumerateCaptureMoves()`](../../src/shared/engine/captureLogic.ts).
    - Backend capture application in [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) near `performOvertakingCapture()`.
    - Sandbox capture in [`TypeScript.sandboxCaptures.applyCaptureSegmentOnBoard`](../../src/client/sandbox/sandboxCaptures.ts) and [`TypeScript.ClientSandboxEngine.applyCaptureSegment`](../../src/client/sandbox/ClientSandboxEngine.ts).
  - Cap and ring removal operations (forced elimination, line rewards, territory, self‑elimination) in:
    - [`TypeScript.BoardManager`](../../src/server/game/BoardManager.ts) and backend helpers.
    - Sandbox elimination helper [`TypeScript.sandboxElimination.forceEliminateCapOnBoard`](../../src/client/sandbox/sandboxElimination.ts).
- **Supporting / tests**
...
- **Primary implementation**
  - Board maps `markers` and `collapsedSpaces` in [`TypeScript.types`](../../src/shared/types/game.ts).
  - Marker/territory invariants and exclusivity (no stack+marker, no marker+collapsed) in [`TypeScript.BoardManager`](../../src/server/game/BoardManager.ts) near `assertBoardInvariants()`.
  - Marker path behaviour and collapse rules in [`TypeScript.applyMarkerEffectsAlongPathOnBoard()`](../../src/shared/engine/core.ts).
  - Sandbox equivalents in:
    - [`TypeScript.ClientSandboxEngine.setMarker`](../../src/client/sandbox/ClientSandboxEngine.ts), `flipMarker`, `collapseMarker`.
    - [`TypeScript.ClientSandboxEngine.applyMarkerEffectsAlongPath`](../../src/client/sandbox/ClientSandboxEngine.ts), which in turn delegates to the shared helper; simple movement and capture chains in the sandbox now rely on the shared Movement/Capture aggregates rather than their own marker-path mutators.
- **Supporting / tests**
...
- **Primary implementation**
  - Region abstraction and `Territory` type in [`TypeScript.types`](../../src/shared/types/game.ts).
  - Region discovery and metadata in [`TypeScript.territoryDetection.findDisconnectedRegions`](../../src/shared/engine/territoryDetection.ts).
  - Territory counters per player (`territorySpaces`) and collapsed-space ownership in `BoardState.collapsedSpaces` and `Player.territorySpaces` in [`TypeScript.types`](../../src/shared/types/game.ts).
  - Territory gains in:
    - Line processing helpers (collapsed markers) in [`TypeScript.lineDecisionHelpers`](../../src/shared/engine/lineDecisionHelpers.ts).
    - Territory region processing in [`TypeScript.territoryProcessing.applyTerritoryRegion`](../../src/shared/engine/territoryProcessing.ts) and sandbox variants.
- **Supporting / tests**
...
- **Primary implementation**
  - Types in [`TypeScript.types`](../../src/shared/types/game.ts).
  - Invariant enforcement in:
    - [`TypeScript.BoardManager`](../../src/server/game/BoardManager.ts) via `assertBoardInvariants()`.
    - Sandbox invariant hook [`TypeScript.ClientSandboxEngine.assertBoardInvariants`](../../src/client/sandbox/ClientSandboxEngine.ts).
  - Ring-conservation and S‑invariant checks in:
    - [`TypeScript.computeProgressSnapshot()`](../../src/shared/engine/core.ts).
    - History appenders [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) near `appendHistoryEntry()` and [`TypeScript.ClientSandboxEngine.appendHistoryEntry`](../../src/client/sandbox/ClientSandboxEngine.ts).
  - Hash-based state equality in [`TypeScript.hashGameState()`](../../src/shared/engine/core.ts) for Python parity and history.
- **Supporting / tests**
...
- **Primary implementation**
  - `totalRingsInPlay`, `victoryThreshold`, `territoryVictoryThreshold` computed in [`TypeScript.types`](../../src/shared/types/game.ts) and initialised in shared initial-state helpers.
  - Elimination crediting in:
    - Line decisions via [`TypeScript.lineDecisionHelpers`](../../src/shared/engine/lineDecisionHelpers.ts) and backend/sandbox cap-elimination helpers.
    - Territory region processing via [`TypeScript.territoryProcessing.applyTerritoryRegion`](../../src/shared/engine/territoryProcessing.ts).
    - Forced elimination via [`TypeScript.turnLogic`](../../src/shared/engine/turnLogic.ts) delegates and [`TypeScript.turn.processForcedElimination`](../../src/server/game/turn/TurnEngine.ts).
    - Sandbox forced elimination in [`TypeScript.sandboxElimination.forceEliminateCapOnBoard`](../../src/client/sandbox/sandboxElimination.ts).
    - Stalemate conversion of rings in hand in [`TypeScript.VictoryAggregate.evaluateVictory`](../../src/shared/engine/aggregates/VictoryAggregate.ts) and Python equivalents.
- **Supporting / tests**
...
- **Primary implementation**
  - Canonical phase machine in [`TypeScript.turnLogic.advanceTurnAndPhase`](../../src/shared/engine/turnLogic.ts).
  - Backend orchestration in [`TypeScript.advanceGameForCurrentPlayer`](../../src/server/game/turn/TurnEngine.ts), which wires:
  ...
  - Backend move application and automatic consequences (lines, territory, victory, phase/turn advancement) in [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) (`makeMove()`, `applyDecisionMove()`, `advanceGame()`).
  - Sandbox equivalent phase progression via [`TypeScript.ClientSandboxEngine.startTurnForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts), [`TypeScript.ClientSandboxEngine.maybeProcessForcedEliminationForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts), and [`TypeScript.ClientSandboxEngine.advanceTurnAndPhaseForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts), which delegate to shared `advanceTurnAndPhase` with sandbox-specific delegates.
- **Supporting / tests**
...
- **Primary implementation**
  - Turn-logic delegates used in [`TypeScript.advanceGameForCurrentPlayer`](../../src/server/game/turn/TurnEngine.ts) and shared [`TypeScript.turnLogic`](../../src/shared/engine/turnLogic.ts) to decide when a player has no placements, movements, or captures and must apply forced elimination.
  - Backend blocked-state resolution & stress tests in [`TypeScript.GameEngine.resolveBlockedStateForCurrentPlayerForTesting`](../../src/server/game/GameEngine.ts).
  - Sandbox forced-elimination detection (no placements, no movement, at-or-above per-player cap) in [`TypeScript.ClientSandboxEngine.maybeProcessForcedEliminationForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts).
- **Supporting / tests**
...
- **Primary implementation**
  - Shared semantics: eliminate an entire cap from a controlled stack when blocked, crediting all eliminated rings to the acting player. Implemented via:
    - [`TypeScript.turn.processForcedElimination`](../../src/server/game/turn/TurnEngine.ts) on the server.
    - Sandbox equivalent in [`TypeScript.ClientSandboxEngine.forceEliminateCap`](../../src/client/sandbox/ClientSandboxEngine.ts) and [`TypeScript.sandboxElimination.forceEliminateCapOnBoard`](../../src/client/sandbox/sandboxElimination.ts).
...
- **Primary implementation**
  - Shared placement validation helper [`TypeScript.validatePlacementOnBoard()`](../../src/shared/engine/validators/PlacementValidator.ts) encodes empty/stack placement legality, capacity, and no‑dead‑placement preconditions given a `PlacementContext`.
  - Backend placement validation in [`TypeScript.RuleEngine.validateRingPlacement`](../../src/server/game/RuleEngine.ts), which now delegates to `PlacementAggregate.validatePlacement`, and enumeration in [`TypeScript.RuleEngine.getValidRingPlacements`](../../src/server/game/RuleEngine.ts).
  - Backend skip semantics in [`TypeScript.RuleEngine.validateSkipPlacement`](../../src/server/game/RuleEngine.ts) and placement-phase move generation in [`TypeScript.RuleEngine.getValidMoves`](../../src/server/game/RuleEngine.ts), both delegating to `PlacementAggregate.validateSkipPlacement` / `evaluateSkipPlacementEligibility` and producing explicit `skip_placement` moves when R080 conditions are met.
  - Shared no‑dead‑placement helper [`TypeScript.hasAnyLegalMoveOrCaptureFromOnBoard()`](../../src/shared/engine/core.ts).
  - Shared placement mutators in [`TypeScript.applyPlacementOnBoard()` / `mutatePlacement()` / `applyPlacementMove()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) act as the single source of truth for placement side‑effects and are consumed by backend `RuleEngine.processRingPlacement`, backend `GameEngine.applyMove`, and sandbox `ClientSandboxEngine.tryPlaceRings()` / canonical move application.
  - Sandbox placement logic aligned with shared helpers:
    - Placement enumeration in [`TypeScript.sandboxPlacement.enumerateLegalRingPlacements`](../../src/client/sandbox/sandboxPlacement.ts).
    - Hypothetical stack creation and no‑dead‑placement checks in [`TypeScript.sandboxPlacement.createHypotheticalBoardWithPlacement`](../../src/client/sandbox/sandboxPlacement.ts) and [`TypeScript.sandboxPlacement.hasAnyLegalMoveOrCaptureFrom`](../../src/client/sandbox/sandboxPlacement.ts).
    - Human placement path in [`TypeScript.ClientSandboxEngine.handleHumanCellClick`](../../src/client/sandbox/ClientSandboxEngine.ts) and AI placement in [`TypeScript.ClientSandboxEngine.maybeRunAITurn`](../../src/client/sandbox/ClientSandboxEngine.ts), both funnelling through [`TypeScript.ClientSandboxEngine.applyCanonicalMoveInternal`](../../src/client/sandbox/ClientSandboxEngine.ts) and `tryPlaceRings()`.
- **Supporting / tests**
...
- **Primary implementation**
  - Shared movement target enumeration in [`TypeScript.enumerateSimpleMoveTargetsFromStack()`](../../src/shared/engine/movementLogic.ts) using `MovementBoardView`, consolidated in [`TypeScript.MovementAggregate`](../../src/shared/engine/aggregates/MovementAggregate.ts) (`validateMovement`, `enumerateMovementTargets`, `enumerateSimpleMovesForPlayer`, `applySimpleMovement`, `applyMovement`).
  - Shared legality checks in [`TypeScript.hasAnyLegalMoveOrCaptureFromOnBoard()`](../../src/shared/engine/core.ts) and `MovementAggregate.validateMovement`.
  - Backend movement validation in [`TypeScript.RuleEngine.validateStackMovement`](../../src/server/game/RuleEngine.ts) (delegating to shared movement helpers) and backend movement application in [`TypeScript.RuleEngine.processStackMovement`](../../src/server/game/RuleEngine.ts) / [`TypeScript.GameEngine.applyMove`](../../src/server/game/GameEngine.ts), both wired through `MovementAggregate.applySimpleMovement`.
  - Sandbox movement enumeration in [`TypeScript.sandboxMovement.enumerateSimpleMovementLandings`](../../src/client/sandbox/sandboxMovement.ts) and click-driven movement host [`TypeScript.ClientSandboxEngine.handleLegacyMovementClick`](../../src/client/sandbox/ClientSandboxEngine.ts).
- **Supporting / tests**
...
- **Primary implementation**
  - Shared path helper [`TypeScript.applyMarkerEffectsAlongPathOnBoard()`](../../src/shared/engine/core.ts), used by both movement and capture segments.
  - Backend usage in movement and capture application inside [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) (`applyMove()`, `performOvertakingCapture()`).
  - Sandbox usage in [`TypeScript.ClientSandboxEngine.applyMarkerEffectsAlongPath`](../../src/client/sandbox/ClientSandboxEngine.ts).
- **Landing-on-own-marker elimination**
  - Implemented in:
    ...
    - Sandbox movement in [`TypeScript.ClientSandboxEngine.handleLegacyMovementClick`](../../src/client/sandbox/ClientSandboxEngine.ts) (checks `landedOnOwnMarker` and performs cap reduction and elimination via MovementAggregate).
    - Sandbox capture application in [`TypeScript.ClientSandboxEngine.applyCaptureSegment`](../../src/client/sandbox/ClientSandboxEngine.ts).
- **Supporting / tests**
...
- **Primary implementation**
  - Shared segment validator [`TypeScript.validateCaptureSegmentOnBoard()`](../../src/shared/engine/core.ts).
  - Shared capture-move generator [`TypeScript.enumerateCaptureMoves()`](../../src/shared/engine/captureLogic.ts).
  - Backend validation in [`TypeScript.RuleEngine.validateCapture`](../../src/server/game/RuleEngine.ts) and [`TypeScript.RuleEngine.validateChainCaptureContinuation`](../../src/server/game/RuleEngine.ts).
  - Backend application in [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) via `performOvertakingCapture()`.
  - Sandbox capture enumeration in [`TypeScript.sandboxCaptures.enumerateCaptureSegmentsFromBoard`](../../src/client/sandbox/sandboxCaptures.ts).
  - Sandbox application in [`TypeScript.sandboxCaptures.applyCaptureSegmentOnBoard`](../../src/client/sandbox/sandboxCaptures.ts) and movement host [`TypeScript.ClientSandboxEngine.applyCaptureSegment`](../../src/client/sandbox/ClientSandboxEngine.ts).
- **Supporting / tests**
  ...
  - Capture-search helpers for AI in [`TypeScript.sandboxCaptureSearch.findMaxCaptureChains`](../../src/client/sandbox/sandboxCaptureSearch.ts), aligning with Rust/Node parity scripts.

**R103 Chain overtaking rule (HC)**

- **Primary implementation**
  - Shared enumeration of follow-up segments via [`TypeScript.enumerateCaptureMoves()`](../../src/shared/engine/captureLogic.ts).
  - Backend chain state tracked in `chainCaptureState` within [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) and [`TypeScript.CaptureAggregate`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
  - Sandbox chain engine now lives in [`TypeScript.ClientSandboxEngine.performCaptureChainInternal`](../../src/client/sandbox/ClientSandboxEngine.ts), which:
...
- **Primary implementation**
  - Shared line detection in [`TypeScript.lineDetection.findAllLines`](../../src/shared/engine/lineDetection.ts) and helper `findLinesForPlayer()`.
  - Backend exposure via [`TypeScript.BoardManager`](../../src/server/game/BoardManager.ts) (`findAllLines()`, `findLinesForPlayer()`).
  - Sandbox adapter [`TypeScript.sandboxLines.findAllLinesOnBoard`](../../src/client/sandbox/sandboxLines.ts), now a thin wrapper over the shared detector.
- **Supporting / tests**
...
- **Primary implementation**
  ...
  - Backend decision-phase integration in [`TypeScript.GameEngine.getValidLineProcessingMoves`](../../src/server/game/GameEngine.ts) and [`TypeScript.GameEngine.applyDecisionMove`](../../src/server/game/GameEngine.ts), including `pendingLineRewardElimination` and explicit `eliminate_rings_from_stack` moves.
  - Sandbox canonical application path in:
    - [`TypeScript.ClientSandboxEngine.getValidLineProcessingMovesForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts), which now calls the shared `enumerateProcessLineMoves()` / `enumerateChooseLineRewardMoves()` helpers and `findLinesForPlayer()` directly.
    - [`TypeScript.ClientSandboxEngine.processLinesForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts), which applies `process_line` / `choose_line_option` Moves (legacy alias: `choose_line_reward`) via `applyProcessLineDecision()` / `applyChooseLineRewardDecision()` and then enforces sandbox-specific automatic cap elimination and history recording policy.
...
- **Primary implementation**
  - Region enumeration and classification in [`TypeScript.territoryDetection.findDisconnectedRegions`](../../src/shared/engine/territoryDetection.ts).
  - Sandbox delegate [`TypeScript.sandboxTerritory.findDisconnectedRegionsOnBoard`](../../src/client/sandbox/sandboxTerritory.ts).
  - Border-marker extraction in [`TypeScript.territoryBorders.getBorderMarkerPositionsForRegion`](../../src/shared/engine/territoryBorders.ts) and sandbox adapter [`TypeScript.sandboxTerritory.getBorderMarkerPositionsForRegion`](../../src/client/sandbox/sandboxTerritory.ts).
...
- **Primary implementation**
  - Shared gating helper [`TypeScript.territoryProcessing.canProcessTerritoryRegion`](../../src/shared/engine/territoryProcessing.ts), used by both TS backend and sandbox to enforce the hypothetical-outside-stack requirement.
  - Backend gating in:
    - [`TypeScript.RuleEngine.canProcessDisconnectedRegionForRules`](../../src/server/game/RuleEngine.ts).
  ...
  - Sandbox gating in:
    - [`TypeScript.ClientSandboxEngine.canProcessDisconnectedRegion`](../../src/client/sandbox/ClientSandboxEngine.ts).
    - [`TypeScript.ClientSandboxEngine.getValidTerritoryProcessingMovesForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts), which filters shared `enumerateProcessTerritoryRegionMoves()` by the same prerequisite.
- **Supporting / tests**
...
- **Primary implementation**
  - Shared canonical region-application logic [`TypeScript.territoryProcessing.applyTerritoryRegion`](../../src/shared/engine/territoryProcessing.ts) and end-to-end decision helpers [`TypeScript.territoryDecisionHelpers.enumerateProcessTerritoryRegionMoves`](../../src/shared/engine/territoryDecisionHelpers.ts) (canonical `choose_territory_option` MoveType; legacy alias: `process_territory_region`) and `applyProcessTerritoryRegionDecision()`.
  - Backend integration:
    - RuleEngine decision move generation for `choose_territory_option` (legacy alias: `process_territory_region`) and `eliminate_rings_from_stack` (territory context) in [`TypeScript.RuleEngine.getValidMoves`](../../src/server/game/RuleEngine.ts).
  ...
  - Sandbox integration:
    - Region discovery and automatic processing in [`TypeScript.ClientSandboxEngine.processDisconnectedRegionsForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts), which now drives `choose_territory_option` (legacy alias: `process_territory_region`) and in-loop self-elimination via shared `applyProcessTerritoryRegionDecision()` and `applyEliminateRingsFromStackDecision()`.
    - Canonical move application in [`TypeScript.ClientSandboxEngine.applyCanonicalMoveInternal`](../../src/client/sandbox/ClientSandboxEngine.ts) for `choose_territory_option` (legacy alias: `process_territory_region`) and `eliminate_rings_from_stack`.
...
- **Primary implementation**
  - Victory evaluation in [`TypeScript.VictoryAggregate.evaluateVictory`](../../src/shared/engine/aggregates/VictoryAggregate.ts), which:
  ...
  - Backend usage:
    - `RuleEngine.checkGameEnd()` in [`TypeScript.RuleEngine.checkGameEnd`](../../src/server/game/RuleEngine.ts).
  ...
  - Sandbox victory wrapper [`TypeScript.sandboxVictory.checkSandboxVictory`](../../src/client/sandbox/sandboxVictory.ts) and game-end orchestration in [`TypeScript.sandboxGameEnd.checkAndApplyVictorySandbox`](../../src/client/sandbox/sandboxGameEnd.ts) and [`TypeScript.ClientSandboxEngine.checkAndApplyVictory`](../../src/client/sandbox/ClientSandboxEngine.ts).
- **Supporting / tests**
...
- **Primary implementation**
  - Stalemate handling and tiebreakers in [`TypeScript.VictoryAggregate.evaluateVictory`](../../src/shared/engine/aggregates/VictoryAggregate.ts), including:
  ...
  - Sandbox victory wrapper mirrors the same fields when building `GameResult` in [`TypeScript.sandboxVictory`](../../src/client/sandbox/sandboxVictory.ts).
- **Supporting / tests**
...
- **Primary implementation**
  - S‑invariant metric and monotonicity in [`TypeScript.computeProgressSnapshot()`](../../src/shared/engine/core.ts) and comments in [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md).
  - Backend history entries that record `progressBefore`/`progressAfter` and hashes in [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) near `appendHistoryEntry()`.
  - Sandbox history mirroring in [`TypeScript.ClientSandboxEngine.appendHistoryEntry`](../../src/client/sandbox/ClientSandboxEngine.ts), including S‑invariant and board summaries.
  - TS↔Python parity checks comparing S‑invariant and state hashes in [`TypeScript.RulesBackendFacade.compareTsAndPython`](../../src/server/game/RulesBackendFacade.ts).
- **Randomness**
  - The core rules engine is deterministic; however, the _AI policies_ intentionally use pseudo‑randomness:
    - Local AI RNG in [`TypeScript.localAIMoveSelection`](../../src/shared/engine/localAIMoveSelection.ts) and sandbox AI in [`TypeScript.sandboxAI`](../../src/client/sandbox/sandboxAI.ts).
...
- **Primary implementation**
  ...
  - Turn-material predicates computed in [`TypeScript.VictoryAggregate`](../../src/shared/engine/aggregates/VictoryAggregate.ts) when determining whether a player has controlled stacks or rings in hand.
  - Backend usage in [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) via `getValidMovesForCurrentPlayer()` and orchestrator-backed `TurnEngineAdapter`.
...
- **Primary implementation**
  ...
  - Invariant checks in [`TypeScript.turnLogic.advanceTurnAndPhase`](../../src/shared/engine/turnLogic.ts) delegates that ensure no player is left without legal actions.
...
- **Primary implementation**
  - Phase-exit / transition logic now lives in the canonical FSM stack: [`TypeScript.fsm.FSMAdapter`](../../src/shared/engine/fsm/FSMAdapter.ts) + [`TypeScript.fsm.TurnStateMachine`](../../src/shared/engine/fsm/TurnStateMachine.ts). (Legacy `orchestration/phaseStateMachine.ts` removed.)
  - Backend phase advancement in [`TypeScript.advanceGameForCurrentPlayer`](../../src/server/game/turn/TurnEngine.ts) and sandbox equivalents.
- **Supporting / tests**
...
- **Primary implementation**
  - Host-level forced elimination triggered by [`TypeScript.turnLogic`](../../src/shared/engine/turnLogic.ts) delegates when a player has stacks but no legal placements, movements, or captures.
...
- **Primary implementation**
  - Last-player-standing (LPS) victory logic in [`TypeScript.VictoryAggregate.evaluateVictory`](../../src/shared/engine/aggregates/VictoryAggregate.ts) distinguishes real actions (placements, movements, captures) from forced elimination.
  - S‑invariant monotonicity and termination guarantees enforced via [`TypeScript.computeProgressSnapshot()`](../../src/shared/engine/core.ts).
- **Supporting / tests**
...
- [`TypeScript.VictoryAggregate`](../../src/shared/engine/aggregates/VictoryAggregate.ts)
...
- [`TypeScript.RulesBackendFacade`](../../src/server/game/RulesBackendFacade.ts) and [`TypeScript.PythonRulesClient`](../../src/server/services/PythonRulesClient.ts)
...
- **R190 No randomness (strict reading)** – No explicit runtime guard prevents injecting randomness into the rules layer. Determinism is instead maintained by convention and testing.
  ...
  - **Evidence:** Randomness is confined to AI policy selection in [`TypeScript.localAIMoveSelection`](../../src/shared/engine/localAIMoveSelection.ts), [`TypeScript.sandboxAI`](../../src/client/sandbox/sandboxAI.ts), and Python training infrastructure.
...
- **Board invariant "repair" behaviour** in [`TypeScript.BoardManager`](../../src/server/game/BoardManager.ts) and sandbox `assertBoardInvariants`.
...
- **Forced-elimination stack-selection heuristic** in [`TypeScript.turn.processForcedElimination`](../../src/server/game/turn/TurnEngine.ts) and sandbox equivalents.
...
- **Blocked-state resolver for synthetic test states** in [`TypeScript.GameEngine.resolveBlockedStateForCurrentPlayerForTesting`](../../src/server/game/GameEngine.ts).
...
- **Legacy territory search helpers** in [`TypeScript.sandboxTerritory`](../../src/client/sandbox/sandboxTerritory.ts) and server `territoryProcessing.ts` are now marked deprecated in comments.
...
- Prove or mechanically check that shared helpers in [`TypeScript.core`](../../src/shared/engine/core.ts), [`TypeScript.movementLogic`](../../src/shared/engine/movementLogic.ts), [`TypeScript.captureLogic`](../../src/shared/engine/captureLogic.ts), [`TypeScript.lineDecisionHelpers`](../../src/shared/engine/lineDecisionHelpers.ts), [`TypeScript.territoryDetection`](../../src/shared/engine/territoryDetection.ts), [`TypeScript.territoryProcessing`](../../src/shared/engine/territoryProcessing.ts), [`TypeScript.turnLogic`](../../src/shared/engine/turnLogic.ts), and [`TypeScript.VictoryAggregate`](../../src/shared/engine/aggregates/VictoryAggregate.ts) satisfy the corresponding RR‑CANON rules enumerated in Section 3.
- Check that S‑invariant accounting in [`TypeScript.computeProgressSnapshot()`](../../src/shared/engine/core.ts) is monotone and bounded for all rule-legal transitions (movement, capture, line, territory, forced elimination, stalemate).
- Confirm that legacy helpers marked deprecated (e.g., old territory region search in [`TypeScript.sandboxTerritory`](../../src/client/sandbox/sandboxTerritory.ts) and server `territoryProcessing.ts`) are not used in normal game flows.
...
- Cross-cutting behaviours:
  - Turn/phase transitions, especially around decision phases (`line_processing`, `territory_processing`, `chain_capture`) in [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts) and [`TypeScript.ClientSandboxEngine`](../../src/client/sandbox/ClientSandboxEngine.ts).
  - Forced elimination entry and stack selection heuristics in [`TypeScript.turn.processForcedElimination`](../../src/server/game/turn/TurnEngine.ts) and sandbox equivalents.
  - Landing-on-own-marker elimination interactions with line and territory processing (ensure orderings match RR‑CANON).
- Host alignment:
  - Confirm that backend, sandbox, and Python engines produce identical legal move sets and state transitions for all RR‑CANON-relevant scenarios, using the parity harnesses wired through [`TypeScript.RulesBackendFacade`](../../src/server/game/RulesBackendFacade.ts) and [`TypeScript.ClientSandboxEngine`](../../src/client/sandbox/ClientSandboxEngine.ts).
