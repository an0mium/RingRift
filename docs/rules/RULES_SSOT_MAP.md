...
- **Legacy `GameEngine` pipeline** – historical turn loop inside [`GameEngine.ts`](../../src/server/game/GameEngine.ts:1) that:
  ...
  - Is now guarded by `config.isTest` and explicit `disableOrchestratorAdapter()` calls and is **not used in production** per [`docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md`](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:793).
  - Intended status: `test-only / diagnostics-only`.
- **Legacy `RuleEngine` orchestration helpers** – methods such as `processMove`, `processLineFormation`, `processTerritoryDisconnection` in [`RuleEngine.ts`](../../src/server/game/RuleEngine.ts:46) predate the orchestrator and are now documented as **DIAGNOSTICS-ONLY (legacy backend pipeline)** in [`docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` §7.3](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:882).
...
- **Legacy `ClientSandboxEngine` pipeline** – non-orchestrator branches in [`ClientSandboxEngine.ts`](../../src/client/sandbox/ClientSandboxEngine.ts:137) that:
  ...
  - Are now treated as **tests/tools-only** surfaces, used primarily in sandbox vs backend parity tests and historical diagnostics (see [`docs/SHARED_ENGINE_CONSOLIDATION_PLAN.md` §3–4](../architecture/SHARED_ENGINE_CONSOLIDATION_PLAN.md:41) and [`docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` §4.2](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:224)).
...
- **Sandbox helper modules** – adapters and analysis tools in `src/client/sandbox/**`:
  ...
  - Legacy local sandbox harness: [`localSandboxController.ts`](../../src/client/sandbox/localSandboxController.ts:1) – explicitly flagged in [`docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` §7.5](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:877) as `DIAGNOSTICS-ONLY (legacy)`; `/sandbox` no longer depends on it for live UI.
...
| Legacy `RuleEngine.processMove` and post-processing helpers | [`RuleEngine.ts`](../../src/server/game/RuleEngine.ts:46)                                                                                                | `diagnostic-only` – historical orchestration helpers around shared aggregates; no production call sites per [`docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` §7.3](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:882).                                         | Medium: reusing these helpers instead of orchestrator would bypass `processTurn` and `PendingDecision` semantics, risking divergence in composite flows (capture → lines → territory) and undermining contract-vector coverage.                             |
...
5. **Invariant/soak story fragmented between TS and Python docs**
   - [`docs/testing/STRICT_INVARIANT_SOAKS.md`](../testing/STRICT_INVARIANT_SOAKS.md:1) focuses on Python’s strict no-move invariant and self-play soaks, while [`docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` §6.2](../architecture/ORCHESTRATOR_ROLLOUT_PLAN.md:456) and [`docs/runbooks/ORCHESTRATOR_ROLLOUT_RUNBOOK.md` §2–3](../runbooks/ORCHESTRATOR_ROLLOUT_RUNBOOK.md:33) describe TS orchestrator invariant soaks and SLOs. The two views are consistent but live in separate documents with limited cross-linking.
