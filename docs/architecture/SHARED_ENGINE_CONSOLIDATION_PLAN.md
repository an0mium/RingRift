...
| Turn orchestration core      | [`turnLogic.ts`](../../src/shared/engine/turnLogic.ts), [`fsm/TurnStateMachine.ts`](../../src/shared/engine/fsm/TurnStateMachine.ts), [`fsm/FSMAdapter.ts`](../../src/shared/engine/fsm/FSMAdapter.ts), [`orchestration/turnOrchestrator.ts`](../../src/shared/engine/orchestration/turnOrchestrator.ts) | ~400              | Shared turn advancement semantics (phase transitions, forced elimination gating, basic LPS hooks). (Legacy `phaseStateMachine.ts` removed in PASS30-R1.)                                                                                       |
...
- Capture aggregate (canonical core):
  - [`ChainCaptureSegment`](../../src/shared/engine/aggregates/CaptureAggregate.ts), [`ChainCaptureState`](../../src/shared/engine/aggregates/CaptureAggregate.ts), [`ChainCaptureStateSnapshot`](../../src/shared/engine/aggregates/CaptureAggregate.ts), [`ChainCaptureEnumerationOptions`](../../src/shared/engine/aggregates/CaptureAggregate.ts), [`ChainCaptureContinuationInfo`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  - [`CaptureSegmentParams`](../../src/shared/engine/aggregates/CaptureAggregate.ts), [`CaptureApplicationOutcome`](../../src/shared/engine/aggregates/CaptureAggregate.ts), [`CaptureValidationResult`](../../src/shared/engine/aggregates/CaptureAggregate.ts), [`CaptureMutationResult`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  - Enumeration and state helpers:
    - [`enumerateCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
    - [`enumerateAllCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
    - [`enumerateChainCaptureSegments`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
    - [`getChainCaptureContinuationInfo`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
    - [`enumerateChainCaptures`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  - Mutation / application helpers:
    - [`validateCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
    - [`mutateCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
    - [`updateChainCaptureStateAfterCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
    - [`applyCaptureSegment`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
    - [`applyCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts)

- Legacy helpers now acting as shims:
  - [`captureLogic.ts`](../../src/shared/engine/captureLogic.ts)
    - Reduced to a thin wrapper that re‑exports `CaptureBoardAdapters` and forwards [`enumerateCaptureMoves`](../../src/shared/engine/captureLogic.ts) to the aggregate implementation. All ray‑walk and geometry logic was removed from this file.
  - [`chainCaptureTracking.ts`](../../src/shared/engine/chainCaptureTracking.ts)
    - Refactored into a compatibility layer that:
      - Defines a legacy [`ChainCaptureStateSnapshot`](../../src/shared/engine/chainCaptureTracking.ts) with `visitedTargets?: string[]` and maps it to the canonical snapshot’s `capturedThisChain: Position[]` via `stringToPosition`.
      - Adapts [`ChainCaptureEnumerationOptions`](../../src/shared/engine/chainCaptureTracking.ts) to the aggregate’s `ChainCaptureEnumerationOptions`, with explicit property copying to keep `exactOptionalPropertyTypes` happy.
      - Delegates:
        - [`enumerateChainCaptureSegments`](../../src/shared/engine/chainCaptureTracking.ts) → aggregate `enumerateChainCaptureSegments`.
        - [`getChainCaptureContinuationInfo`](../../src/shared/engine/chainCaptureTracking.ts) → aggregate `enumerateChainCaptureSegments`, re‑exposed as `{ hasFurtherCaptures, segments }`.
        - [`canCapture`](../../src/shared/engine/chainCaptureTracking.ts) → shared [`validateCaptureSegmentOnBoard`](../../src/shared/engine/core.ts).
        - [`getValidCaptureTargets`](../../src/shared/engine/chainCaptureTracking.ts) → aggregate `enumerateCaptureMoves` grouped by `captureTarget`.
        - [`processChainCapture`](../../src/shared/engine/chainCaptureTracking.ts) → uses `canCapture` plus aggregate enumeration on the _unmutated_ state; documented as an approximate, analysis‑only helper.
  - Mutator shim:
    - [`mutators/CaptureMutator.ts`](../../src/shared/engine/mutators/CaptureMutator.ts)
      - Now bridges the engine `GameState` type to the shared `GameState` and delegates `mutateCapture` calls directly to [`CaptureAggregate.mutateCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts), making the aggregate the single TS source of truth for capture application.

**Significant internal refactors:**

- All capture ray‑walk and reachability logic in legacy helpers has been removed in favour of [`enumerateCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and [`enumerateChainCaptureSegments`](../../src/shared/engine/aggregates/CaptureAggregate.ts). Backend and sandbox enumeration already flow through these helpers (directly or via adapters), so future P1.3/P1.4 work can safely delete any remaining host‑side capture geometry.
- Capture mutation semantics (departure marker, intermediate marker processing, ring transfer, landing‑on‑own‑marker elimination) are now implemented only in [`mutateCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts). Backend and sandbox mutators are thin adapters over this core.
...
- **Landing on any marker:** per canonical spec ([`RR-CANON-R091/R092`](../../RULES_CANONICAL_SPEC.md) and [`RR-CANON-R101/R102`](../../RULES_CANONICAL_SPEC.md)), landing on any marker (own or opponent) is legal during both movement and captures, but incurs a cap-elimination cost. The marker is removed and the top ring of the moving stack's cap is eliminated. This is enforced by:
  - Using [`validateCaptureSegmentOnBoard`](../../src/shared/engine/core.ts) for all segment legality checks.
  - [`mutateCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts) removes any marker found at the landing cell and eliminates the top ring of the attacking stack's cap.
- **Visited‑set policy:** the aggregate keeps `capturedThisChain` and an optional `disallowRevisitedTargets` flag (used only by analysis tools such as [`scripts/findCyclicCaptures.js`](../../scripts/findCyclicCaptures.js)). Rules‑level helpers such as [`getChainCaptureContinuationInfo`](../../src/shared/engine/aggregates/CaptureAggregate.ts) call `enumerateChainCaptureSegments` without this flag, ensuring cyclic and revisiting chains remain legal as required by [`RR-CANON-R103`](../../RULES_CANONICAL_SPEC.md).
- **TS coverage:** targeted Jest runs over capture‑focused suites (including `captureLogic.shared`, `chainCaptureTracking.test.ts`, `captureSequenceEnumeration`, `GameEngine.chainCapture`, and [`CaptureAggregate.chainCapture.shared`](../../tests/unit/CaptureAggregate.chainCapture.shared.test.ts)) now report:
  - [`captureLogic.ts`](../../src/shared/engine/captureLogic.ts) — 100% statements/branches/functions/lines.
  - [`chainCaptureTracking.ts`](../../src/shared/engine/chainCaptureTracking.ts) — ~92% statements/lines and >80% branches.
  - [`CaptureAggregate.ts`](../../src/shared/engine/aggregates/CaptureAggregate.ts) — ≈93% statements, ≈94% branches, ≈94% lines.
...
No intentional semantic changes to the canonical capture rules were introduced in this step; all adjustments either removed duplicated implementations or clarified behaviour already mandated by [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md). Any future divergence discovered by TS–Python parity tests (e.g. [`test_chain_capture_parity.py`](../../ai-service/tests/parity/test_chain_capture_parity.py)) should be treated as a bug against this consolidated TS source of truth.
...
Backend capture enumeration, validation, and mutation in the Node/TS backend now delegate exclusively to the shared capture core in [`CaptureAggregate.ts`](../../src/shared/engine/aggregates/CaptureAggregate.ts). The key backend entry points and their wiring are:

- [`GameEngine.applyMove`](../../src/server/game/GameEngine.ts) — the `'overtaking_capture'` and `'continue_capture_segment'` branches call the canonical [`applyCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts) helper (imported as `applyCaptureAggregate`) and then copy the resulting `board` maps back into the existing `GameState.board` instance to preserve Map identities for legacy tests.
- [`GameEngine.getValidMoves`](../../src/server/game/GameEngine.ts) — when `currentPhase === 'chain_capture'`, continuation options are computed via [`getCaptureOptionsFromPosition`](../../src/server/game/GameEngine.ts), which is a thin adapter over [`getChainCaptureContinuationInfo`](../../src/shared/engine/aggregates/CaptureAggregate.ts). Chain state bookkeeping uses [`updateChainCaptureStateAfterCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and the shared [`ChainCaptureState`](../../src/shared/engine/aggregates/CaptureAggregate.ts) type.
- [`GameEngine.hasAnyRealActionForPlayer`](../../src/server/game/GameEngine.ts) and test-only helpers [`GameEngine.resolveBlockedStateForCurrentPlayerForTesting`](../../src/server/game/GameEngine.ts) / [`GameEngine.stepAutomaticPhasesForTesting`](../../src/server/game/GameEngine.ts) now detect global capture availability via [`enumerateAllCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts) (re-exported as `enumerateAllCaptureMovesAggregate`) instead of any backend-specific ray-walk logic.
- [`TurnEngine.hasValidCaptures`](../../src/server/game/turn/TurnEngine.ts) and [`TurnEngine.hasValidActions`](../../src/server/game/turn/TurnEngine.ts) implement the `hasAnyCapture` predicate entirely via the shared [`enumerateAllCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts) helper, with additional filtering for `mustMoveFromStackKey` so that forced-move semantics remain consistent with [`GameEngine.getValidMoves`](../../src/server/game/GameEngine.ts).
- [`RuleEngine.getValidMoves` capture/movement branches](../../src/server/game/RuleEngine.ts) delegate capture enumeration to [`getValidCaptures`](../../src/server/game/RuleEngine.ts), which wraps the canonical per-stack [`enumerateCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts). No backend code walks capture rays directly.
- [`RuleEngine.validateCapture`](../../src/server/game/RuleEngine.ts) and [`RuleEngine.validateChainCaptureContinuation`](../../src/server/game/RuleEngine.ts) validate segments via the shared [`validateCaptureSegmentOnBoard`](../../src/shared/engine/core.ts) helper using a [`CaptureSegmentBoardView`](../../src/shared/engine/aggregates/CaptureAggregate.ts) adapter; all geometry and path-blocking rules for capture segments now live in the shared engine.
- [`RuleEngine.processCapture`](../../src/server/game/RuleEngine.ts) has been reduced to a thin adapter over `applyCaptureAggregate`, mutating the cloned `GameState` returned by [`RuleEngine.cloneGameState`](../../src/server/game/RuleEngine.ts) with `result.newState`. Marker-path effects, ring transfer, and landing-on-own-marker elimination are all implemented only in [`mutateCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts).

Legacy helpers and residual backend-specific capture code have been isolated to diagnostics only:

- [`RuleEngine.processChainReactions`](../../src/server/game/RuleEngine.ts), [`RuleEngine.getCaptureDirections`](../../src/server/game/RuleEngine.ts), [`RuleEngine.isPathClear`](../../src/server/game/RuleEngine.ts), and [`RuleEngine.isStraightLineMovement`](../../src/server/game/RuleEngine.ts) are no longer used by any live capture enumeration or mutation path. They are retained only for historical/debugging purposes and are referenced via [`RuleEngine._debugUseInternalHelpers`](../../src/server/game/RuleEngine.ts) until they are formally removed in Phase 5 clean-up.
- [`GameEngine.performOvertakingCapture`](../../src/server/game/GameEngine.ts) is similarly unused; all backend capture application now flows through the shared aggregate, and this method is kept only as a legacy reference for comparison against the consolidated implementation.

Backend tests exercising capture behaviour at the engine surface have been re-run against this wiring:

- [`tests/unit/GameEngine.chainCapture.test.ts`](../../tests/unit/GameEngine.chainCapture.test.ts) — validates multi-step chain-capture behaviour via `GameEngine.makeMove` and `GameEngine.getValidMoves` in the `chain_capture` phase.
- [`tests/scenarios/RulesMatrix.ChainCapture.GameEngine.test.ts`](../../tests/scenarios/RulesMatrix.ChainCapture.GameEngine.test.ts) — scenario-level chain-capture flows through `GameEngine`, ensuring continuation options and termination semantics match the shared core.
- [`tests/unit/CaptureMarker.GameEngine_vs_Sandbox.test.ts`](../../tests/unit/CaptureMarker.GameEngine_vs_Sandbox.test.ts) — updated so that the backend-style path applies captures via [`applyCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts) on a backend-shaped `GameState` and compares the resulting board to the sandbox capture helpers, confirming that both adapters agree on marker + capture interactions.
...
- [x] Sandbox capture helpers [`sandboxCaptures.ts`](../../src/client/sandbox/sandboxCaptures.ts) use TS capture core for reachability decisions where possible (or share common helpers with backend).
- [x] Python capture methods in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py) (`_get_capture_moves`, `_apply_chain_capture`) are reviewed and documented against the canonical capture-chain spec, with any deviations surfaced via tests.
- [x] Chain-capture parity tests in [`test_chain_capture_parity.py`](../../ai-service/tests/parity/test_chain_capture_parity.py) remain green.
...
This subphase aligns the **TS sandbox capture implementation** with the shared capture core so that all rules‑level capture enumeration and application flows through [`CaptureAggregate.ts`](../../src/shared/engine/aggregates/CaptureAggregate.ts), with the sandbox acting purely as an adapter plus UI tooling.

**Sandbox entry points now calling shared capture APIs**

- **Per‑stack capture enumeration (sandbox helpers):**
  - [`enumerateCaptureSegmentsFromBoard()`](../../src/client/sandbox/sandboxCaptures.ts) is a thin adapter over the canonical [`enumerateCaptureMoves()`](../../src/shared/engine/aggregates/CaptureAggregate.ts) via [`captureLogic.enumerateCaptureMoves()`](../../src/shared/engine/captureLogic.ts). It builds a lightweight [`CaptureBoardAdapters`](../../src/shared/engine/aggregates/CaptureAggregate.ts) view from the sandbox `BoardState`.
...
- **UI landing enumeration and highlights:**
  - [`enumerateCaptureSegmentsFrom()`](../../src/client/sandbox/ClientSandboxEngine.ts) constructs sandbox `CaptureBoardAdapters` and delegates to `enumerateCaptureSegmentsFromBoard`.
  - [`getValidLandingPositionsForCurrentPlayer()`](../../src/client/sandbox/ClientSandboxEngine.ts):
    - First calls `enumerateCaptureSegmentsFrom(from, currentPlayer)` to obtain canonical capture landings from the shared core.
    - Falls back to [`enumerateSimpleMovementLandings()`](../../src/client/sandbox/ClientSandboxEngine.ts) (which itself uses shared movement helpers) only when no capture segments exist.
...
- **Capture application (sandbox engine + hooks):**
  - [`applyCaptureSegment()`](../../src/client/sandbox/ClientSandboxEngine.ts) is now a thin wrapper over [`applyCaptureSegment`](../../src/shared/engine/aggregates/CaptureAggregate.ts):
    ...
    - All marker path effects, ring transfer, landing‑on‑own‑marker elimination, and defensive marker clearing are owned solely by [`mutateCapture()`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
  - `ClientSandboxEngine.applyCaptureSegment()` calls the shared [`applyCaptureSegment`](../../src/shared/engine/aggregates/CaptureAggregate.ts), updating the host’s `GameState` directly.
  - The older sandbox‑local mutator [`applyCaptureSegmentOnBoard()`](../../src/client/sandbox/sandboxCaptures.ts) remains only for **analysis/debug** use (e.g. offline chain enumeration tests); its doc comment explicitly marks it as non‑canonical and not used by live engine flows.

- **Chain continuation semantics (legacy sandbox path):**
  - In the non‑orchestrator capture branch of [`applyCanonicalMoveInternal()`](../../src/client/sandbox/ClientSandboxEngine.ts), the capture cases:
    - Apply the segment via `this.applyCaptureSegment(...)`, which delegates to `CaptureAggregate.applyCaptureSegment`.
    - Ask the shared core whether the chain must continue using [`getChainCaptureContinuationInfoAggregate()`](../../src/client/sandbox/ClientSandboxEngine.ts), which maps directly to [`getChainCaptureContinuationInfo()`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
...
- **Global “any capture?” checks in sandbox flows:**
  - Sandbox turn/lifecycle helpers use shared reachability primitives for forced‑elimination and last‑player‑standing logic:
    - No‑dead‑placement and “has any capture from this stack” are enforced via [`hasAnyLegalMoveOrCaptureFromOnBoard()`](../../src/shared/engine/core.ts) underneath the sandbox adapter (see [`hasAnyLegalMoveOrCaptureFrom()`](../../src/client/sandbox/ClientSandboxEngine.ts)).
    - This matches the backend’s use of `hasAnyLegalMoveOrCaptureFromOnBoard` and [`enumerateAllCaptureMoves()`](../../src/shared/engine/aggregates/CaptureAggregate.ts) inside `TurnEngine`.
...
- [`sandboxCaptures.applyCaptureSegmentOnBoard()`](../../src/client/sandbox/sandboxCaptures.ts)
  - Retained as a lightweight, mutable board helper used only by:
    - Chain search / enumeration tooling (e.g. [`captureSequenceEnumeration.test.ts`](../../tests/unit/captureSequenceEnumeration.test.ts)).
...
- [`sandboxCaptureSearch.findMaxCaptureChains()`](../../src/client/sandbox/sandboxCaptureSearch.ts)
  ...
  - Its header comment explicitly documents:
    ...
    - All canonical legality and continuation semantics must instead come from `CaptureAggregate` (or shims in [`captureLogic.ts`](../../src/shared/engine/captureLogic.ts) / [`chainCaptureTracking.ts`](../../src/shared/engine/chainCaptureTracking.ts)).

**Tests updated / extended for sandbox–core parity**

- **Single‑segment capture parity (backend vs sandbox):**
  - [`CaptureMarker.GameEngine_vs_Sandbox.test.ts`](../../tests/unit/CaptureMarker.GameEngine_vs_Sandbox.test.ts) already compares:
    - Backend path: applying a single [`overtaking_capture`](../../src/shared/types/game.ts) via [`applyCapture()`](../../src/shared/engine/aggregates/CaptureAggregate.ts) on a backend‑shaped `GameState`.
    - Sandbox path: applying the corresponding segment through [`ClientSandboxEngine.applyCaptureSegment()`](../../src/client/sandbox/ClientSandboxEngine.ts), which now delegates to `CaptureAggregate.applyCaptureSegment`.
...
- **Two‑segment chain capture parity (backend vs sandbox):**
  - A new test in [`CaptureMarker.GameEngine_vs_Sandbox.test.ts`](../../tests/unit/CaptureMarker.GameEngine_vs_Sandbox.test.ts) exercises a **two‑segment chain**:
    - Backend:
      - Applies two capture moves (`'overtaking_capture'` then `'continue_capture_segment'`) via [`applyCapture()`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and records the final `GameState`.
    - Sandbox:
      - Seeds a `ClientSandboxEngine` with the same board and players.
      - Invokes the internal [`applyCaptureSegment()`](../../src/client/sandbox/ClientSandboxEngine.ts) twice with the same `(from, target, landing, player)` triples.
...
- **Capture sequence enumeration parity (sandbox helper vs aggregate):**
  - [`captureSequenceEnumeration.test.ts`](../../tests/unit/captureSequenceEnumeration.test.ts) remains the primary sanity check for:
    - Enumerating bounded capture chains using `sandboxCaptures.enumerateCaptureSegmentsFromBoard` + `applyCaptureSegmentOnBoard`.
    - Matching those sequences against continuation information and geometry implied by [`getChainCaptureContinuationInfo()`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and `CaptureAggregate` reachability on equivalent test boards.
...
On the Python side, chain‑capture semantics are implemented in [`GameEngine`](../../ai-service/app/game_engine/__init__.py) and validated against the TS SSoT via dedicated parity tests:

- **Capture enumeration:**
  - [`GameEngine._get_capture_moves`](../../ai-service/app/game_engine/__init__.py) ray‑walks from an attacker in each movement direction to find candidate targets and landings, then validates each `(from, target, landing)` triple via an internal `_validate_capture_segment_on_board` helper.
  - `_validate_capture_segment_on_board` is a direct Python analogue of TS [`validateCaptureSegmentOnBoard`](../../src/shared/engine/core.ts), enforcing:
...
- **Capture application and chain state:**
  - `_apply_chain_capture` (see [`GameEngine._apply_chain_capture`](../../ai-service/app/game_engine/__init__.py)) mirrors TS [`mutateCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and the chain state helpers in `CaptureAggregate`:
...
- **Parity tests:**
  - [`test_chain_capture_parity.py`](../../ai-service/tests/parity/test_chain_capture_parity.py) covers a canonical chain scenario:
...
This section captures the **single source of truth** for overtaking capture-chains across all hosts. It is grounded in [`RR-CANON-R100–R103`](../../RULES_CANONICAL_SPEC.md) and the termination analysis in [`RULES_TERMINATION_ANALYSIS.md`](../supplementary/RULES_TERMINATION_ANALYSIS.md), and in the current shared TS implementation in [`CaptureAggregate.ts`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and [`captureLogic.ts`](../../src/shared/engine/captureLogic.ts).
...
- Local stack conditions (from [`RR-CANON-R101`](../../RULES_CANONICAL_SPEC.md)):
...
- Geometry and path:
  - `from`, `target`, `landing` lie on a straight movement ray for the board type, using movement adjacency from [`RR-CANON-R001–R003`](../../RULES_CANONICAL_SPEC.md).
  ...
  - Total distance `distance(from, landing) ≥ H`, where `distance` is Chebyshev on square boards and cube distance on hex, as in [`core.calculateDistance`](../../src/shared/engine/core.ts) and [`BoardGeometry.calculate_distance`](../../ai-service/app/rules/geometry.py).
- Landing cell:
...
These conditions are enforced in TS via [`validateCaptureSegmentOnBoard`](../../src/shared/engine/core.ts), and in Python via [`GameEngine._validate_capture_segment_on_board_for_reachability`](../../ai-service/app/game_engine/__init__.py).

**Application semantics** of a single legal segment (from [`RR-CANON-R102`](../../RULES_CANONICAL_SPEC.md)):
...
These semantics are implemented in TS via [`mutateCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and in Python via [`GameEngine._apply_chain_capture`](../../ai-service/app/game_engine/__init__.py).
...
Normative chain rules (from [`RR-CANON-R103`](../../RULES_CANONICAL_SPEC.md)):
...
Canonical TS chain state is represented by [`ChainCaptureState`](../../src/shared/engine/aggregates/CaptureAggregate.ts):
...
For **enumeration** purposes, a lighter snapshot [`ChainCaptureStateSnapshot`](../../src/shared/engine/aggregates/CaptureAggregate.ts) is used:
...
In Python, [`ChainCaptureState`](../../ai-service/app/models/core.py) mirrors the TS shape and is maintained by [`GameEngine._apply_chain_capture`](../../ai-service/app/game_engine/__init__.py).

**Visited-set semantics (normative vs implementation):**

- The **rules** do **not** forbid revisiting previously targeted stacks or previously occupied positions within a chain, as long as each individual segment is legal under [`RR-CANON-R101`](../../RULES_CANONICAL_SPEC.md).
- TS provides an optional `disallowRevisitedTargets` flag on [`ChainCaptureEnumerationOptions`](../../src/shared/engine/aggregates/CaptureAggregate.ts) that filters candidate segments whose `captureTarget` appears in `capturedThisChain`. This is a **search/heuristic** knob (used in offline analysis like [`findCyclicCaptures.js`](../../scripts/findCyclicCaptures.js)), not part of the legal-move surface.
- The canonical **legal-move** enumeration used for `getValidMoves` in capture or `chain_capture` phases **must not** exclude segments solely because they revisit a previously targeted stack or landing position. In TS, [`getChainCaptureContinuationInfo`](../../src/shared/engine/aggregates/CaptureAggregate.ts) calls [`enumerateChainCaptureSegments`](../../src/shared/engine/aggregates/CaptureAggregate.ts) without `disallowRevisitedTargets`, which is the required behaviour.
...
The semantic model therefore treats a full chain as a **path** in a tree whose edges are segments and whose branching factor is “number of legal segments from the current position”. This is reflected in TS by [`enumerateChainCaptureSegments`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and in Python by [`GameEngine._get_capture_moves`](../../ai-service/app/game_engine/__init__.py) when `game_state.chain_capture_state` is set.
...
1. **No legal continuation**
   - After applying the latest segment, the shared continuation helper reports `mustContinue === false` and an empty continuation set, as in [`getChainCaptureContinuationInfo`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
   - Hosts then:
     - Clear `chainCaptureState`.
     - Advance to line processing and territory processing per [`RR-CANON-R070–R071`](../../RULES_CANONICAL_SPEC.md).
...
2. **Player chooses a terminal branch**
   ...
   - This is precisely the “end-early” allowance in [`RR-CANON-R103`](../../RULES_CANONICAL_SPEC.md).
...
3. **Global termination of the game**
   ...
   - In these cases, the game ends after the chain, but before the next player acts, per [`RR-CANON-R170–R173`](../../RULES_CANONICAL_SPEC.md) and the S-invariant analysis in [`RULES_TERMINATION_ANALYSIS.md`](../supplementary/RULES_TERMINATION_ANALYSIS.md).
...
- Compute:
  - `hasAnyNonCaptureMovement` using movement rules [`RR-CANON-R090–R092`](../../RULES_CANONICAL_SPEC.md).
  - `hasAnyCapture` using capture rules [`RR-CANON-R101`](../../RULES_CANONICAL_SPEC.md).
- Cases:
  ...
  - If `hasAnyNonCaptureMovement === false` and `hasAnyCapture === false` but P controls at least one stack: P must perform a **forced elimination** per [`RR-CANON-R100`](../../RULES_CANONICAL_SPEC.md) (eliminate an entire cap from a controlled stack).
  - If P controls no stacks and has no placements, they are temporarily inactive for that turn (see [`RR-CANON-R072`](../../RULES_CANONICAL_SPEC.md) and [`RR-CANON-R172`](../../RULES_CANONICAL_SPEC.md)).

In TS, the global “does this player have any capture options?” predicate is implemented via shared helpers such as [`core.hasAnyLegalMoveOrCaptureFromOnBoard`](../../src/shared/engine/core.ts) and [`CaptureAggregate.enumerateAllCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts), and is used by the turn logic in [`TurnEngine.hasValidCaptures`](../../src/server/game/turn/TurnEngine.ts) and [`TurnEngine.hasValidActions`](../../src/server/game/turn/TurnEngine.ts). Python mirrors this via [`GameEngine._has_valid_captures`](../../ai-service/app/game_engine/__init__.py) and [`GameEngine._has_valid_actions`](../../ai-service/app/game_engine/__init__.py).
...
- Chains may:
  ...
  - Capture additional rings from the same stack in multiple segments, as long as [`RR-CANON-R101`](../../RULES_CANONICAL_SPEC.md) is satisfied each time.
- Cycles do **not** threaten termination because:
  - Every legal movement or capture strictly increases the progress metric `S = M + C + E`, and `S` is bounded above, as proven in [`RR-CANON-R191`](../../RULES_CANONICAL_SPEC.md) and elaborated in [`RULES_TERMINATION_ANALYSIS.md`](../supplementary/RULES_TERMINATION_ANALYSIS.md).
...
- **Search / analysis tools:**
  - Tools such as [`sandboxCaptureSearch.findMaxCaptureChains`](../../src/client/sandbox/sandboxCaptureSearch.ts) may take an option like `pruneVisitedPositions` to avoid enumerating very long cyclic trees; this is acceptable because:
...
- **Directions**: rays follow `movementAdjacency` directions from [`RR-CANON-R001–R003`](../../RULES_CANONICAL_SPEC.md):
  - `square8` and `square19`: 8-direction Moore neighbourhood (orthogonal + diagonal).
  - `hexagonal`: 6 cube-axis directions.
  - Implemented via [`getMovementDirectionsForBoardType`](../../src/shared/engine/core.ts) in TS and [`BoardManager._get_all_directions`](../../ai-service/app/board_manager.py) in Python.
- **Distance**:
  - Square boards: `distance(from, to) = max(|dx|, |dy|)`.
  - Hex: `distance = (|dx| + |dy| + |dz|) / 2`.
  - Implemented via [`core.calculateDistance`](../../src/shared/engine/core.ts) and [`BoardGeometry.calculate_distance`](../../ai-service/app/rules/geometry.py).
- **Path blocking**:
  ...
  - Implemented in TS by [`validateCaptureSegmentOnBoard`](../../src/shared/engine/core.ts) and in Python by `_validate_capture_segment_on_board_for_reachability` in [`GameEngine`](../../ai-service/app/game_engine/__init__.py).
...
- T₁ is overtaken **twice** in the same chain (once in `s₁`, again in `s₃`), as allowed by [`RR-CANON-R103`](../../RULES_CANONICAL_SPEC.md).
...
- [`ChainCaptureSegment`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
...
- [`ChainCaptureState`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
...
- [`ChainCaptureStateSnapshot`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  - Lightweight struct for enumeration:
    - `player`
    - `currentPosition`
    - `capturedThisChain: Position[]` (targets already captured in this chain).

- [`ChainCaptureEnumerationOptions`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
...
- [`ChainCaptureContinuationInfo`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  - Return type describing continuations from a position:
    - `mustContinue: boolean`
    - `availableContinuations: Move[]`

- [`CaptureSegmentParams`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  - Parameters for applying a single segment:
    - `from`, `target`, `landing`, `player`.

- [`CaptureApplicationOutcome`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  - Result of applying one segment:
    - `nextState: GameState`
    - `ringsTransferred: number` (normally `1`)
    - `chainContinuationRequired: boolean`

- [`CaptureValidationResult`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and [`CaptureMutationResult`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
...
- [`enumerateCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  **Signature:**
  ...
  - Behaviour:
    ...
    - Validates each candidate segment via [`validateCaptureSegmentOnBoard`](../../src/shared/engine/core.ts).
...
- [`validateCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  - Validates an [`OvertakingCaptureAction`](../../src/shared/engine/types.ts) against a full `GameState`:
    ...
    - Delegates segment legality to [`validateCaptureSegmentOnBoard`](../../src/shared/engine/core.ts).
  - Returns a [`CaptureValidationResult`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
...
- [`updateChainCaptureStateAfterCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  - Host utility for maintaining [`ChainCaptureState`](../../src/shared/engine/aggregates/CaptureAggregate.ts) after each applied capture `Move`:
...
- [`applyCaptureSegment`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  - Pure helper that:
    - Builds an `OvertakingCaptureAction` from [`CaptureSegmentParams`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
    - Delegates to `mutateCapture`.
    - Invokes `getChainCaptureContinuationInfo` on the result.
    - Returns a [`CaptureApplicationOutcome`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
...
- [`applyCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts)
  ...
  - Returns a [`CaptureMutationResult`](../../src/shared/engine/aggregates/CaptureAggregate.ts) discriminated union.

**Host responsibilities (TS):**

- Backend [`GameEngine.makeMove`](../../src/server/game/GameEngine.ts) and [`GameEngine.getValidMoves`](../../src/server/game/GameEngine.ts) must:
...
- Sandbox [`ClientSandboxEngine`](../../src/client/sandbox/ClientSandboxEngine.ts) must:
...
- [`PyChainCaptureSegment`](../../ai-service/app/rules/core.py)
  - Dataclass analogue of TS [`ChainCaptureSegment`](../../src/shared/engine/aggregates/CaptureAggregate.ts):
    - `from_pos: Position`
    - `target_pos: Position`
    - `landing_pos: Position`
    - `captured_cap_height: int`

- [`PyChainCaptureStateSnapshot`](../../ai-service/app/rules/core.py)
  - Equivalent to TS [`ChainCaptureStateSnapshot`](../../src/shared/engine/aggregates/CaptureAggregate.ts):
    - `player: int`
    - `current_position: Position`
    - `captured_this_chain: list[Position]`

- [`PyChainCaptureContinuationInfo`](../../ai-service/app/rules/core.py)
  - Equivalent to TS [`ChainCaptureContinuationInfo`](../../src/shared/engine/aggregates/CaptureAggregate.ts):
    - `must_continue: bool`
    - `available_continuations: list[Move]`

- [`PyCaptureSegmentParams`](../../ai-service/app/rules/core.py) and [`PyCaptureApplicationOutcome`](../../ai-service/app/rules/core.py)
  - Shape aligned with TS `CaptureSegmentParams` / `CaptureApplicationOutcome`.

Python already defines [`ChainCaptureState`](../../ai-service/app/models/core.py); hosts should continue to own this and update it using the helpers below.

##### 2.2 Enumeration functions

- [`enumerate_capture_moves_py()`](../../ai-service/app/rules/core.py)
  **Signature (proposed):**
  `def enumerate_capture_moves_py(state: GameState, player: int, from_pos: Position, *, move_number: int, kind: str = "initial") -> list[Move]: ...`
  - Behaviour:
    - Uses `BoardGeometry` and `BoardManager` to enumerate all legal capture segments from `from_pos`, mirroring TS [`enumerateCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
    - Sets `Move.type` to:
      - `MoveType.OVERTAKING_CAPTURE` when `kind == "initial"`.
      - `MoveType.CONTINUE_CAPTURE_SEGMENT` when `kind == "continuation"`.
    - Does **not** mutate `state`.

- [`enumerate_chain_capture_segments_py()`](../../ai-service/app/rules/core.py)
  **Signature (proposed):**
  ...
  - Behaviour:
    - Equivalent to TS [`enumerateChainCaptureSegments`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
...
- [`get_chain_capture_continuation_info_py()`](../../ai-service/app/rules/core.py)
  **Signature (proposed):**
...
These functions will replace the inline enumeration logic currently in [`GameEngine._get_capture_moves`](../../ai-service/app/game_engine/__init__.py), which should be reduced to a host adapter that:
...
- [`apply_capture_segment_py()`](../../ai-service/app/rules/core.py)
  **Signature (proposed):**
  `def apply_capture_segment_py(state: GameState, params: PyCaptureSegmentParams) -> PyCaptureApplicationOutcome: ...`
  - Behaviour:
    - Purely applies a single capture segment to a **copy** of `state`, mirroring TS [`applyCaptureSegment`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
    - Reuses:
      - [`BoardManager`](../../ai-service/app/board_manager.py) for stack/marker writes.
      - [`GameEngine._process_markers_along_path`](../../ai-service/app/game_engine/__init__.py) for marker semantics.
      - [`GameEngine._eliminate_top_ring_at`](../../ai-service/app/game_engine/__init__.py) for landing-on-own-marker elimination.
    - Computes `chainContinuationRequired` using `get_chain_capture_continuation_info_py` on the resulting state.

- [`apply_capture_py()`](../../ai-service/app/rules/core.py)
  **Signature (proposed):**
...
- [`update_chain_capture_state_py()`](../../ai-service/app/rules/core.py)
  - Python analogue of TS [`updateChainCaptureStateAfterCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts), updating the `chain_capture_state` field on `GameState` using the shared [`ChainCaptureState`](../../ai-service/app/models/core.py) dataclass.

**Host responsibilities (Python):**

- [`CaptureValidator.validate`](../../ai-service/app/rules/validators/capture.py) continues to:
...
- [`CaptureMutator.apply`](../../ai-service/app/rules/mutators/capture.py) becomes a thin wrapper:
...
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Single-segment validation                      | [`RuleEngine.validateCapture`](../../src/server/game/RuleEngine.ts)                                                                                                                             | Delegate geometry and path checks to [`validateCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts), which wraps [`validateCaptureSegmentOnBoard`](../../src/shared/engine/core.ts).                                                                                      | No behavioural change expected; this is already effectively the case.                                                                                                                                                                                                                                    |
| Capture enumeration in movement/capture phases | [`RuleEngine.getValidMoves` (capture branch)](../../src/server/game/RuleEngine.ts) and any remaining logic in [`captureLogic.ts`](../../src/shared/engine/captureLogic.ts)                      | Replace ray-walk logic with [`enumerateAllCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and/or [`enumerateCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts).                                                                             | Expected behaviour: identical or stricter; any divergence should be surfaced by existing parity fixtures (e.g. [`trace_scenario_chain_capture.json`](../../ai-service/tests/parity/vectors/trace_scenario_chain_capture.json)). Legacy `captureChainEngine` helpers become thin adapters or are removed. |
| Applying a single capture segment              | [`GameEngine.performOvertakingCapture`](../../src/server/game/GameEngine.ts)                                                                                                                    | Internally delegate to [`applyCaptureSegment`](../../src/shared/engine/aggregates/CaptureAggregate.ts) or [`applyCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts) instead of duplicating marker/stack logic.                                                          | Minor internal refactor; semantics already match shared core. Any difference around landing-on-opponent-marker should be normalised to “illegal by enumeration; defensive removal only”.                                                                                                                 |
| Chain-capture phase continuation               | [`GameEngine.makeMove` chain-capture branch](../../src/server/game/GameEngine.ts) and [`GameEngine.getValidMoves` when `currentPhase === 'chain_capture'`](../../src/server/game/GameEngine.ts) | Use [`ChainCaptureState`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and [`getChainCaptureContinuationInfo`](../../src/shared/engine/aggregates/CaptureAggregate.ts) as the sole source of continuation options, relabelling `Move.type` as `'continue_capture_segment'`. | Behaviour is already aligned; this migration mainly removes bespoke relabelling code and ensures `disallowRevisitedTargets` is never used for rules-level enumeration.                                                                                                                                   |
| Forced-capture-only detection                  | [`TurnEngine.hasValidCaptures`](../../src/server/game/turn/TurnEngine.ts) and [`TurnEngine.hasValidActions`](../../src/server/game/turn/TurnEngine.ts)                                          | Implement `hasAnyCapture` via [`enumerateAllCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts).                                                                                                                                                                    | No intended semantic change; ensures forced elimination is only entered when **no** captures exist anywhere on the board.                                                                                                                                                                                |

#### Sandbox (TS)

| Concern                                 | Current entry point                                                                                                                      | Mapping to canonical API                                                                                                                                                                                                                                                                                                       | Behaviour notes                                                                                                                                          |
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Per-stack capture enumeration           | [`sandboxCaptures.enumerateCaptureSegmentsFromBoard`](../../src/client/sandbox/sandboxCaptures.ts)                                       | Continue delegating to shared [`enumerateCaptureMoves`](../../src/shared/engine/aggregates/CaptureAggregate.ts) via adapters; treat this as the canonical path.                                                                                                                                                                | Already wired; this task freezes it as the official route.                                                                                               |
| Single-segment application              | [`sandboxCaptures.applyCaptureSegmentOnBoard`](../../src/client/sandbox/sandboxCaptures.ts)                                              | Align semantics with [`applyCaptureSegment`](../../src/shared/engine/aggregates/CaptureAggregate.ts): departure marker, two-leg marker processing, target ring transfer, landing-on-own-marker elimination. Optionally, refactor to call the shared helper via an adapter.                                                     | Today this closely matches backend semantics; any differences (e.g. marker clearing at landing) are already being corrected and should be fully aligned. |
| Chain search / AI heuristics            | [`sandboxCaptureSearch.findMaxCaptureChains`](../../src/client/sandbox/sandboxCaptureSearch.ts)                                          | Treat this as a **diagnostic/search** tool that uses shared enumeration (`enumerateCaptureSegmentsFromBoard`) plus optional `pruneVisitedPositions`. It should not be used for rules-level legality.                                                                                                                           | Behaviour can retain optional visited-set pruning. This spec clarifies that `pruneVisitedPositions` is heuristic-only.                                   |
| Canonical move application for captures | [`ClientSandboxEngine.applyCanonicalMoveInternal` (capture and chain-capture branches)](../../src/client/sandbox/ClientSandboxEngine.ts) | For non-orchestrator paths, delegate capture application to [`applyCapture`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and continuation decisions to [`getChainCaptureContinuationInfo`](../../src/shared/engine/aggregates/CaptureAggregate.ts). For orchestrator paths, rely on shared orchestrator semantics. | Expected to reduce duplication between backend and sandbox; semantics should remain unchanged where parity tests already exist.                          |

#### Python (ai-service)

| Concern                                  | Current entry point                                                                                                                                                                                                                                           | Mapping to canonical API                                                                                                                                                                                                          | Behaviour notes                                                                                                                                             |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Capture validation                       | [`CaptureValidator.validate`](../../ai-service/app/rules/validators/capture.py)                                                                                                                                                                               | Continue enforcing phase/turn/must-move checks, but delegate segment legality to shared functions (either `_validate_capture_segment_on_board_for_reachability` or the new `enumerate_capture_moves_py` for reachability checks). | No intended behaviour change; this just clarifies that geometry rules must remain centralised.                                                              |
| Capture enumeration                      | [`GameEngine._get_capture_moves`](../../ai-service/app/game_engine/__init__.py)                                                                                                                                                                               | Reduce to a thin adapter over [`enumerate_chain_capture_segments_py`](../../ai-service/app/rules/core.py), using `chain_capture_state` (if present) or last move's `to` field to select `current_position`.                       | This removes the local ray-walk implementation and ensures TS–Python parity for capture reachability.                                                       |
| Single-segment application               | [`GameEngine._apply_chain_capture`](../../ai-service/app/game_engine/__init__.py)                                                                                                                                                                             | Internally call [`apply_capture_segment_py`](../../ai-service/app/rules/core.py) or `apply_capture_py`, keeping hash and logging semantics local to `GameEngine`.                                                                 | Behaviour is already intended to match TS; any remaining edge cases (e.g. landing on opponent markers) will be aligned with TS during migration.            |
| Mutator entry point                      | [`CaptureMutator.apply`](../../ai-service/app/rules/mutators/capture.py)                                                                                                                                                                                      | Delegate to `apply_capture_py` (or, during an intermediate phase, to `GameEngine.apply_move` which itself uses the canonical helpers).                                                                                            | No semantic change; this just removes duplication of capture application logic.                                                                             |
| Forced-capture-only / forced elimination | [`GameEngine._has_valid_captures`](../../ai-service/app/game_engine/__init__.py), [`GameEngine._has_valid_actions`](../../ai-service/app/game_engine/__init__.py), [`GameEngine._get_forced_elimination_moves`](../../ai-service/app/game_engine/__init__.py) | Ensure `has_valid_captures` uses `enumerate_chain_capture_segments_py` (or `enumerate_capture_moves_py`) for consistency with TS.                                                                                                 | Behaviour should remain the same; this task explicitly documents that forced elimination is only entered when **no** capture (or other real action) exists. |
...
- TS–Python contract vectors, especially [`test_chain_capture_parity.py`](../../ai-service/tests/parity/test_chain_capture_parity.py).
- Strict invariants and soak tests documented in [`STRICT_INVARIANT_SOAKS.md`](../testing/STRICT_INVARIANT_SOAKS.md).

must be treated as either:

- Bugs in pre-existing implementations to be fixed to match this spec, or
- Genuine ambiguities in the rules requiring updates to [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md).
...
- Sandbox and backend movement code that is mostly adapter glue:
  ...
  - Backend non-capturing movement in [`GameEngine.applyMove`](../../src/server/game/GameEngine.ts) delegates `move_stack` / `move_ring` mutation to `applySimpleMovementAggregate`, then merges the resulting `GameState.board` back into the existing board for identity-preserving updates.
...
- [ ] Backend placement validation in [`RuleEngine.ts`](../../src/server/game/RuleEngine.ts) and sandbox placement helpers [`sandboxPlacement.ts`](../../src/client/sandbox/sandboxPlacement.ts) delegate fully to the shared placement aggregate.
...
This section captures the **single source of truth** for ring placement semantics and placement APIs across all hosts. It is grounded in [`RR-CANON-R020`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R070–R072`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R080–R082`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R090–R092`](../../RULES_CANONICAL_SPEC.md), [`RR-CANON-R100`](../../RULES_CANONICAL_SPEC.md), and [`RR-CANON-R172`](../../RULES_CANONICAL_SPEC.md), together with the shared no-dead-placement core described in [`RULES_ENGINE_ARCHITECTURE.md` §6.2](RULES_ENGINE_ARCHITECTURE.md). It reconciles existing TS behaviour in [`PlacementAggregate.ts`](../../src/shared/engine/aggregates/PlacementAggregate.ts), [`placementHelpers.ts`](../../src/shared/engine/placementHelpers.ts), backend [`RuleEngine.ts`](../../src/server/game/RuleEngine.ts) / [`GameEngine.ts`](../../src/server/game/GameEngine.ts), sandbox [`ClientSandboxEngine.ts`](../../src/client/sandbox/ClientSandboxEngine.ts) / [`sandboxPlacement.ts`](../../src/client/sandbox/sandboxPlacement.ts), and Python behaviour in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py), [`validators/placement.py`](../../ai-service/app/rules/validators/placement.py), and [`mutators/placement.py`](../../ai-service/app/rules/mutators/placement.py).
...
- Define **legal placement cells** and **stacking rules** in a way that:
  - Preserves the own-colour ring caps of [`RR-CANON-R020`](../../RULES_CANONICAL_SPEC.md).
  - Respects board-cell exclusivity invariants (stack vs marker vs collapsed) from [`RULES_ENGINE_ARCHITECTURE.md` §5.0](RULES_ENGINE_ARCHITECTURE.md).
- Make the shared no-dead-placement rule (NDP) from [`RULES_ENGINE_ARCHITECTURE.md` §6.2](RULES_ENGINE_ARCHITECTURE.md) the **only** legality criterion for "does this placement leave at least one real action from the placed stack".
- Specify **skip-placement** semantics and their interaction with forced elimination and LPS such that:
  ...
  - LPS counting uses the same definition of "real action" as [`RR-CANON-R172`](../../RULES_CANONICAL_SPEC.md).
...
- Placement always occurs in the `ring_placement` phase as defined in [`RR-CANON-R070`](../../RULES_CANONICAL_SPEC.md):
...
- For current player P:
  ...
  - Let `hasAnyMoveOrCapture` be "there exists at least one legal non-capture move or overtaking capture for P from any controlled stack", computed via the shared movement/capture helpers (`MovementAggregate`, [`CaptureAggregate.ts`](../../src/shared/engine/aggregates/CaptureAggregate.ts)) or their Python analogues.
- [`RR-CANON-R080`](../../RULES_CANONICAL_SPEC.md) defines when placement is **forbidden**, **mandatory**, and **optional**:
...
- **Per-cell caps (from existing TS/Python behaviour and consistent with [`RR-CANON-R081–R082`](../../RULES_CANONICAL_SPEC.md)):**
  - On an **empty** cell:
...
- **Initial placements vs midgame:**
  ...
  - Board-type differences (square8, square19, hex8, hexagonal) affect:
    - The total own-colour supply cap (`ringsPerPlayer`, [`RR-CANON-R020`](../../RULES_CANONICAL_SPEC.md)).
...
- **Stacking on empty cells (multi-ring):**
  - Legal placement is:
    ...
    - In TS, this is validated by [`validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) using a [`PlacementContext`](../../src/shared/engine/aggregates/PlacementAggregate.ts).
    - In Python, by `_get_ring_placement_moves` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py) which enumerates `PLACE_RING` moves with allowed counts.
...
- **Stacking on existing stacks (single ring):**
  ...
  - Application semantics:
    ...
    - Recompute `controllingPlayer`, `stackHeight`, and `capHeight` according to [`RR-CANON-R022`](../../RULES_CANONICAL_SPEC.md).
    - This is implemented in TS by [`applyPlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) and in Python by `_apply_place_ring` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py).
- **Global own-colour caps:**
  - For each placement, P must satisfy [`RR-CANON-R020`](../../RULES_CANONICAL_SPEC.md):
  ...
  - In TS:
    - [`PlacementContext`](../../src/shared/engine/aggregates/PlacementAggregate.ts) carries:
      - Board type and per-player caps.
      - Current `ringsInHand[P]` and (optionally) a precomputed count of P-colour rings on board.
    - [`PlacementAggregate.validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) rejects placements that would exceed the cap.
  - In Python:
    - `_estimate_rings_per_player` and `_count_rings_of_player_on_board` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py) implement equivalent checks in `_get_ring_placement_moves`.

##### 2.4 Markers, Territory & board types

- Placement **never**:
  - Creates markers (markers come only from movement/capture per [`RR-CANON-R092`](../../RULES_CANONICAL_SPEC.md)).
...
- These constraints are enforced by:
  - TS: [`validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) and shared board invariants in [`core.ts`](../../src/shared/engine/core.ts).
  - Sandbox: `ClientSandboxEngine.tryPlaceRings` and [`sandboxPlacement.enumerateLegalRingPlacements()`](../../src/client/sandbox/sandboxPlacement.ts), both delegating to the shared validator.
  - Python: `_get_ring_placement_moves` and [`PlacementValidator.validate()`](../../ai-service/app/rules/validators/placement.py), which explicitly reject collapsed/marker cells.
...
- **Shared core (TS)** – [`hasAnyLegalMoveOrCaptureFromOnBoard()`](../../src/shared/engine/core.ts):
  - Inputs:
  ...
  - Behaviour:
    - Enumerates all legal non-capture moves from `from` for player `player` using [`movementLogic.ts`](../../src/shared/engine/movementLogic.ts).
    - If none, enumerates all legal overtaking captures from `from` using [`captureLogic.ts`](../../src/shared/engine/captureLogic.ts) / [`CaptureAggregate`](../../src/shared/engine/aggregates/CaptureAggregate.ts).
    - Returns `true` iff at least one such move or capture exists.
  - Placement usage:
    - [`PlacementAggregate.validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) constructs a **hypothetical board** with the placement applied and calls `hasAnyLegalMoveOrCaptureFromOnBoard` at `to`.
    - If it returns `false`, the placement is rejected as "dead".
- **Python analogue** – `_create_hypothetical_board_with_placement` and `_has_any_movement_or_capture_after_hypothetical_placement` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py):
...
- **TS behaviour:**
  - Backend:
    - [`RuleEngine.validateSkipPlacement()`](../../src/server/game/RuleEngine.ts) implements the canonical conditions above and is the authoritative TS behaviour for skip-placement in production (`getValidMoves` surface).
  - Shared aggregate:
    - [`PlacementAggregate.evaluateSkipPlacementEligibility()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) provides a reusable helper to compute:
...
- **Python behaviour:**
  - `_get_skip_placement_moves` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py):
...
- **LPS interaction:**
  - `skip_placement` is **not** itself a "real action" for LPS purposes under [`RR-CANON-R172`](../../RULES_CANONICAL_SPEC.md).
...
- **Forced elimination entry (RR-CANON-R072, R100):**
  - At the start of P's action for a turn (after any mandatory placement has been applied), compute:
    - `hasValidPlacements` – using canonical placement enumerators:
      - TS: [`PlacementAggregate.enumeratePlacementPositions()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) or equivalent host wrapper used by [`TurnLogicDelegates.hasAnyPlacement`](../../src/shared/engine/turnLogic.ts).
      - Python: `_get_ring_placement_moves` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py), wrapped by `_has_valid_placements`.
  ...
  - If P controls at least one stack and
    ...
    - `hasValidCaptures == false`,
      then P **must** perform a forced elimination (eliminate an entire cap from a controlled stack) per [`RR-CANON-R100`](../../RULES_CANONICAL_SPEC.md).
- **NDP interaction:**
...
- **LPS (`RR-CANON-R172`):**
  ...
  - Engine-level LPS helpers:
    - TS backend: `hasAnyRealActionForPlayer` in [`GameEngine.ts`](../../src/server/game/GameEngine.ts) and LPS helpers in [`turnLogic.ts`](../../src/shared/engine/turnLogic.ts).
    - Python: `_has_real_action_for_player` and LPS logic in `_check_victory` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py).
...
- Initial state:
  ...
  - `ringsInHand[P1] = ringsInHand[P2] = 18` (from [`RR-CANON-R020`](../../RULES_CANONICAL_SPEC.md)).
...
- **Placement availability:**
  ...
  - NDP behaviour:
    - For every candidate `to` and allowed `count`, NDP rejects the placement:
      - `_get_ring_placement_moves` in Python and [`PlacementAggregate.enumeratePlacementPositions()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) in TS both return **no** `place_ring` moves for P.
...
- **Forced elimination:**
  - Forced elimination condition from [`RR-CANON-R100`](../../RULES_CANONICAL_SPEC.md) is met:
...
The core types are defined in [`PlacementAggregate.ts`](../../src/shared/engine/aggregates/PlacementAggregate.ts) and related modules:

- [`PlacementContext`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
  - Describes global and per-player placement constraints:
    - `boardType`.
    - Per-player own-colour caps derived from [`RR-CANON-R020`](../../RULES_CANONICAL_SPEC.md).
    - `ringsInHand` for the acting player.
    - Optional cached counts of own-colour rings already on the board.
- [`PlacementValidationResult`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
...
- [`PlacementApplicationOutcome`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
  - Result of applying a placement:
    - `nextState: GameState`.
    - `ringsPlaced: number`.
    - Optional computed metadata (e.g. `newStackHeight` at the target).
- [`SkipPlacementEligibilityResult`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
  - Structured result for skip-placement checks:
    - `canSkip: boolean`.
    - `reason?: string` for diagnostic use.
- Action / move types:
  - [`PlaceRingAction`](../../src/shared/engine/aggregates/PlacementAggregate.ts) – canonical action for placements.
  - [`SkipPlacementAction`](../../src/shared/engine/aggregates/PlacementAggregate.ts) – canonical action for skip-placement.
  - [`Move`](../../src/shared/types/game.ts) – overarching move type used by the orchestrator.

##### 4.2 Board-level helpers

The board-level helpers operate on a `BoardState` plus a [`PlacementContext`](../../src/shared/engine/aggregates/PlacementAggregate.ts):

- [`validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
  ...
  - Behaviour:
    ...
    - Applies NDP by constructing a hypothetical board and calling [`hasAnyLegalMoveOrCaptureFromOnBoard()`](../../src/shared/engine/core.ts) at `to`.
  - Determinism:
    - Pure and deterministic for fixed `board`, `to`, `requestedCount`, and `ctx`.
- [`applyPlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
  - Signature (conceptual):
...
- [`validatePlacement()`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
  - Validates a [`PlaceRingAction`](../../src/shared/engine/aggregates/PlacementAggregate.ts) against a `GameState`:
    - Ensures `currentPhase === 'ring_placement'` and `currentPlayer` matches.
    - Constructs an appropriate [`PlacementContext`](../../src/shared/engine/aggregates/PlacementAggregate.ts) and delegates to [`validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts).
- [`enumeratePlacementPositions()`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
  - Signature (conceptual):
  ...
  - Behaviour:
    - Scans all cells of the board and:
      ...
      - For each allowed `count` (1–3 or 1 on stacks), calls [`validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) with the current [`PlacementContext`](../../src/shared/engine/aggregates/PlacementAggregate.ts).
      - Collects positions with at least one legal `count`.
    - Intended for:
      - Forced-elimination gating.
      - AI search and diagnostics.
- [`mutatePlacement()`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
...
- [`applyPlacementMove()`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
  - Accepts a canonical `Move` of type `'place_ring'`:
    - Validates it (optionally).
    - Delegates to [`mutatePlacement()`](../../src/shared/engine/aggregates/PlacementAggregate.ts).
    - Returns a [`PlacementApplicationOutcome`](../../src/shared/engine/aggregates/PlacementAggregate.ts).
- [`evaluateSkipPlacementEligibility()`](../../src/shared/engine/aggregates/PlacementAggregate.ts)
  - Computes whether a `skip_placement` is legal for a given `GameState` and player, according to §2.6.
- Host responsibilities:
  - Backend:
    - [`RuleEngine.getValidRingPlacements`](../../src/server/game/RuleEngine.ts) should be treated as an adapter over [`validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) / [`enumeratePlacementPositions()`](../../src/shared/engine/aggregates/PlacementAggregate.ts).
    - [`GameEngine.applyMove`](../../src/server/game/GameEngine.ts) should apply placement via [`applyPlacementMove()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) or [`mutatePlacement()`](../../src/shared/engine/aggregates/PlacementAggregate.ts).
  - Sandbox:
    - [`ClientSandboxEngine.enumerateLegalRingPlacements`](../../src/client/sandbox/ClientSandboxEngine.ts) and `tryPlaceRings` must delegate to the shared helpers (directly or via lightweight adapters).
...
Python should expose a thin rules-level façade, mirroring the TS contracts and delegating to the existing `GameEngine` helpers. This façade can live in a new module [`ai-service/app/rules/placement.py`](../../ai-service/app/rules/placement.py).

##### 5.1 Types

Proposed dataclasses (Pythonic analogues of §4.1):

- [`PlacementContextPy`](../../ai-service/app/rules/placement.py)
  - Encapsulates:
    - Board type and own-colour caps.
    - Acting player.
    - `rings_in_hand`.
    - Optional cached counts of rings on board.
- [`PlacementValidationResultPy`](../../ai-service/app/rules/placement.py)
  - Mirrors [`PlacementValidationResult`](../../src/shared/engine/aggregates/PlacementAggregate.ts) with Python enums/strings for error codes.
- [`PlacementApplicationOutcomePy`](../../ai-service/app/rules/placement.py)
  - Mirrors [`PlacementApplicationOutcome`](../../src/shared/engine/aggregates/PlacementAggregate.ts).
- [`SkipPlacementEligibilityResultPy`](../../ai-service/app/rules/placement.py)
  - Mirrors [`SkipPlacementEligibilityResult`](../../src/shared/engine/aggregates/PlacementAggregate.ts).

##### 5.2 Pure helper functions

- [`validate_placement_on_board_py()`](../../ai-service/app/rules/placement.py)
  - Signature (conceptual):
    - `def validate_placement_on_board_py(board, pos, requested_count, ctx: PlacementContextPy) -> PlacementValidationResultPy: ...`
  - Behaviour:
    - Performs structural and cap checks equivalent to [`validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts).
    - Uses `_create_hypothetical_board_with_placement` and `_has_any_movement_or_capture_after_hypothetical_placement` from [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py) to enforce NDP.
- [`enumerate_placement_moves_py()`](../../ai-service/app/rules/placement.py)
  - Signature (conceptual):
    - `def enumerate_placement_moves_py(state: GameState, player: int) -> list[Move]: ...`
  - Behaviour:
    - Wraps `_get_ring_placement_moves` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py) and returns a list of `Move` objects of type `PLACE_RING`.
    - Intended for:
      - AI search.
      - Forced-elimination gating.
      - Parity diagnostics.
- [`evaluate_skip_placement_eligibility_py()`](../../ai-service/app/rules/placement.py)
  - Signature (conceptual):
    - `def evaluate_skip_placement_eligibility_py(state: GameState, player: int) -> SkipPlacementEligibilityResultPy: ...`
  - Behaviour:
    - Wraps `_get_skip_placement_moves` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py):
...
- [`apply_place_ring_py()`](../../ai-service/app/rules/placement.py)
  - Signature (conceptual):
    - `def apply_place_ring_py(state: GameState, move: Move) -> GameState: ...`
  - Behaviour:
    - Thin wrapper around `_apply_place_ring` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py).
...
| Concern                                 | Current entry point                                                                            | Mapping to canonical API                                                                                                                                                                                                                                                            | Behaviour notes                                                                                                                 |
| --------------------------------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Single-placement validation             | [`RuleEngine.validateRingPlacement`](../../src/server/game/RuleEngine.ts)                      | Delegate structural, cap, and NDP checks to [`validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) with a [`PlacementContext`](../../src/shared/engine/aggregates/PlacementAggregate.ts) derived from the backend `GameState`.                    | Behaviour should match current production semantics; any divergence is a bug against the TS SSoT.                               |
| Placement enumeration in ring_placement | [`RuleEngine.getValidRingPlacements`](../../src/server/game/RuleEngine.ts)                     | Treat as an adapter over [`enumerate_placement_positions`](../../src/shared/engine/aggregates/PlacementAggregate.ts) / [`enumeratePlacementPositions()`](../../src/shared/engine/aggregates/PlacementAggregate.ts), surfacing `place_ring` moves for each legal `(to, count)` pair. | Expected behaviour: identical to current `getValidRingPlacements`, but the shared aggregate is treated explicitly as SSoT.      |
| Skip-placement validation               | [`RuleEngine.validateSkipPlacement`](../../src/server/game/RuleEngine.ts)                      | Delegate to [`evaluateSkipPlacementEligibility()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) and enforce canonical conditions from §2.6.                                                                                                                            | Confirms and documents that skip-placement is only legal when `ringsInHand > 0`, `controlsAnyStack`, and `hasAnyMoveOrCapture`. |
| Applying a placement move               | `'place_ring'` branch in [`GameEngine.applyMove`](../../src/server/game/GameEngine.ts)         | Delegate to [`applyPlacementMove()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) (or [`mutatePlacement()`](../../src/shared/engine/aggregates/PlacementAggregate.ts)) to update board and `ringsInHand`.                                                              | No intended semantic change; refactor removes backend-local duplication of stack/hand updates.                                  |
| Forced-elimination gating               | `TurnLogicDelegates.hasAnyPlacement` in [`turnLogic.ts`](../../src/shared/engine/turnLogic.ts) | Implement via [`enumeratePlacementPositions()`](../../src/shared/engine/aggregates/PlacementAggregate.ts), consistent with §2.7.                                                                                                                                                    | Ensures forced elimination is entered only when **no** legal placement exists, after NDP is applied.                            |

##### Sandbox (TS)

| Concern                          | Current entry point                                                                                               | Mapping to canonical API                                                                                                                                                                                                                                                                                                           | Behaviour notes                                                                                         |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Per-player placement enumeration | [`ClientSandboxEngine.enumerateLegalRingPlacements`](../../src/client/sandbox/ClientSandboxEngine.ts)             | Adapter over [`enumeratePlacementPositions()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) and/or [`validatePlacementOnBoard()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) with a [`PlacementContext`](../../src/shared/engine/aggregates/PlacementAggregate.ts) built from the sandbox `GameState`. | Already mostly wired; this spec freezes that path as canonical for sandbox UI and AI.                   |
| Manual placement in sandbox UI   | `tryPlaceRings` and click-handling in [`ClientSandboxEngine.ts`](../../src/client/sandbox/ClientSandboxEngine.ts) | Use [`validatePlacement()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) / [`applyPlacementMove()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) when not delegating to the orchestrator.                                                                                                                | Ensures sandbox humans and AI see exactly the same placement legality as backend and Python AI.         |
| Legacy placement helpers         | [`sandboxPlacement.ts`](../../src/client/sandbox/sandboxPlacement.ts)                                             | Retain only as analysis/debug helpers when they **do not** use a [`PlacementContext`](../../src/shared/engine/aggregates/PlacementAggregate.ts); mark clearly as non-SSOT.                                                                                                                                                         | Any paths in sandbox that still compute placements without NDP or caps must be considered tooling-only. |
| Skip-placement in sandbox AI     | `maybeRunAITurnSandbox` in [`sandboxAI.ts`](../../src/client/sandbox/sandboxAI.ts)                                | Compute skip eligibility via [`evaluateSkipPlacementEligibility()`](../../src/shared/engine/aggregates/PlacementAggregate.ts) or by mirroring the canonical conditions in §2.6.                                                                                                                                                    | Aligns sandbox AI skip decisions with backend and Python rules.                                         |

##### Python (ai-service)

| Concern                          | Current entry point                                                                                      | Mapping to canonical API                                                                                                                                                                                | Behaviour notes                                                                                      |
| -------------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| Placement move enumeration       | `_get_ring_placement_moves` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py) | Treat as the canonical implementation underlying [`enumerate_placement_moves_py()`](../../ai-service/app/rules/placement.py).                                                                           | No change in behaviour; this spec formalises its role as the Python placement SSoT.                  |
| Skip-placement enumeration       | `_get_skip_placement_moves` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py) | Treat as the canonical implementation underlying [`evaluate_skip_placement_eligibility_py()`](../../ai-service/app/rules/placement.py).                                                                 | Confirms alignment with RR-CANON-R080; no changes needed.                                            |
| Placement validation entry point | [`PlacementValidator.validate`](../../ai-service/app/rules/validators/placement.py)                      | Delegate structural, cap, and NDP checks to [`validate_placement_on_board_py()`](../../ai-service/app/rules/placement.py); keep phase/turn/must-move checks locally.                                    | Tightens validator behaviour to match TS; per-player caps become explicitly enforced here.           |
| Placement mutator entry point    | [`PlacementMutator.apply`](../../ai-service/app/rules/mutators/placement.py)                             | Thin wrapper over [`apply_place_ring_py()`](../../ai-service/app/rules/placement.py) and, indirectly, `_apply_place_ring` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py). | Behaviour remains the same; the façade just clarifies the semantic boundary.                         |
| Forced-elimination gating        | `_has_valid_placements` in [`game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py)     | Implement via [`enumerate_placement_moves_py()`](../../ai-service/app/rules/placement.py) (or `_get_ring_placement_moves`) so that "no legal placements" is defined consistently with TS.               | Ensures forced elimination in Python uses the same notion of "legal placement" as TS, including NDP. |
...
2. **Python validator cap enforcement tightening.**
   - Today, the Python [`PlacementValidator`](../../ai-service/app/rules/validators/placement.py) relies on `_get_ring_placement_moves` for some legality but does not centrally enforce own-colour caps.
   - Moving to [`validate_placement_on_board_py()`](../../ai-service/app/rules/placement.py) with explicit cap checks will:
     - Reject over-cap placements that may currently be rejected only indirectly.
     - Align Python behaviour exactly with TS [`PlacementAggregate`](../../src/shared/engine/aggregates/PlacementAggregate.ts).

3. **Legacy sandbox placement helpers treated as non-SSOT.**
   - `sandboxPlacement` and any helpers that construct hypothetical boards without a [`PlacementContext`](../../src/shared/engine/aggregates/PlacementAggregate.ts) are:
...
5. **Movement aggregate changes may refine NDP but not placement semantics.**
   - Future P2.2 changes to [`MovementAggregate.ts`](../../src/shared/engine/aggregates/MovementAggregate.ts) may refine movement enumeration (e.g., path-blocking, markers along paths).
...
**Input Dependencies:** P1–P3, shared orchestrator design in [`RULES_ENGINE_CONSOLIDATION_DESIGN.md`](../archive/RULES_ENGINE_CONSOLIDATION_DESIGN.md), existing [`turnLogic.ts`](../../src/shared/engine/turnLogic.ts) and the canonical FSM stack ([`fsm/TurnStateMachine.ts`](../../src/shared/engine/fsm/TurnStateMachine.ts), [`fsm/FSMAdapter.ts`](../../src/shared/engine/fsm/FSMAdapter.ts)). (Legacy `orchestration/phaseStateMachine.ts` removed in PASS30-R1.)
