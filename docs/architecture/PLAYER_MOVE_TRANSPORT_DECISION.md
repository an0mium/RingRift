...
- **WebSocket gameplay protocol (canonical today):**
  - Socket.IO v4 server under [`WebSocketServer`](../../src/server/websocket/server.ts).
  - Per-game interaction pipeline in [`WebSocketInteractionHandler.ts`](../../src/server/game/WebSocketInteractionHandler.ts).
  - Session and state orchestration in [`GameSessionManager.ts`](../../src/server/game/GameSessionManager.ts) and [`TurnEngine.ts`](../../src/server/game/turn/TurnEngine.ts), backed by the canonical turn orchestrator in [`turnOrchestrator.ts`](../../src/shared/engine/orchestration/turnOrchestrator.ts).
  - Real-time client protocol documented in [`CANONICAL_ENGINE_API.md`](CANONICAL_ENGINE_API.md) and surfaced in the WebSocket section of [`API_REFERENCE.md`](API_REFERENCE.md).
- **HTTP APIs (supporting surfaces):**
  - Auth, game creation, game state readbacks, and diagnostics provided via [`src/server/routes`](../../src/server/routes/index.ts), including [`game.ts`](../../src/server/routes/game.ts).
  - REST API contract documented in [`API_REFERENCE.md`](API_REFERENCE.md).
- **Domain move semantics:**
  - All rules semantics and turn sequencing live in the shared engine and orchestrator stack under [`src/shared/engine`](../../src/shared/engine/types.ts) and the backend host described in [`RULES_ENGINE_ARCHITECTURE.md`](RULES_ENGINE_ARCHITECTURE.md).
  - Server-side gameplay hosts (including [`GameSessionManager.ts`](../../src/server/game/GameSessionManager.ts)) ultimately delegate move application to the orchestrator-backed domain API (conceptually `GameSessionManager.applyMove()`), with WebSocket as the public transport.

### 1.2 Existing k6 load tests

- k6 scenarios live under [`tests/load/scenarios`](../../tests/load/scenarios/game-creation.js):
  - [`game-creation.js`](../../tests/load/scenarios/game-creation.js) – create-game and GET `/api/games/:gameId` performance.
  - [`concurrent-games.js`](../../tests/load/scenarios/concurrent-games.js) – many simultaneous active games and state polling.
  - [`player-moves.js`](../../tests/load/scenarios/player-moves.js) – intended to exercise move submission and turn processing.
  - `websocket-stress.js` – WebSocket connection and ping-pong stress (transport-level only).
- WebSocket gameplay SLOs and stall definitions are specified in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md).
...
- **All production gameplay moves are carried over WebSockets** via `player_move` / `player_move_by_id` (see the WebSocket API section of [`API_REFERENCE.md`](API_REFERENCE.md)).
- A thin HTTP move harness is implemented in [`game.ts`](../../src/server/routes/game.ts) as `POST /api/games/:gameId/moves`, but it is gated by:
  - `ENABLE_HTTP_MOVE_HARNESS` (feature flag)
  - `HTTP_MOVE_HARNESS_TIMEOUT_MS` (timeout guard)
- The HTTP-based move submission path in [`player-moves.js`](../../tests/load/scenarios/player-moves.js) is still intentionally guarded:
  - `const MOVE_HTTP_ENDPOINT_ENABLED = false;` in [`player-moves.js`](../../tests/load/scenarios/player-moves.js) keeps the harness inactive unless explicitly enabled.
- As a result:
...
- **Non-goals**
  ...
  - Change the canonical **rules/turn semantics SSoT**, which remains the shared engine + orchestrator stack described in [`RULES_ENGINE_ARCHITECTURE.md`](RULES_ENGINE_ARCHITECTURE.md) and [`CANONICAL_ENGINE_API.md`](CANONICAL_ENGINE_API.md).
...
- Matches existing implementation and the WebSocket gameplay SLOs in [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md).
...
1. **Canonical transport**
   ...
   - Client applications (web, mobile, desktop) **must** submit moves over the WebSocket protocol (`player_move` / `player_move_by_id` events) as documented in [`CANONICAL_ENGINE_API.md`](CANONICAL_ENGINE_API.md) and the WebSocket section of [`API_REFERENCE.md`](API_REFERENCE.md).
...
2. **Single domain API for move semantics**
   - All move semantics are implemented and validated in a **single, shared domain API**, conceptually:
     - [`GameSessionManager.applyMove()`](../../src/server/game/GameSessionManager.ts), which:
       - Accepts a validated, canonical `Move`/`PlayerChoice` payload.
       - Delegates to the orchestrator-backed rules engine stack described in [`RULES_ENGINE_ARCHITECTURE.md`](RULES_ENGINE_ARCHITECTURE.md).
   ...
   - Both of the following call sites must use this same domain API:
     - WebSocket move handlers in [`WebSocketInteractionHandler.ts`](../../src/server/game/WebSocketInteractionHandler.ts).
     - Any HTTP move harness endpoint implemented in [`game.ts`](../../src/server/routes/game.ts).

3. **HTTP move harness semantics**
   - An HTTP endpoint of the form:
     - `POST /api/games/:gameId/moves`
   - **may be implemented** as a **thin adapter** over [`GameSessionManager.applyMove()`](../../src/server/game/GameSessionManager.ts) with the following constraints:
     - The harness performs:
       - Authentication using the same JWT/session model as other game routes.
       - Authorization consistent with game-seat/spectator rules from [`CANONICAL_ENGINE_API.md`](CANONICAL_ENGINE_API.md).
   ...
   - The harness is **not** a general public API:
     ...
     - It should be clearly documented as such in [`API_REFERENCE.md`](API_REFERENCE.md) under an "Internal / Test harness APIs" section.
...
4. **Environment and feature-flag controls**
   ...
   - Any change to harness exposure in production must be treated like an API surface change and reviewed under the security model in [`SECURITY_THREAT_MODEL.md`](../security/SECURITY_THREAT_MODEL.md).

5. **k6 player-moves scenario behaviour**
   - [`player-moves.js`](../../tests/load/scenarios/player-moves.js) remains the canonical HTTP-oriented load test for move and turn performance.
   - Its behaviour is defined as:
     - When the harness is **enabled**:
       ...
       - Record:
         ...
         - `stalled_moves_total` for moves exceeding the stall threshold from [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md).
...
- **Clear production SLO mapping**
  - WebSocket gameplay SLOs in [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md) now map unambiguously to the actual production move path.
...
- **Complexity in environment configuration**
  - Additional feature flags and environment-specific configuration must be maintained and documented in [`ENVIRONMENT_VARIABLES.md`](../operations/ENVIRONMENT_VARIABLES.md) and deployment runbooks.
...
- **Load tests**
  - HTTP-based:
    - [`player-moves.js`](../../tests/load/scenarios/player-moves.js) drives `POST /api/games/:gameId/moves` when the harness is enabled.
    - [`game-creation.js`](../../tests/load/scenarios/game-creation.js) and [`concurrent-games.js`](../../tests/load/scenarios/concurrent-games.js) continue to focus on game creation and state polling.
  - WebSocket-based:
...
- Security reviews for the harness should:
  - Reuse the threat categories from [`SECURITY_THREAT_MODEL.md`](../security/SECURITY_THREAT_MODEL.md).
...
1. **HTTP move harness endpoint (implemented)**
   - `POST /api/games/:gameId/moves` exists in [`game.ts`](../../src/server/routes/game.ts) as a thin adapter over the canonical move pipeline (`WebSocketServer.handlePlayerMoveFromHttp` → `GameSession.handlePlayerMoveFromHttp`).
...
2. **Update k6 `player-moves` scenario**
   - Replace the hard-coded `MOVE_HTTP_ENDPOINT_ENABLED = false` in [`player-moves.js`](../../tests/load/scenarios/player-moves.js) with an environment-driven toggle (for example, `__ENV.MOVE_HTTP_ENDPOINT_ENABLED`).
   ...
   - Add a `moves_attempted_total` counter and tighten thresholds so the scenario fails if:
     ...
     - Move success rate or stall metrics violate the SLOs in [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md).
...
3. **Design WebSocket-based move load tests**
   - Extend existing WebSocket stress tooling or introduce a Playwright-based harness that:
     ...
     - Measures end-to-end move latency and stall rates consistent with [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md).
   - Ensure these tests run (at least) in staging as part of the P-01 performance gate defined in [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md).

4. **Document harness configuration and runbooks**
   - Keep [`ENVIRONMENT_VARIABLES.md`](../operations/ENVIRONMENT_VARIABLES.md) and [`API_REFERENCE.md`](API_REFERENCE.md) aligned with the harness flags (`ENABLE_HTTP_MOVE_HARNESS`, `HTTP_MOVE_HARNESS_TIMEOUT_MS`).
