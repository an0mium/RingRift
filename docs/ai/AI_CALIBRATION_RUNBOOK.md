...
&gt; **Role:** Step‑by‑step operational guide for running AI difficulty calibration cycles for the Square‑8 2‑player ladder tiers D2 / D4 / D6 / D8, using the analysis design in [`AI_DIFFICULTY_CALIBRATION_ANALYSIS.md`](AI_DIFFICULTY_CALIBRATION_ANALYSIS.md) and the CLI in [`analyze_difficulty_calibration.py`](../../ai-service/scripts/analyze_difficulty_calibration.py).

---

## 1. Purpose and scope

- This runbook is part of the remediation track for the **hardest problem** identified in [`WEAKNESS_AND_HARDEST_PROBLEM_REPORT.md`](../archive/assessments/WEAKNESS_AND_HARDEST_PROBLEM_REPORT.md): _advanced AI strength and a stable high‑tier Square‑8 2‑player ladder_.
- It defines the **operational procedure** for running an AI difficulty calibration cycle:
  ...
  - invoking [`analyze_difficulty_calibration.py`](../../ai-service/scripts/analyze_difficulty_calibration.py);
...
- It **does not** redefine calibration theory, metrics, or thresholds. Those live in:
  - [`AI_DIFFICULTY_CALIBRATION_ANALYSIS.md`](AI_DIFFICULTY_CALIBRATION_ANALYSIS.md) – analysis design and decision rules;
  - [`AI_HUMAN_CALIBRATION_GUIDE.md`](AI_HUMAN_CALIBRATION_GUIDE.md) – human‑study templates;
  - the script implementation [`analyze_difficulty_calibration.py`](../../ai-service/scripts/analyze_difficulty_calibration.py).
...
- Python is installed with a version compatible with `ai-service` (see [`requirements.txt`](../../ai-service/requirements.txt)).
- Dependencies are installed as described in [`ai-service/README.md`](../../ai-service/README.md) (for example via `pip install -r ai-service/requirements.txt` or the project’s bootstrap scripts).
...
- Read the **tier candidate registry**:
  - [`tier_candidate_registry.square8_2p.json`](../../ai-service/config/tier_candidate_registry.square8_2p.json)
...
The file must match the schema validated by [`load_calibration_aggregates()`](../../ai-service/scripts/analyze_difficulty_calibration.py) in [`analyze_difficulty_calibration.py`](../../ai-service/scripts/analyze_difficulty_calibration.py). Conceptually:
...
- Fields are per **segment** (e.g. `new`, `intermediate`, `strong`) as described in [`AI_DIFFICULTY_CALIBRATION_ANALYSIS.md`](AI_DIFFICULTY_CALIBRATION_ANALYSIS.md).
...
- Default path: [`tier_candidate_registry.square8_2p.json`](../../ai-service/config/tier_candidate_registry.square8_2p.json)
- Canonical loader: [`load_square8_two_player_registry()`](../../ai-service/app/training/tier_promotion_registry.py)
...
1. **Choose the time window.**
   ...
   - Ensure:
     - Sufficient sample sizes per tier/segment (see thresholds in [`AI_DIFFICULTY_CALIBRATION_ANALYSIS.md`](AI_DIFFICULTY_CALIBRATION_ANALYSIS.md)).
     - No major mid‑window ladder changes if possible.

2. **Confirm tiers in scope.**
   - Board: `square8` (8×8 compact ruleset).
   - Players: `2`.
   - Difficulty tiers: `D2`, `D4`, `D6`, `D8`, matching the ladder scope in [`AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md`](AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md).
...
4. **Seed the run with the notes template.**
   - Copy the per‑run template [`TEMPLATE.md`](calibration_runs/TEMPLATE.md) into the new directory as `notes.md`:
...
3.  **Choose `--eval-root` and verify tier directories.**
    ...
    - These artefacts are typically produced by:
      - [`run_tier_gate.py`](../../ai-service/scripts/run_tier_gate.py) and/or
      - the combined wrapper [`run_full_tier_gating.py`](../../ai-service/scripts/run_full_tier_gating.py),

      as described in [`AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md`](AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md).

4.  **Record inputs in `notes.md`.**
    - In the run’s `notes.md` (from [`TEMPLATE.md`](calibration_runs/TEMPLATE.md)), fill the **Inputs** section with:
...
From the **project root**, run a command along the lines of:
...
Key flags (see [`parse_args()`](../../ai-service/scripts/analyze_difficulty_calibration.py)):

- `--calibration-aggregates` – path to the aggregates JSON exported in §3.2.1.
- `--registry-path` – path to the Square‑8 2p registry JSON (defaults to [`DEFAULT_SQUARE8_2P_REGISTRY_PATH`](../../ai-service/app/training/tier_promotion_registry.py)).
...
Shape is defined by [`build_calibration_summary()`](../../ai-service/scripts/analyze_difficulty_calibration.py). At a high level it contains:

- `board`, `num_players`
- `window` – including `start`, `end`, and `label`
- `tiers[]` – one entry per tier, each with:
  - `tier`, `difficulty`
  - `ladder` – current ladder model id, AI type, heuristic profile (from [`get_ladder_tier_config`](../../ai-service/app/config/ladder_config.py)).
...
The Markdown is produced by [`build_markdown_report()`](../../ai-service/scripts/analyze_difficulty_calibration.py) and is the primary **human‑facing report** for this run. For each tier it includes:
...
How to interpret `too_easy` / `too_hard` / `in_band` is defined precisely in [`AI_DIFFICULTY_CALIBRATION_ANALYSIS.md`](AI_DIFFICULTY_CALIBRATION_ANALYSIS.md). Use that document as the **normative reference** when turning this report into actions.
...
- Ladder and training context from:
  ...
  - [`AI_TIER_PERF_BUDGETS.md`](AI_TIER_PERF_BUDGETS.md)
...
- **Tier appears too easy** for its intended segment and passes gates/perf:
  ...
  - Optionally tighten future gate expectations as described in [`AI_DIFFICULTY_CALIBRATION_ANALYSIS.md`](AI_DIFFICULTY_CALIBRATION_ANALYSIS.md).
...
- **Data inconclusive** (low `n_games` or mixed signals):
  - Schedule additional calibration telemetry collection or structured human sessions using templates A/B/C from [`AI_HUMAN_CALIBRATION_GUIDE.md`](AI_HUMAN_CALIBRATION_GUIDE.md).
- **Automated gates or perf fail**:
  - Treat this primarily as an H‑AI‑9/H‑AI‑8 issue; calibration may still be informative but should not drive promotions that violate the invariants documented in [`AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md`](AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md) and [`AI_TIER_PERF_BUDGETS.md`](AI_TIER_PERF_BUDGETS.md).
...
The per‑run template [`TEMPLATE.md`](calibration_runs/TEMPLATE.md):
...
- **Data / Telemetry Owner**
  ...
  - Confirms that schema changes in telemetry or exports do not break [`analyze_difficulty_calibration.py`](../../ai-service/scripts/analyze_difficulty_calibration.py).
...
1. **Plan**
   ...
   - [ ] Copy [`TEMPLATE.md`](calibration_runs/TEMPLATE.md) → `notes.md`.

2. **Gather inputs**
   - [ ] Export calibration aggregates JSON into the run directory.
   - [ ] (Optional) Snapshot [`tier_candidate_registry.square8_2p.json`](../../ai-service/config/tier_candidate_registry.square8_2p.json) into the run directory.
...
3. **Run analysis**
   - [ ] Run [`analyze_difficulty_calibration.py`](../../ai-service/scripts/analyze_difficulty_calibration.py) with `--calibration-aggregates`, `--registry-path`, `--eval-root`, `--output-json`, `--output-md`, and a `--window-label`.
