...
- Human calibration guide: [`AI_HUMAN_CALIBRATION_GUIDE.md`](AI_HUMAN_CALIBRATION_GUIDE.md)
- Training and promotion pipeline: [`AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md`](AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md)
- Tier performance budgets: [`AI_TIER_PERF_BUDGETS.md`](AI_TIER_PERF_BUDGETS.md)
- Tier training orchestration: [`run_tier_training_pipeline.py`](../../ai-service/scripts/run_tier_training_pipeline.py)
- Full gating wrapper: [`run_full_tier_gating.py`](../../ai-service/scripts/run_full_tier_gating.py)
- Promotion-plan applier and registry helpers: [`python.tier_promotion_registry`](../../ai-service/app/training/tier_promotion_registry.py), [`apply_tier_promotion_plan.py`](../../ai-service/scripts/apply_tier_promotion_plan.py)

It is **analysis/process-level only**. No ladder configs, models, or UI are changed directly here.

## For step-by-step operational instructions on running a calibration cycle, see [`AI_CALIBRATION_RUNBOOK.md`](AI_CALIBRATION_RUNBOOK.md).
...
- AI families and current intended strength ordering are as in [`AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md`](AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md)
...
3. Produce an **explicit calibration report artefact** that downstream tools (e.g. a future [`python.analyze_difficulty_calibration.py`](../../ai-service/scripts/analyze_difficulty_calibration.py) script) can generate and CI or reviewers can read.
...
- Shared event payload: [`difficultyCalibrationEvents.ts`](../../src/shared/telemetry/difficultyCalibrationEvents.ts)
...
- Metrics sink: [`typescript.MetricsService.recordDifficultyCalibrationEvent`](../../src/server/services/MetricsService.ts)
...
Exact label set is defined in [`AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md`](AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md) and the metrics module.
...
Core building blocks (see [`AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md`](AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md)):

- Tier evaluation configs: [`python.TIER_EVAL_CONFIGS`](../../ai-service/app/training/tier_eval_config.py)
- Evaluation runner returning `TierEvaluationResult`: [`python.run_tier_evaluation`](../../ai-service/app/training/tier_eval_runner.py)
- Tier gating CLI: [`run_tier_gate.py`](../../ai-service/scripts/run_tier_gate.py)
- Combined gating + perf wrapper: [`run_full_tier_gating.py`](../../ai-service/scripts/run_full_tier_gating.py)
- Perf budgets and harness:
  - [`python.TIER_PERF_BUDGETS`](../../ai-service/app/config/perf_budgets.py)
  - [`python.run_tier_perf_benchmark`](../../ai-service/app/training/tier_perf_benchmark.py)

**Run directory layout**

A standard Square-8 2p tier training + gating cycle uses:

1. Train candidate (stub or real) via [`python.main()`](../../ai-service/scripts/run_tier_training_pipeline.py):
   - Run dir example:
...
2. Gate candidate via [`python.main()`](../../ai-service/scripts/run_full_tier_gating.py) pointing at that run dir:
...
Registry helpers: [`python.tier_promotion_registry`](../../ai-service/app/training/tier_promotion_registry.py)

- Default Square-8 2p registry path:
  - [`python.DEFAULT_SQUARE8_2P_REGISTRY_PATH`](../../ai-service/app/training/tier_promotion_registry.py)  
    → `ai-service/config/tier_candidate_registry.square8_2p.json`

- Loader and saver:
  - [`python.load_square8_two_player_registry()`](../../ai-service/app/training/tier_promotion_registry.py)
  - [`python.save_square8_two_player_registry()`](../../ai-service/app/training/tier_promotion_registry.py)

- Registry update from a promotion plan:
  - [`python.update_square8_two_player_registry_for_run()`](../../ai-service/app/training/tier_promotion_registry.py)
  - [`python.record_promotion_plan()`](../../ai-service/app/training/tier_promotion_registry.py)
...
- Structured playtests following templates A/B/C in [`AI_HUMAN_CALIBRATION_GUIDE.md`](AI_HUMAN_CALIBRATION_GUIDE.md)
...
Following [`AI_HUMAN_CALIBRATION_GUIDE.md`](AI_HUMAN_CALIBRATION_GUIDE.md) and its templates, we distinguish:
...
Restating the informal targets from [`AI_HUMAN_CALIBRATION_GUIDE.md`](AI_HUMAN_CALIBRATION_GUIDE.md):
...
Conceptually, a future [`python.analyze_difficulty_calibration.py`](../../ai-service/scripts/analyze_difficulty_calibration.py) script will:
...
1. Resolve the current ladder assignment via the registry:
   - Load registry with [`python.load_square8_two_player_registry()`](../../ai-service/app/training/tier_promotion_registry.py)
...
1. **Model selection within tier T**
   - Promote or roll back a candidate model at fixed difficulty T using:
     - The full H-AI-9 pipeline and promotion helpers:
       - [`run_tier_training_pipeline.py`](../../ai-service/scripts/run_tier_training_pipeline.py)
       - [`run_full_tier_gating.py`](../../ai-service/scripts/run_full_tier_gating.py)
       - [`apply_tier_promotion_plan.py`](../../ai-service/scripts/apply_tier_promotion_plan.py)
...
2. **Maintain tier ordering and non-regression**
   ...
   - No new Dn should be substantially weaker than the previous production Dn (see [`AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md`](AI_TIER_TRAINING_AND_PROMOTION_PIPELINE.md))

3. **Respect perf budgets as UX guardrails**
   - D4 / D6 / D8 must respect the budgets in [`AI_TIER_PERF_BUDGETS.md`](AI_TIER_PERF_BUDGETS.md)
...
- Human-readable reports:
  - `docs/ai/calibration_runs/CALIB_YYYYMMDD_square8_2p.md`
- Registry snapshots:
  - `ai-service/config/tier_candidate_registry.square8_2p.json` (managed via [`python.save_square8_two_player_registry()`](../../ai-service/app/training/tier_promotion_registry.py))
...
- [`python.main()`](../../ai-service/scripts/analyze_difficulty_calibration.py) in `ai-service/scripts/analyze_difficulty_calibration.py`
