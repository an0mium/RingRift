...
> - [`LOAD_TEST_BASELINE_REPORT.md`](LOAD_TEST_BASELINE_REPORT.md)
> - [`API_REFERENCE.md`](../architecture/API_REFERENCE.md)
> - [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js)
> - [`player-moves.js`](../../tests/load/scenarios/player-moves.js)
...
This document defines an implementation-ready strategy for validating the **WebSocket gameplay SLOs** (move latency, decision latency, stalls, and reliability) on the **canonical WebSocket path**, as defined in the WebSocket gameplay SLOs in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md).
...
- Produce **load-test metrics directly mapped** to the WebSocket gameplay SLOs and stall definitions in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md).
...
The canonical move transport decision is documented in [`PLAYER_MOVE_TRANSPORT_DECISION.md`](../architecture/PLAYER_MOVE_TRANSPORT_DECISION.md):

- **WebSocket is the canonical move transport** for interactive clients.
- Any HTTP move endpoint (for example, `POST /api/games/:gameId/moves`) is a **thin internal/test harness** only, exposed under “Internal / Test harness APIs” in [`API_REFERENCE.md`](../architecture/API_REFERENCE.md).

Existing k6 scenarios under `tests/load/scenarios` focus primarily on HTTP flows and low-level WebSocket transport:

- [`game-creation.js`](../../tests/load/scenarios/game-creation.js) and [`concurrent-games.js`](../../tests/load/scenarios/concurrent-games.js) validate **HTTP game lifecycle and polling** (auth, game creation, state reads).
- [`player-moves.js`](../../tests/load/scenarios/player-moves.js) drives the **HTTP move harness** (when enabled) and defines move-centric metrics tied to the HTTP path.
- [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js) measures pure WebSocket transport characteristics using `diagnostic:ping` / `diagnostic:pong`, but **does not exercise gameplay semantics** (no real moves or game state).
...
- Emit metrics that map **directly** to the gameplay SLOs in [`STRATEGIC_ROADMAP.md` §2.2–2.3](../planning/STRATEGIC_ROADMAP.md), including:
...
[`k6`](../../tests/load/scenarios/game-creation.js) already underpins the current load-testing framework and is used together with **manual Engine.IO/Socket.IO framing** in [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js).
...
- **Excellent for synthetic load and concurrency (P-01 scenarios)**
  ...
  - Well suited to the P-01 target scales in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md).
- **Socket.IO framing code already exists**
  - [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js) contains working Engine.IO/Socket.IO handshake and framing logic that can be adapted to gameplay events (`join_game`, `player_move_by_id`, etc.).
...
A second option is a **TypeScript harness** under a directory such as `tests/load/harnesses/`, built using [`socket.io-client`](../../package.json) and the shared WebSocket/game types:

- [`websocket.ts`](../../src/shared/types/websocket.ts)
- [`game.ts`](../../src/shared/types/game.ts)
- [`websocketSchemas.ts`](../../src/shared/validation/websocketSchemas.ts)
...
- **New plumbing and commands for CI**
  ...
  - Needs explicit integration into the P-01 perf gate described in [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md).
...
These tests would use the **real React SPA** and the [`GameContext`](../../src/client/contexts/GameContext.tsx) WebSocket client.
...
- **Natural SLO spot checks**
  - Easy to encode assertions like “p95 click-to-board-update latency must be below the staging SLOs in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md)”.
...
- **Primary:**
  - Implement a k6 WebSocket gameplay scenario [`websocket-gameplay.js`](../../tests/load/scenarios/websocket-gameplay.js) alongside [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js), focused on move throughput and stall rates.
- **Supplementary:**
  - Add a small Playwright E2E spec [`websocket-move-latency.e2e.spec.ts`](../../tests/e2e/websocket-move-latency.e2e.spec.ts) that measures user-perceived move round-trip latency via the real browser client path.
...
- k6 WebSocket gameplay scenario [`websocket-gameplay.js`](../../tests/load/scenarios/websocket-gameplay.js) in a **“smoke” configuration**.
...
- `ws_move_rtt_ms` (Trend):
  - p95 and p99 should be **comfortably below** the staging-level WebSocket SLOs in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md) when run against staging/perf.
...
- WebSocket connection and handshake success:
  - Reuse/extend metrics from [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js); success rate close to 100% on healthy environments.
...
- [`websocket-gameplay.js`](../../tests/load/scenarios/websocket-gameplay.js) in a **“throughput” configuration** (distinct from S1’s smoke config).
...
- `ws_move_rtt_ms` (Trend):
  - p95 and p99 must meet or beat the WebSocket gameplay SLOs in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md).
- `ws_moves_attempted_total` (Counter):
...
- `ws_move_stalled_total` (Counter) and stall rate:
  - Stall definition: RTT > 2000 ms, matching [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md).
  - Stall rate targets:
    - ≤ **0.5%** of moves in staging.
    - ≤ **0.2%** in a dedicated perf environment.
- WebSocket connection/handshake metrics (from [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js)):
...
- Connect WebSocket move and AI-turn metrics to the AI SLOs in [`STRATEGIC_ROADMAP.md` §2.3](../planning/STRATEGIC_ROADMAP.md).

**Tool / harness**

- Variant of [`websocket-gameplay.js`](../../tests/load/scenarios/websocket-gameplay.js) that creates and drives **human-vs-AI** games.
  - AI seating and difficulty configuration should mirror the HTTP harness behaviour in [`player-moves.js`](../../tests/load/scenarios/player-moves.js).
...
- AI turn latency: `ws_ai_turn_latency_ms` (Trend)
  ...
  - Targets aligned with AI SLOs in [`STRATEGIC_ROADMAP.md` §2.3](../planning/STRATEGIC_ROADMAP.md) for end-to-end AI turn latency.
- Error and fallback rates:
  ...
  - Should remain within the AI fallback SLOs in [`STRATEGIC_ROADMAP.md` §2.3](../planning/STRATEGIC_ROADMAP.md).
...
- `ws_decision_stalled_total` (Counter):
  - Count of decisions whose RTT exceeds a stall threshold (for example, 2000 ms, mirroring the move stall definition in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md)).
...
This section defines the core metrics expected from the k6 WebSocket gameplay scenario, the Node/socket.io-client harness, and the Playwright E2E spec, and explains how they map to the SLOs in [`STRATEGIC_ROADMAP.md` §2.2–2.3](../planning/STRATEGIC_ROADMAP.md).
...
- `ws_move_rtt_ms` (Trend)
  ...
  - **SLO mapping:**
    - Directly corresponds to “human move submission → authoritative broadcast” latency in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md).
...
- `ws_move_success_rate` (Rate)
  ...
  - **SLO mapping:**
    - Ties into the WebSocket availability/error-budget targets in [`STRATEGIC_ROADMAP.md` §2.4](../planning/STRATEGIC_ROADMAP.md).

- `ws_move_stalled_total` (Counter)
  - **Definition:**
    - Count of moves where `ws_move_rtt_ms` exceeds **2000 ms**, matching the stall definition for human moves in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md).
  - **Derived metrics:**
...
The WebSocket gameplay protocol defines structured error codes in [`websocket.ts`](../../src/shared/types/websocket.ts). Load tests should expose per-code counters, for example:
...
The new WebSocket gameplay scenarios should **reuse and extend** the connection metrics already emitted by [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js), including:
...
- `ws_decision_rtt_ms` (Trend)
  ...
  - Aligned with decision metadata and `GameStateUpdateMeta` semantics in [`websocketSchemas.ts`](../../src/shared/validation/websocketSchemas.ts).
...
The Playwright spec [`websocket-move-latency.e2e.spec.ts`](../../tests/e2e/websocket-move-latency.e2e.spec.ts) should measure **user-perceived** latency:
...
- Click-to-board-update timings:
  ...
  - Aggregated as a Trend (for example, `browser_ws_move_rtt_ms`) with p95/p99 compared against staging-level SLOs in [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md).
...
**File:** [`websocket-gameplay.js`](../../tests/load/scenarios/websocket-gameplay.js)

**High-level behaviour**

- **`setup()` phase:**
  - Perform a health check via `GET /health`.
  - Authenticate using [`loginAndGetToken()`](../../tests/load/auth/helpers.js) to obtain a JWT suitable for WebSocket auth and HTTP calls.

- **Per-VU execution:**
  - Use `POST /api/games` to create AI-capable games (for example, human vs AI).
  - Derive the WebSocket URL (for example, `/socket.io`) from the base URL and reuse the Engine.IO/Socket.IO handshake logic from [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js).
...
- Emit the metrics defined in section 4, at minimum:
  ...
  - Connection/handshake metrics aligned with [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js).
...
- **S2-like throughput config:**
  ...
  - Intended for staging/perf runs as part of the P-01 performance gate in [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md).

### 5.2 Playwright WebSocket move latency spec

**File:** [`websocket-move-latency.e2e.spec.ts`](../../tests/e2e/websocket-move-latency.e2e.spec.ts)

**Behaviour**

- Use existing Playwright helpers and fixtures (see `tests/e2e` and [`playwright.config.ts`](../../playwright.config.ts)) to:
...
- Within the browser context:
  ...
  - Assert that these timings meet staging-level SLOs (for example, p95 ≤ 300 ms, p99 ≤ 600 ms), consistent with [`STRATEGIC_ROADMAP.md` §2.2](../planning/STRATEGIC_ROADMAP.md).
...
- Add npm scripts in [`package.json`](../../package.json) such as:
...
- Tag [`websocket-move-latency.e2e.spec.ts`](../../tests/e2e/websocket-move-latency.e2e.spec.ts) appropriately (for example, with a `@perf-smoke` annotation or a Playwright project name).
- Add a CI job that:
...
- As observed in the HTTP harness runs in [`LOAD_TEST_BASELINE_REPORT.md`](LOAD_TEST_BASELINE_REPORT.md), the local/dev environment can hit adaptive rate limits (for example, on `POST /api/games`) and even transient connection failures under aggressive k6 patterns.
...
- Interpret S3 metrics **alongside AI service metrics** from [`ai-service/app/metrics_base.py`](../../ai-service/app/metrics_base.py) and Node-side metrics in [`rulesParityMetrics`](../../src/server/utils/rulesParityMetrics.ts).
...
- Centralise Socket.IO framing helpers in a single module (for example, a shared helper imported by both [`websocket-stress.js`](../../tests/load/scenarios/websocket-stress.js) and [`websocket-gameplay.js`](../../tests/load/scenarios/websocket-gameplay.js)).
- Document and pin the expected Socket.IO/Engine.IO versions in [`package.json`](../../package.json) and/or a short protocol note, and review WebSocket load scripts whenever these dependencies change.
...
- Standardise metric names and tags as described in section 4, especially for:
  ...
  - Error-code counters derived from [`websocket.ts`](../../src/shared/types/websocket.ts).
