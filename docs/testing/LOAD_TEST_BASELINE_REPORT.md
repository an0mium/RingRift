...
- Topology: single-node staging stack via `docker compose -f docker-compose.yml -f docker-compose.staging.yml up --build` as described in [`QUICKSTART.md`](../../QUICKSTART.md).
- Target base URL: `http://localhost:3000` (Node backend + built client, API + WebSocket on the `app` service).
- Threshold environment: `THRESHOLD_ENV=staging`, mapping to the `staging` block in [`thresholds.json`](../../tests/load/config/thresholds.json).
- SLO sources (unchanged): [`PROJECT_GOALS.md`](../../PROJECT_GOALS.md), [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md), and [`docs/operations/ALERTING_THRESHOLDS.md`](../operations/ALERTING_THRESHOLDS.md).
...
- Script: [`game-creation.js`](../../tests/load/scenarios/game-creation.js)
- During [`setup()`](../../tests/load/scenarios/game-creation.js), the scenario attempted:
...
- Until a successful staging run is recorded:
  - Continue to treat the local/Docker baseline in [`docs/testing/LOAD_TEST_BASELINE.md`](LOAD_TEST_BASELINE.md) as the reference for “healthy” behaviour.
  - Use the SLO values and alert thresholds in [`PROJECT_GOALS.md`](../../PROJECT_GOALS.md), [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md), and [`docs/operations/ALERTING_THRESHOLDS.md`](../operations/ALERTING_THRESHOLDS.md) as the authoritative SSoTs.

**Planned commands for a complete Wave 3.1 staging baseline (once environment is healthy):**

These commands should be executed against a healthy staging stack on `http://localhost:3000` with `THRESHOLD_ENV=staging`, producing per-scenario summaries under `results/load/*.staging.summary.json` via [`tests/load/summary.js`](../../tests/load/summary.js):
...
The commands above all use the shared `handleSummary` factory in [`tests/load/summary.js`](../../tests/load/summary.js). For each scenario:
...
- The JSON structure is:
  ...
  - `thresholdsEnv`: the environment key used when looking up SLO thresholds in [`tests/load/config/thresholds.json`](../../tests/load/config/thresholds.json) (e.g. `staging`, `production`).
...
- [`tests/load/config/thresholds.json`](../../tests/load/config/thresholds.json)
- SLO strategy docs: [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md), [`PROJECT_GOALS.md`](../../PROJECT_GOALS.md), and (where present) [`docs/operations/ALERTING_THRESHOLDS.md`](../operations/ALERTING_THRESHOLDS.md).
...
- Script: [player-moves.js](../../tests/load/scenarios/player-moves.js)
...
**Threshold status vs script thresholds and SLOs ([STRATEGIC_ROADMAP.md](../planning/STRATEGIC_ROADMAP.md) §2.2):**
...
- The HTTP move harness endpoint and k6 scenario wiring are **functionally validated**:
  ...
  - The move payload generator [generateRandomMove()](../../tests/load/scenarios/player-moves.js) matches the backend MoveSchema used by the HTTP harness.
