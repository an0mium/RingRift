# Orchestrator Rollout Runbook Phases (Archived)

This file preserves the phase-by-phase rollout playbook that previously lived in
`docs/runbooks/ORCHESTRATOR_ROLLOUT_RUNBOOK.md`. It references removed flags
and is not executable. Use the current runbook for live operations.
...
defined in `docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` (§6) and
`docs/archive/ORCHESTRATOR_ROLLOUT_PHASES.md` (§8).
...
1. **CI gates (SLO-CI-ORCH-PARITY)**
   - Confirm the latest `main` build has a green `orchestrator-parity` job in
     CI (see `docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` §6.2):
     - `npm run test:orchestrator-parity:ts`
     - `./scripts/run-python-contract-tests.sh --verbose`
   - **PASS18 Weakest Aspect Check:**
     - Verify that the **Capture/Territory Host Parity** and **AI RNG Parity** suites listed in `docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` §6.5 are explicitly green.
   ...
   - The `orchestrator-parity` job is documented alongside other
     orchestrator-related lanes in `docs/security/SUPPLY_CHAIN_AND_CI_SECURITY.md`
...
6. **Optional AI evaluation overlay (non-blocking)**
   - For releases that materially change AI difficulty profiles or models, consider running the AI evaluation harness alongside the orchestrator gates:
     - See `docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` §6.10 and `docs/runbooks/AI_PERFORMANCE.md`.
...
3. **SLO check**
   - Confirm the staging SLOs from `docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` §6.3:
...
**Goal:** Run the orchestrator as the authoritative rules path for a **small slice of production traffic (1–10%)** to validate real-world latency, error rates, and parity before moving beyond 10%. This corresponds to the **Production Preview band** described in `docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` (§6.1.1) and `docs/archive/ORCHESTRATOR_ROLLOUT_PHASES.md` (§8.5).
...
1. **Environment posture**
   ...
   - If you want an extra shadow-only warmup:
     - Use the Phase 2 shadow config from `docs/archive/ORCHESTRATOR_ROLLOUT_PHASES.md` (§8.4) and verify shadow mismatch SLOs before switching `RINGRIFT_RULES_MODE=ts` for preview.
...
Use the SLO targets and success definition in `docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` (§6.1.1 and §6.4) as the numeric reference; this runbook focuses on **what to do** when those metrics move.
...
2. **Check orchestrator posture**
   - Confirm current flags via `/api/admin/orchestrator/status`:
     ...
     - `rolloutPercentage`
     - `shadowModeEnabled`
   - Map the environment to Phase 1/2/3 using the environment → phase table in `docs/archive/ORCHESTRATOR_ROLLOUT_PHASES.md` (§8.7).
...
5. **Close the loop**
   - After stabilising the system:
     - Ensure SLOs from `docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md` §6 are back within
       budget.
