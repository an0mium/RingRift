...
> - Training-data provenance SSoT for AI: `ai-service/TRAINING_DATA_REGISTRY.md` (local-only, gitignored)
...
- `ai-service/TRAINING_DATA_REGISTRY.md` (local-only, gitignored) lists many canonical DBs as `pending_gate` due to tooling issues, while square19 replay parity is now described as resolved in [`KNOWN_ISSUES.md`](../../../KNOWN_ISSUES.md). This likely reflects drift between “what is true now” and “what the registry currently asserts”.
...
| --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Documentation hygiene / drift**                                                 | There is demonstrable drift between status/registry docs and current reality (see [`CURRENT_STATE_ASSESSMENT.md`](../historical/CURRENT_STATE_ASSESSMENT.md) and `ai-service/TRAINING_DATA_REGISTRY.md` (local-only, gitignored)). | This is a _process and clarity risk_, but it does not directly threaten v1.0 correctness or uptime. It is also easier to remediate than production-scale validation.                                 |
...
- [ ] **PASS32-R2 — Repair doc drift in high-authority status/registry docs**
  ...
  - Acceptance criteria:
    ...
    - `ai-service/TRAINING_DATA_REGISTRY.md` (local-only, gitignored) entries about canonical DB gate status reflect the latest parity/health facts (or explicitly declare what is unknown and why).
  - Owner mode: **architect**
  - Depends on: none
