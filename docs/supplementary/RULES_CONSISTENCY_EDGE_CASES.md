...
- Implementation mapping in [`RULES_IMPLEMENTATION_MAPPING.md`](../rules/RULES_IMPLEMENTATION_MAPPING.md).
- Static analysis in [`archive/RULES_STATIC_VERIFICATION.md`](../../archive/RULES_STATIC_VERIFICATION.md).
- Scenario and soak analysis in [`archive/RULES_DYNAMIC_VERIFICATION.md`](../../archive/RULES_DYNAMIC_VERIFICATION.md).

Code behaviour is inferred primarily from the shared engine and orchestration:

- Turn / phase / forced elimination via [`TypeScript.turnLogic.advanceTurnAndPhase()`](../../src/shared/engine/turnLogic.ts) and backend / sandbox delegates.
- Movement / capture / markers via [`TypeScript.core`](../../src/shared/engine/core.ts), [`TypeScript.movementLogic`](../../src/shared/engine/movementLogic.ts), and [`TypeScript.captureLogic`](../../src/shared/engine/captureLogic.ts).
- Lines via [`TypeScript.lineDecisionHelpers`](../../src/shared/engine/lineDecisionHelpers.ts).
- Territory via [`TypeScript.territoryDetection`](../../src/shared/engine/territoryDetection.ts), [`TypeScript.territoryProcessing`](../../src/shared/engine/territoryProcessing.ts), and [`TypeScript.territoryDecisionHelpers`](../../src/shared/engine/territoryDecisionHelpers.ts).
- Victory / stalemate via [`TypeScript.victoryLogic.evaluateVictory()`](../../src/shared/engine/aggregates/VictoryAggregate.ts).
- Board invariants and repairs via [`TypeScript.BoardManager`](../../src/server/game/BoardManager.ts).
- Sandbox turn orchestration via [`TypeScript.ClientSandboxEngine` turn helpers](../../src/client/sandbox/ClientSandboxEngine.ts) composed with [`TypeScript.turnLogic.advanceTurnAndPhase`](../../src/shared/engine/turnLogic.ts).
...
**RR‑CANON rules:** `R070–R072`, `R080–R082`, `R100`, `R170–R173` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).  
**Key implementations:** [`TypeScript.turnLogic.advanceTurnAndPhase()`](../../src/shared/engine/turnLogic.ts), [`TypeScript.advanceGameForCurrentPlayer`](../../src/server/game/turn/TurnEngine.ts), [`TypeScript.RuleEngine`](../../src/server/game/RuleEngine.ts), [`TypeScript.ClientSandboxEngine.startTurnForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts), [`TypeScript.ClientSandboxEngine.maybeProcessForcedEliminationForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts), Python strict invariants (`ai-service/tests/invariants/**`).

#### 2.1.1 Intended interaction

- Phases execute in the strict order placement → movement / capture (including chains) → line processing → territory processing → victory ([`RR‑CANON‑R070–R071`](../../RULES_CANONICAL_SPEC.md)).
- At the start of a player’s action, if they control a stack but have **no legal placement, movement, or capture**, they must perform forced elimination ([`RR‑CANON‑R072`](../../RULES_CANONICAL_SPEC.md), [`RR‑CANON‑R100`](../../RULES_CANONICAL_SPEC.md)).
- Placement is:
  ...
  - Forbidden when `ringsInHand == 0` ([`RR‑CANON‑R080`](../../RULES_CANONICAL_SPEC.md)).
...
- **Backend / shared engine**
  - [`TypeScript.turnLogic.advanceTurnAndPhase()`](../../src/shared/engine/turnLogic.ts) exactly enforces the phase ladder: `ring_placement → movement/capture/chain_capture → line_processing → territory_processing`, then rotates `currentPlayer` and considers forced elimination for the next player.
  - Forced elimination eligibility uses delegates `hasAnyPlacement`, `hasAnyMovement`, `hasAnyCapture`. Backend implementations in [`TypeScript.TurnEngine.hasValidPlacements`](../../src/server/game/turn/TurnEngine.ts) and siblings query [`TypeScript.RuleEngine.getValidMoves`](../../src/server/game/RuleEngine.ts), so “no actions” is defined in terms of actual move enumeration, including `skip_placement`.
  - If a player has stacks but `hasAnyPlacement == hasAnyMovement == hasAnyCapture == false`, `advanceTurnAndPhase` calls `applyForcedElimination`, which in backend is [`TypeScript.turn.processForcedElimination()`](../../src/server/game/turn/TurnEngine.ts) eliminating an entire cap from a chosen stack and then re‑running victory checks. This is consistent with [`RR‑CANON‑R100`](../../RULES_CANONICAL_SPEC.md).
...
- **Sandbox (historical path; legacy `sandboxTurnEngine`, now removed)**
  - In pre‑consolidation builds, sandbox phase advancement used the shared sequencer via [`TypeScript.sandboxTurnEngine.advanceTurnAndPhaseForCurrentPlayerSandbox`](../../src/client/sandbox/ClientSandboxEngine.ts) but wrapped it with start‑of‑turn helpers.
  - At the beginning of a turn, [`TypeScript.sandboxTurnEngine.startTurnForCurrentPlayerSandbox`](../../src/client/sandbox/ClientSandboxEngine.ts) called [`TypeScript.sandboxTurnEngine.maybeProcessForcedEliminationForCurrentPlayerSandbox`](../../src/client/sandbox/ClientSandboxEngine.ts), which:
  - In the current architecture, the same semantics are implemented by [`TypeScript.ClientSandboxEngine` turn helpers](../../src/client/sandbox/ClientSandboxEngine.ts) composed with the shared [`TypeScript.turnLogic.advanceTurnAndPhase`](../../src/shared/engine/turnLogic.ts); sandbox no longer uses a separate consolidated `sandboxTurnEngine.ts` module.
...
- **Python**
  ...
  - A strict “no active player without a legal move or forced elimination” invariant (see [`archive/RULES_STATIC_VERIFICATION.md`](../../archive/RULES_STATIC_VERIFICATION.md)) ensures dynamically that every ACTIVE state offers at least one action to the `current_player`, including forced elimination.
...
**RR‑CANON rules:** `R090–R092`, `R100–R103`, `R120–R122`, `R140–R145` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).  
**Key implementations:** [`TypeScript.core`](../../src/shared/engine/core.ts), [`TypeScript.movementLogic.enumerateSimpleMoveTargetsFromStack()`](../../src/shared/engine/movementLogic.ts), [`TypeScript.captureLogic.enumerateCaptureMoves()`](../../src/shared/engine/captureLogic.ts), backend and sandbox movement / capture application, [`TypeScript.lineDecisionHelpers`](../../src/shared/engine/lineDecisionHelpers.ts), [`TypeScript.territoryProcessing.applyTerritoryRegion()`](../../src/shared/engine/territoryProcessing.ts), [`TypeScript.territoryDecisionHelpers`](../../src/shared/engine/territoryDecisionHelpers.ts), [`TypeScript.turnLogic.advanceTurnAndPhase()`](../../src/shared/engine/turnLogic.ts).

#### 2.2.1 Intended interaction

- Movement and capture segments:
  - Apply marker path effects immediately (departure marker, intermediate flips/collapses, landing‑cell own‑marker removal and top‑ring elimination) ([`RR‑CANON‑R092`](../../RULES_CANONICAL_SPEC.md), [`RR‑CANON‑R102`](../../RULES_CANONICAL_SPEC.md)).
...
- After **all** movement and any full capture chain:
  - Detect and process lines sequentially ([`RR‑CANON‑R120–R122`](../../RULES_CANONICAL_SPEC.md)).
  - Then detect and process disconnected regions ([`RR‑CANON‑R140–R145`](../../RULES_CANONICAL_SPEC.md)).
  - Then evaluate victory ([`RR‑CANON‑R170–R173`](../../RULES_CANONICAL_SPEC.md)).
...
- Movement and capture reachability and paths are centralised in [`TypeScript.core.hasAnyLegalMoveOrCaptureFromOnBoard()`](../../src/shared/engine/core.ts) and [`TypeScript.movementLogic.enumerateSimpleMoveTargetsFromStack()`](../../src/shared/engine/movementLogic.ts), which:
  - Use [`TypeScript.getMovementDirectionsForBoardType()`](../../src/shared/engine/core.ts) and [`TypeScript.getPathPositions()`](../../src/shared/engine/core.ts) to ensure straight‑line rays and distance ≥ stackHeight.
  - Treat stacks and collapsed spaces as blocking, with markers allowed along the path.
- Marker path effects for both movement and captures are applied via [`TypeScript.applyMarkerEffectsAlongPathOnBoard()`](../../src/shared/engine/core.ts) using host‑provided marker mutators (`setMarker`, `flipMarker`, `collapseMarker`).
...
- Phase transitions in [`TypeScript.turnLogic.advanceTurnAndPhase()`](../../src/shared/engine/turnLogic.ts) always move from `movement` / `capture` / `chain_capture` to `line_processing`, and from `line_processing` to `territory_processing`, before any turn rotation.
- Line decisions:
  - Detection via [`TypeScript.lineDetection.findAllLines`](../../src/shared/engine/lineDetection.ts).
  - Decision surfaces and state updates via [`TypeScript.lineDecisionHelpers.enumerateProcessLineMoves()`](../../src/shared/engine/lineDecisionHelpers.ts), [`TypeScript.lineDecisionHelpers.applyProcessLineDecision()`](../../src/shared/engine/lineDecisionHelpers.ts), and [`TypeScript.lineDecisionHelpers.applyChooseLineRewardDecision()`](../../src/shared/engine/lineDecisionHelpers.ts).
  - Exact‑length lines always set `pendingLineRewardElimination = true`; overlength lines set it only for “collapse all” choices, matching [`RR‑CANON‑R122`](../../RULES_CANONICAL_SPEC.md).
- Territory decisions:
  - Region detection via [`TypeScript.territoryDetection.findDisconnectedRegions`](../../src/shared/engine/territoryDetection.ts).
  - Self‑elimination gating plus per‑region processing via [`TypeScript.territoryProcessing`](../../src/shared/engine/territoryProcessing.ts) and [`TypeScript.territoryDecisionHelpers.applyProcessTerritoryRegionDecision()`](../../src/shared/engine/territoryDecisionHelpers.ts).
...
- Both backend and sandbox attach victory checks **after** line and territory decisions in their move / decision pipelines, ultimately funnelling into [`TypeScript.victoryLogic.evaluateVictory()`](../../src/shared/engine/aggregates/VictoryAggregate.ts).
...
**RR‑CANON rules:** `R140–R145` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).  
**Key implementations:** [`TypeScript.territoryDetection.findDisconnectedRegions`](../../src/shared/engine/territoryDetection.ts), [`TypeScript.territoryProcessing.canProcessTerritoryRegion()`](../../src/shared/engine/territoryProcessing.ts), [`TypeScript.territoryProcessing.applyTerritoryRegion()`](../../src/shared/engine/territoryProcessing.ts), [`TypeScript.territoryDecisionHelpers.enumerateProcessTerritoryRegionMoves()`](../../src/shared/engine/territoryDecisionHelpers.ts), [`TypeScript.territoryDecisionHelpers.applyProcessTerritoryRegionDecision()`](../../src/shared/engine/territoryDecisionHelpers.ts), [`TypeScript.territoryDecisionHelpers.enumerateTerritoryEliminationMoves()`](../../src/shared/engine/territoryDecisionHelpers.ts).

#### 2.3.1 Intended interaction

- Regions are maximal sets of non‑collapsed cells connected by `territoryAdjacency` ([`RR‑CANON‑R040`](../../RULES_CANONICAL_SPEC.md), [`RR‑CANON‑R140`](../../RULES_CANONICAL_SPEC.md)).
- A region is “disconnected” only if it is both:
  - Physically isolated by collapsed spaces and/or markers of a **single** border colour ([`RR‑CANON‑R141`](../../RULES_CANONICAL_SPEC.md)).
  - Colour‑disconnected: its RegionColors are a strict subset of ActiveColors ([`RR‑CANON‑R142`](../../RULES_CANONICAL_SPEC.md)).
- For each region and moving player `P`:
  - `P` may process the region only if, in the hypothetical state where the region’s rings are removed, `P` still controls at least one ring/cap **outside** the region ([`RR‑CANON‑R143`](../../RULES_CANONICAL_SPEC.md)).
  - Processing a region always requires **exactly one** self‑elimination from outside that region ([`RR‑CANON‑R145`](../../RULES_CANONICAL_SPEC.md)).
...
- Region processing order is per‑region optional; players may process any subset of eligible regions in any order ([`RR‑CANON‑R144`](../../RULES_CANONICAL_SPEC.md)).

#### 2.3.2 Observed behaviour

- Disconnected regions are computed centrally by [`TypeScript.territoryDetection.findDisconnectedRegions`](../../src/shared/engine/territoryDetection.ts) and re‑used by backend and sandbox via [`TypeScript.BoardManager.findDisconnectedRegions`](../../src/server/game/BoardManager.ts) and sandbox adapters.
- Processability is determined by [`TypeScript.territoryProcessing.canProcessTerritoryRegion`](../../src/shared/engine/territoryProcessing.ts), which checks for at least one controlled stack **outside** the region.
- [`TypeScript.territoryDecisionHelpers.enumerateProcessTerritoryRegionMoves`](../../src/shared/engine/territoryDecisionHelpers.ts) filters disconnected regions through `canProcessTerritoryRegion` at enumeration time, and hosts re‑call it after each region/self‑elimination pair, naturally implementing the “one outside cap per region” budget.
- [`TypeScript.territoryDecisionHelpers.applyProcessTerritoryRegionDecision`](../../src/shared/engine/territoryDecisionHelpers.ts) calls [`TypeScript.territoryProcessing.applyTerritoryRegion`](../../src/shared/engine/territoryProcessing.ts) to:
...
- Self‑elimination is then expressed as one or more `eliminate_rings_from_stack` decisions; [`TypeScript.territoryDecisionHelpers.enumerateTerritoryEliminationMoves`](../../src/shared/engine/territoryDecisionHelpers.ts) currently:
...
- Multi‑region chains:
  - After each region + self‑elimination, engines re‑detect or re‑filter regions using current board state, so chain‑reaction regions (newly disconnected due to the collapse) are discovered and processed deterministically in whatever order the player chooses, matching [`RR‑CANON‑R144`](../../RULES_CANONICAL_SPEC.md).
...
**RR‑CANON rules:** `R170–R173`, `R190–R191` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).  
**Key implementations:** [`TypeScript.victoryLogic.evaluateVictory()`](../../src/shared/engine/aggregates/VictoryAggregate.ts), [`TypeScript.turnLogic`](../../src/shared/engine/turnLogic.ts), backend `RuleEngine.checkGameEnd`, sandbox victory wrappers, Python `GameEngine` and strict no‑move invariants (`ai-service/tests/invariants/**`).

#### 2.4.1 Intended interaction

- Elimination victory when a player’s credited eliminations reach `victoryThreshold` ([`RR‑CANON‑R170`](../../RULES_CANONICAL_SPEC.md)).
- Territory victory when `territorySpaces` reach `territoryVictoryThreshold` ([`RR‑CANON‑R171`](../../RULES_CANONICAL_SPEC.md)).
- Last‑player‑standing when, after **three consecutive full rounds** of turns:
  ...
  - All others have no legal placements, movements, or captures during those rounds (even if they own buried rings) ([`RR‑CANON‑R172`](../../RULES_CANONICAL_SPEC.md)).
- Global stalemate only when **no player** has any legal placement, movement, capture, or forced elimination, which can occur only after all stacks are gone; tie‑breaking ladder territory → eliminated rings (including converted rings in hand) → markers → last actor ([`RR‑CANON‑R173`](../../RULES_CANONICAL_SPEC.md)).
- S‑invariant `S = M + C + E` must be monotone and bounded under all **legal** moves; illegal states are outside the rules model ([`RR‑CANON‑R191`](../../RULES_CANONICAL_SPEC.md)).

#### 2.4.2 Observed behaviour

- [`TypeScript.victoryLogic.evaluateVictory()`](../../src/shared/engine/aggregates/VictoryAggregate.ts):
  ...
  - There is **no explicit implementation** of non‑bare‑board last‑player‑standing as defined in [`RR‑CANON‑R172`](../../RULES_CANONICAL_SPEC.md) (three-round exclusive real-action condition); instead, games continue until ring‑elimination, territory victory, or bare‑board stalemate occurs.
- Turn rotation in [`TypeScript.turnLogic`](../../src/shared/engine/turnLogic.ts) and Python strict invariants ensure that:
...
- S‑invariant accounting is centralised in [`TypeScript.computeProgressSnapshot()`](../../src/shared/engine/core.ts) and is used extensively in backend / sandbox history and TS↔Python parity, as well as in Python self‑play soaks.
...
**RR‑CANON rules:** `R021–R023`, `R030–R031`, `R050–R052`, `R190–R191` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).  
**Key implementations:** [`TypeScript.BoardManager.assertBoardInvariants()`](../../src/server/game/BoardManager.ts), marker / collapsed helpers in [`TypeScript.BoardManager`](../../src/server/game/BoardManager.ts), shared `BoardState` invariants in [`src/shared/types/game.ts`](../../src/shared/types/game.ts), S‑invariant helpers in [`TypeScript.core.computeProgressSnapshot()`](../../src/shared/engine/core.ts), sandbox `ClientSandboxEngine.assertBoardInvariants`.

#### 2.5.1 Intended interaction

- Each cell is in exactly one of: empty, stack, marker, collapsed territory ([`RR‑CANON‑R021`](../../RULES_CANONICAL_SPEC.md), [`RR‑CANON‑R030–R031`](../../RULES_CANONICAL_SPEC.md), [`RR‑CANON‑R052`](../../RULES_CANONICAL_SPEC.md)).
...
- [`TypeScript.BoardManager.assertBoardInvariants()`](../../src/server/game/BoardManager.ts) first performs a **repair pass**:
...
- Sandbox mirrors exclusivity semantics in [`TypeScript.ClientSandboxEngine`](../../src/client/sandbox/ClientSandboxEngine.ts), but its `assertBoardInvariants` helper in tests chooses to **throw** rather than repair; there is no marker‑deletion repair step there.
...
- **RR‑CANON:** Exclusivity of stack / marker / collapsed (`R021`, `R030–R031`, `R052`, `R191`) in [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md).
- **Code:** [`TypeScript.BoardManager.assertBoardInvariants()`](../../src/server/game/BoardManager.ts) and stack / marker / collapsed setters.
- **Assessment:**
...
- **RR‑CANON:** Ring counts and caps (`R020–R023`, `R060–R062`, `R080–R082`) in [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md).
- **Code:** `ringsPerPlayer` cap enforced using **total height of stacks controlled by the player**, not the count of rings of that colour, in:
  - Backend placement validator and enumeration (see [`archive/RULES_STATIC_VERIFICATION.md`](../../archive/RULES_STATIC_VERIFICATION.md)).
  - Sandbox `hasAnyPlacement` and forced‑elimination gating via [`TypeScript.ClientSandboxEngine` turn helpers](../../src/client/sandbox/ClientSandboxEngine.ts).
- **Assessment:**
...
- **Recommendation:** either:
  - **Canonicalise** this as “placement cap is based on the total height of stacks you control, including buried rings, up to `ringsPerPlayer`”, and update [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md) accordingly; or
...
- **RR‑CANON:** Placement optionality and turn phases (`R070–R072`, `R080–R082`) in [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md).
- **Code:** Backend `skip_placement` moves in [`TypeScript.RuleEngine`](../../src/server/game/RuleEngine.ts) and backend turn logic, vs sandbox heuristics in [`TypeScript.ClientSandboxEngine.startTurnForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts) and [`TypeScript.ClientSandboxEngine.maybeProcessForcedEliminationForCurrentPlayer`](../../src/client/sandbox/ClientSandboxEngine.ts).
- **Assessment:**
...
- **RR‑CANON:** Chain capture semantics (`R101–R103`) in [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md).
- **Code:** Shared stub [`TypeScript.captureChainHelpers.enumerateChainCaptureSegments()`](../../src/shared/engine/chainCaptureTracking.ts) and [`TypeScript.captureChainHelpers.getChainCaptureContinuationInfo()`](../../src/shared/engine/chainCaptureTracking.ts) vs live logic in:
  - Backend [`TypeScript.CaptureAggregate`](../../src/shared/engine/aggregates/CaptureAggregate.ts) and [`TypeScript.GameEngine`](../../src/server/game/GameEngine.ts).
  - Sandbox [`TypeScript.sandboxMovementEngine.performCaptureChainSandbox`](../../src/client/sandbox/sandboxMovement.ts).
- **Assessment:**
  ...
  - Future refactors that centralise chain logic into the shared helper must preserve:
    - Enumeration equivalence with [`TypeScript.captureLogic.enumerateCaptureMoves()`](../../src/shared/engine/captureLogic.ts).
...
- **RR‑CANON:** Victory, last‑player‑standing, stalemate ladder (`R170–R173`) in [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md).
- **Code:** [`TypeScript.victoryLogic.evaluateVictory()`](../../src/shared/engine/aggregates/VictoryAggregate.ts), backend `RuleEngine.checkGameEnd`, sandbox victory wrappers, Python `GameEngine`.
- **Assessment:**
...
- **RR‑CANON:** Self‑elimination prerequisite and processing order (`R143–R145`) in [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md).
- **Code:** [`TypeScript.territoryProcessing.canProcessTerritoryRegion()`](../../src/shared/engine/territoryProcessing.ts), [`TypeScript.territoryProcessing.applyTerritoryRegion()`](../../src/shared/engine/territoryProcessing.ts), [`TypeScript.territoryDecisionHelpers`](../../src/shared/engine/territoryDecisionHelpers.ts).
- **Assessment:**
  ...
  - Shared elimination helper [`TypeScript.territoryDecisionHelpers.enumerateTerritoryEliminationMoves`](../../src/shared/engine/territoryDecisionHelpers.ts) currently **ignores** `processedRegionId` and simply offers eliminations from any controlled stack.
...
- **RR‑CANON rules:** `R021`, `R030–R031`, `R050–R052`, `R191` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).
- **Code / tests:** [`TypeScript.BoardManager.assertBoardInvariants()`](../../src/server/game/BoardManager.ts), [`TypeScript.BoardManager.setMarker`](../../src/server/game/BoardManager.ts), [`TypeScript.BoardManager.setStack`](../../src/server/game/BoardManager.ts), sandbox invariant checks in `ClientSandboxEngine.assertBoardInvariants` and invariant soaks documented in [`docs/testing/STRICT_INVARIANT_SOAKS.md`](../testing/STRICT_INVARIANT_SOAKS.md).
...
- **RR‑CANON rules:** `R020–R023`, `R060–R062`, `R080–R082` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).
- **Code / tests:** Shared placement validator and RuleEngine enumeration described in [`archive/RULES_STATIC_VERIFICATION.md`](../../archive/RULES_STATIC_VERIFICATION.md); sandbox `hasAnyPlacement` and forced‑elimination gating in [`TypeScript.sandboxTurnEngine`](../../src/client/sandbox/ClientSandboxEngine.ts); AI / Python parity tests around placement capacity in `ai-service/tests/**`.
...
- **RR‑CANON rules:** `R070–R072`, `R080–R082` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).
- **Code / tests:** Backend `skip_placement` handling in [`TypeScript.RuleEngine`](../../src/server/game/RuleEngine.ts), shared turn logic in [`TypeScript.turnLogic`](../../src/shared/engine/turnLogic.ts), sandbox start‑of‑turn helpers in [`TypeScript.sandboxTurnEngine.startTurnForCurrentPlayerSandbox`](../../src/client/sandbox/ClientSandboxEngine.ts) and [`TypeScript.sandboxTurnEngine.maybeProcessForcedEliminationForCurrentPlayerSandbox`](../../src/client/sandbox/ClientSandboxEngine.ts), parity tests in `tests/unit/ClientSandboxEngine.*.test.ts`.
...
- **RR‑CANON rules:** `R101–R103` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).
- **Code / tests:** [`TypeScript.captureChainHelpers`](../../src/shared/engine/chainCaptureTracking.ts), backend [`TypeScript.CaptureAggregate`](../../src/shared/engine/aggregates/CaptureAggregate.ts), sandbox [`TypeScript.ClientSandboxEngine.performCaptureChainInternal`](../../src/client/sandbox/ClientSandboxEngine.ts), FAQ Q15 and cyclic capture tests in [`tests/scenarios/FAQ_Q15.test.ts`](../../tests/scenarios/FAQ_Q15.test.ts) and `tests/unit/GameEngine.cyclicCapture.*.test.ts`.
...
- **RR‑CANON rules:** `R143–R145` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).
- **Code / tests:** [`TypeScript.territoryProcessing`](../../src/shared/engine/territoryProcessing.ts), [`TypeScript.territoryDecisionHelpers`](../../src/shared/engine/territoryDecisionHelpers.ts), territory tests in [`tests/unit/territoryDecisionHelpers.shared.test.ts`](../../tests/unit/territoryDecisionHelpers.shared.test.ts) and [`tests/unit/BoardManager.territoryDisconnection.test.ts`](../../tests/unit/BoardManager.territoryDisconnection.test.ts).
...
- **RR‑CANON rules:** `R170–R173` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).
- **Code / tests:** [`TypeScript.victoryLogic.evaluateVictory()`](../../src/shared/engine/aggregates/VictoryAggregate.ts), backend victory scenarios in [`tests/unit/GameEngine.victory.scenarios.test.ts`](../../tests/unit/GameEngine.victory.scenarios.test.ts), stalemate / forced‑elimination scenarios in [`tests/scenarios/ForcedEliminationAndStalemate.test.ts`](../../tests/scenarios/ForcedEliminationAndStalemate.test.ts), Python invariants in `ai-service/tests/invariants/**`.
...
- **RR‑CANON rules:** `R072`, `R100` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).
- **Code / tests:** [`TypeScript.turn.processForcedElimination`](../../src/server/game/turn/TurnEngine.ts), [`TypeScript.ClientSandboxEngine.forceEliminateCap`](../../src/client/sandbox/ClientSandboxEngine.ts), invariant tests in `ai-service/tests/invariants/test_active_no_moves_movement_forced_elimination_regression.py`.
...
- **RR‑CANON rules:** `R090–R092`, `R100–R103`, `R120–R122`, `R140–R145`, `R170–R173` ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).
- **Code / tests:** Movement / capture helpers in [`TypeScript.core`](../../src/shared/engine/core.ts), [`TypeScript.movementLogic`](../../src/shared/engine/movementLogic.ts), [`TypeScript.captureLogic`](../../src/shared/engine/captureLogic.ts); line and territory helpers in [`TypeScript.lineDecisionHelpers`](../../src/shared/engine/lineDecisionHelpers.ts) and [`TypeScript.territoryDecisionHelpers`](../../src/shared/engine/territoryDecisionHelpers.ts); turn sequencing in [`TypeScript.turnLogic`](../../src/shared/engine/turnLogic.ts); dynamic scenarios in [`tests/unit/GameEngine.lines.scenarios.test.ts`](../../tests/unit/GameEngine.lines.scenarios.test.ts), [`tests/unit/BoardManager.territoryDisconnection.test.ts`](../../tests/unit/BoardManager.territoryDisconnection.test.ts), and [`tests/scenarios/RulesMatrix.Territory.MiniRegion.test.ts`](../../tests/scenarios/RulesMatrix.Territory.MiniRegion.test.ts).
...
- **RR‑CANON rules:** `R110–R115` (Recovery Action), `R172` (Last Player Standing), `R200–R207` (Active No Moves), `R100` (Forced Elimination) ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).
- **Code / tests:** Recovery action detection in [`TypeScript.globalActions.canPerformRecoveryAction()`](../../src/shared/engine/globalActions.ts), [`TypeScript.globalActions.enumerateRecoverySlides()`](../../src/shared/engine/globalActions.ts); line processing via [`TypeScript.lineDecisionHelpers`](../../src/shared/engine/lineDecisionHelpers.ts); victory logic in [`TypeScript.victoryLogic.evaluateVictory()`](../../src/shared/engine/aggregates/VictoryAggregate.ts); ANM detection in shared engine helpers; weird state detection in [`TypeScript.gameStateWeirdness`](../../src/client/utils/gameStateWeirdness.ts).
...
- **RR‑CANON rules:** `R091–R092` (movement landing), `R101–R102` (capture landing) ([`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md)).
- **Code / tests:** [`TypeScript.CaptureAggregate.mutateCapture()`](../../src/shared/engine/aggregates/CaptureAggregate.ts) section 5 (landing marker handling), [`Python.game_engine/__init__.py`](../../ai-service/app/game_engine/__init__.py) (parallel implementation), [`tests/unit/captureLogic.shared.test.ts`](../../tests/unit/captureLogic.shared.test.ts), [`tests/unit/CaptureAggregate.chainCapture.shared.test.ts`](../../tests/unit/CaptureAggregate.chainCapture.shared.test.ts).
...
- **Intended behaviour (RR‑CANON):**
  - Per [`RR‑CANON‑R091`](../../RULES_CANONICAL_SPEC.md): "Landing cell may contain any marker (own or opponent); landing on markers is always legal but incurs a cap‑elimination cost."
  - Per [`RR‑CANON‑R092`](../../RULES_CANONICAL_SPEC.md): "At landing cell: If there is any marker (own or opponent), remove that marker (do NOT collapse it), then place the moving stack. If a marker was present (regardless of owner), immediately eliminate the top ring of the moving stack's cap and credit it to P."
  - Per [`RR‑CANON‑R101`](../../RULES_CANONICAL_SPEC.md): "Landing is an empty cell or a cell with any marker (own or opponent)."
  - Per [`RR‑CANON‑R102`](../../RULES_CANONICAL_SPEC.md): "If landing on any marker (own or opponent), remove the marker (do NOT collapse it), then land and immediately eliminate the top ring of the attacking stack's cap, crediting it to P (before line/territory processing)."
- **Observed behaviour:**
...
- **S‑invariant impact:** Each capture landing on a marker causes:
  ...
  - **Net ΔS = +1** (progress preserved; documented in [`RULES_TERMINATION_ANALYSIS.md`](RULES_TERMINATION_ANALYSIS.md) Section 2.2).
- **Cyclic capture and 180° reversal termination:** Even when marker landings offset height gains (net ΔH = 0 per capture segment), infinite cyclic captures and 180° reversals are **impossible**. Both patterns are bounded by the **same three termination mechanisms**. See [`RULES_TERMINATION_ANALYSIS.md`](RULES_TERMINATION_ANALYSIS.md) Section 4.1 for deep analysis showing termination is guaranteed by:
...
High‑risk themes identified in [`archive/RULES_STATIC_VERIFICATION.md`](../../archive/RULES_STATIC_VERIFICATION.md) and [`archive/RULES_DYNAMIC_VERIFICATION.md`](../../archive/RULES_DYNAMIC_VERIFICATION.md) are addressed as follows:
