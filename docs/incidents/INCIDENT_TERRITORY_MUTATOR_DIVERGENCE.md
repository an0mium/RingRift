...
This document records the incident where the Python [`TerritoryMutator`](../../ai-service/app/rules/mutators/territory.py) diverged from the canonical [`GameEngine.apply_move()`](../../ai-service/app/game_engine/__init__.py) during territory-processing, causing crashes in the **territory dataset generator** CLI when running in mixed-engine mode.
...
would intermittently fail with a `RuntimeError` raised from [`DefaultRulesEngine.apply_move()`](../../ai-service/app/rules/default_engine.py) while exercising territory moves under mutator shadow contracts. The representative error was:
...
- The crash occurred during internal shadow-contract checks in [`DefaultRulesEngine`](../../ai-service/app/rules/default_engine.py), not in the canonical [`GameEngine.apply_move()`](../../ai-service/app/game_engine/__init__.py) result.
...
- [`test_territory_forced_elimination_divergence_pattern`](../../ai-service/tests/test_territory_forced_elimination_divergence.py)
...
3. The territory-processing decision surface exposes **explicit self-elimination moves**:
   - In TS, via `eliminate_rings_from_stack` moves enumerated by [`enumerateTerritoryEliminationMoves()`](../../src/shared/engine/territoryDecisionHelpers.ts).
   - In Python, via `ELIMINATE_RINGS_FROM_STACK` moves enumerated by [`GameEngine._get_territory_processing_moves()`](../../ai-service/app/game_engine/__init__.py).
...
1. The canonical engine [`GameEngine`](../../ai-service/app/game_engine/__init__.py) enumerates territory decisions via:
   - Region and elimination enumeration: [`_get_territory_processing_moves()`](../../ai-service/app/game_engine/__init__.py).
   - For `ELIMINATE_RINGS_FROM_STACK`:
     - The move id is of the form `eliminate-{pos.to_key()}`.
     - The move is applied inside [`GameEngine.apply_move()`](../../ai-service/app/game_engine/__init__.py), which delegates to [`GameEngine._apply_forced_elimination()`](../../ai-service/app/game_engine/__init__.py) when `move.type == MoveType.ELIMINATE_RINGS_FROM_STACK`.

2. After applying the explicit elimination, [`GameEngine._update_phase()`](../../ai-service/app/game_engine/__init__.py) is invoked:
   - For `MoveType.ELIMINATE_RINGS_FROM_STACK`, `_update_phase` calls [`GameEngine._end_turn()`](../../ai-service/app/game_engine/__init__.py) directly:
     - See the explicit branch at [`_update_phase` lines handling `ELIMINATE_RINGS_FROM_STACK`](../../ai-service/app/game_engine/__init__.py).

3. Inside [`_end_turn`](../../ai-service/app/game_engine/__init__.py):
   ...
   - For each candidate next player:
     ...
     - Computes whether the player has any legal placements/movements/captures via [`_has_valid_actions()`](../../ai-service/app/game_engine/__init__.py).
   - If the next player controls at least one stack but has **no legal actions**, `_end_turn` calls [`_perform_forced_elimination_for_player()`](../../ai-service/app/game_engine/__init__.py).

4. [`_perform_forced_elimination_for_player`](../../ai-service/app/game_engine/__init__.py):
   - Enumerates forced elimination moves via [`_get_forced_elimination_moves()`](../../ai-service/app/game_engine/__init__.py).
   - Applies the first such move via [`_apply_forced_elimination()`](../../ai-service/app/game_engine/__init__.py).
   - Calls [`_check_victory()`](../../ai-service/app/game_engine/__init__.py) to see if victory thresholds have been reached.
...
- [`TerritoryMutator`](../../ai-service/app/rules/mutators/territory.py) is a thin façade around the engine:
  ...
  Critically, it does **not** call [`GameEngine._update_phase()`](../../ai-service/app/game_engine/__init__.py) or [`GameEngine._end_turn()`](../../ai-service/app/game_engine/__init__.py). It only applies the **explicit** elimination step.

- [`DefaultRulesEngine.apply_move()`](../../ai-service/app/rules/default_engine.py) maintains per-move **shadow contracts** for mutators:
  - For territory moves:
  ...
  - `next_via_engine` is the canonical result from [`GameEngine.apply_move(state, move)`](../../ai-service/app/game_engine/__init__.py), which includes:
...
- Region detection and Q23 gating live in [`territoryDetection.ts`](../../src/shared/engine/territoryDetection.ts) and [`territoryProcessing.ts`](../../src/shared/engine/territoryProcessing.ts).
- Explicit territory decisions (region selection and elimination from a stack) are modelled as `choose_territory_option` (legacy alias: `process_territory_region`) and `eliminate_rings_from_stack` moves via [`territoryDecisionHelpers.ts`](../../src/shared/engine/territoryDecisionHelpers.ts).
...
The fix adds a targeted **escape hatch** in the territory branch of [`DefaultRulesEngine.apply_move`](../../ai-service/app/rules/default_engine.py) that mirrors the existing movement escape hatch.
...
The mutator-first orchestration path in [`_apply_move_with_mutators`](../../ai-service/app/rules/default_engine.py):

- Uses the same [`GameEngine._apply_forced_elimination`](../../ai-service/app/game_engine/__init__.py) helper as the canonical engine when processing `ELIMINATE_RINGS_FROM_STACK` and `FORCED_ELIMINATION` moves.
- Explicitly calls [`GameEngine._update_phase()`](../../ai-service/app/game_engine/__init__.py) and `GameEngine._check_victory()` on the mutator-driven state.
...
All of the following live in [`test_territory_forced_elimination_divergence.py`](../../ai-service/tests/test_territory_forced_elimination_divergence.py):

1. [`test_territory_forced_elimination_divergence_pattern`](../../ai-service/tests/test_territory_forced_elimination_divergence.py)
...
2. [`test_default_engine_no_forced_elim_strict_contract`](../../ai-service/tests/test_territory_forced_elimination_divergence.py)
...
3. [`test_default_engine_forced_elim_escape_hatch`](../../ai-service/tests/test_territory_forced_elimination_divergence.py)
...
The CLI smoke test [`test_generate_territory_dataset_mixed_smoke`](../../ai-service/tests/test_generate_territory_dataset_smoke.py) drives the full territory dataset generator in mixed mode:
...
The fix is also validated indirectly by the broader parity and mutator equivalence tests documented in [`RULES_ENGINE_ARCHITECTURE.md`](../architecture/RULES_ENGINE_ARCHITECTURE.md), including:

- Python vs TS trace fixtures:
  - [`ai-service/tests/parity/test_rules_parity_fixtures.py`](../../ai-service/tests/parity/test_rules_parity_fixtures.py) ensures [`GameEngine.apply_move`](../../ai-service/app/game_engine/__init__.py) matches TS trace fixtures (hash + S-invariant parity).
  - Additional tests ensure [`DefaultRulesEngine.apply_move`](../../ai-service/app/rules/default_engine.py) remains in full-state lockstep with `GameEngine.apply_move` for all moves in the traces.
- Mutator-first scenario tests:
  - [`ai-service/tests/rules/test_default_engine_mutator_first_scenarios.py`](../../ai-service/tests/rules/test_default_engine_mutator_first_scenarios.py) exercises `mutator_first=True` scenarios, including synthetic territory-processing setups, and asserts equality with the canonical engine.
...
- These failures:
  ...
  - Remain tracked in the general AI/engine test backlog (see [`docs/supplementary/AI_IMPROVEMENT_BACKLOG.md`](../supplementary/AI_IMPROVEMENT_BACKLOG.md) and [`KNOWN_ISSUES.md`](../../KNOWN_ISSUES.md)).
...
- Strengthening TS↔Python parity for territory decision helpers and forced-elimination sequences:
  - TS: [`territoryDecisionHelpers.ts`](../../src/shared/engine/territoryDecisionHelpers.ts), [`territoryProcessing.ts`](../../src/shared/engine/territoryProcessing.ts).
  - Python: [`BoardManager.find_disconnected_regions`](../../ai-service/app/board_manager.py), [`GameEngine._apply_territory_claim`](../../ai-service/app/game_engine/__init__.py), [`GameEngine._perform_forced_elimination_for_player`](../../ai-service/app/game_engine/__init__.py).
...
These items are captured and prioritized in the AI/rules/training section of the strategic roadmap; see [`STRATEGIC_ROADMAP.md`](../planning/STRATEGIC_ROADMAP.md) for the consolidated checklist.
