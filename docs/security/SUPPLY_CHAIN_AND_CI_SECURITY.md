...
- Secret handling in local `.env` files (for example [`.env.example`](../../.env.example), [`.env.staging`](../../.env.staging)) and in CI (for example `GITHUB_TOKEN`, `SNYK_TOKEN`).
...
- **Scope (likely files):** [`README.md`](../../README.md), [`.env.example`](../../.env.example), [`.env.staging`](../../.env.staging), [`docs/operations/OPERATIONS_DB.md`](../operations/OPERATIONS_DB.md), [`docs/security/SECURITY_THREAT_MODEL.md`](SECURITY_THREAT_MODEL.md), and possibly comments in [`ci.yml`](../../.github/workflows/ci.yml).
- **Implementation outline:**
