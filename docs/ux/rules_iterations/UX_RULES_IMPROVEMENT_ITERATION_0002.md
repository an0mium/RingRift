...
This iteration builds directly on the weakest‑aspect assessment in [`WEAKNESS_AND_HARDEST_PROBLEM_REPORT.md`](../../archive/assessments/WEAKNESS_AND_HARDEST_PROBLEM_REPORT.md), which identifies **rules UX and onboarding** as RingRift’s weakest area, and on the rules‑UX system described in [`UX_RULES_IMPROVEMENT_LOOP.md`](../UX_RULES_IMPROVEMENT_LOOP.md). It also assumes the concepts vocabulary in [`UX_RULES_CONCEPTS_INDEX.md`](../UX_RULES_CONCEPTS_INDEX.md) and the structured game‑end payload model [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) are the canonical reference points for rules concepts and explanations.

Iteration 0002 is explicitly **hotspot‑driven**. It assumes the existence of rules‑UX telemetry aggregates conforming to [`UX_RULES_TELEMETRY_SPEC.md`](../UX_RULES_TELEMETRY_SPEC.md) and pre‑aggregated hotspot snapshots for Square‑8 2‑player games produced by the analyzer [`scripts/analyze_rules_ux_telemetry.ts`](../../../scripts/analyze_rules_ux_telemetry.ts). The goal is to turn those hotspots into concrete changes to game‑end explanations (HUD + VictoryModal), teaching flows, and (optionally) sandbox/debug UX, without redefining underlying rules semantics.

Compared to Iteration 0001 ([`UX_RULES_IMPROVEMENT_ITERATION_0001.md`](UX_RULES_IMPROVEMENT_ITERATION_0001.md)), which primarily focused on **copy routing and teaching flows**, Iteration 0002 treats [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) as the central contract for _“Why did the game end?”_ and scopes changes around using that model consistently for a small set of high‑confusion concepts.
...
- A **pre‑aggregated telemetry JSON snapshot** for Square‑8 2‑player games over a clearly defined window (e.g. the last 28 days), matching the [`RulesUxAggregatesRoot`](../../../src/shared/telemetry/rulesUxHotspotTypes.ts) shape described in the telemetry tooling docs and consumed by [`scripts/analyze_rules_ux_telemetry.ts`](../../../scripts/analyze_rules_ux_telemetry.ts).
- A **hotspot summary JSON** and short Markdown report emitted by [`scripts/analyze_rules_ux_telemetry.ts`](../../../scripts/analyze_rules_ux_telemetry.ts) for the same window. These must, at minimum, expose per‑`rulesContext` metrics for:
...
- The snapshot must be filterable on the following dimensions (even if not all are used in this iteration):
  - `rulesContext` / `rules_concept` (see [`UX_RULES_TELEMETRY_SPEC.md`](../UX_RULES_TELEMETRY_SPEC.md));
  - `source` / `RulesUxSource` (HUD, VictoryModal, TeachingOverlay, sandbox) as defined in [`rulesUxEvents.ts`](../../../src/shared/telemetry/rulesUxEvents.ts);
...
- Canonical high‑risk concepts and linkages from [`UX_RULES_CONCEPTS_INDEX.md`](../UX_RULES_CONCEPTS_INDEX.md), especially:
...
- Teaching flows and scenarios from [`UX_RULES_TEACHING_SCENARIOS.md`](../UX_RULES_TEACHING_SCENARIOS.md), particularly:
...
- Existing TeachingOverlay topics and routing described in [`TeachingOverlay.tsx`](../../../src/client/components/TeachingOverlay.tsx) and implemented metadata in [`teachingScenarios.ts`](../../../src/shared/teaching/teachingScenarios.ts).

### 2.3 Explanation model and weird‑state mapping

- The structured game‑end payload model [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md), including:
...
- Weird‑state reason codes and mappings from [`UX_RULES_WEIRD_STATES_SPEC.md`](../UX_RULES_WEIRD_STATES_SPEC.md), especially:
...
Iteration 0002 focuses on **three** concepts from [`UX_RULES_CONCEPTS_INDEX.md`](../UX_RULES_CONCEPTS_INDEX.md). In each case, the intent is to standardise how [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) represents the concept and how HUD, VictoryModal, TeachingOverlay, sandbox, and telemetry consume that representation.

### 3.1 `anm_fe_core` – Active‑No‑Moves & Forced Elimination

- **Concept id:** `anm_fe_core`.
- **Canonical telemetry contexts:**
  - `rulesContext = 'anm_forced_elimination'` (see [`UX_RULES_TELEMETRY_SPEC.md`](../UX_RULES_TELEMETRY_SPEC.md));
  - `weirdStateType ∈ { 'active-no-moves-movement', 'forced-elimination' }` per [`RulesUxWeirdStateType`](../../../src/shared/telemetry/rulesUxEvents.ts).
- **Rationale for selection:**
  - Historically top source of "I have pieces but no moves and my caps disappeared" complaints (see [`WEAKNESS_AND_HARDEST_PROBLEM_REPORT.md`](../../archive/assessments/WEAKNESS_AND_HARDEST_PROBLEM_REPORT.md) and [`ACTIVE_NO_MOVES_BEHAVIOUR.md`](../../rules/ACTIVE_NO_MOVES_BEHAVIOUR.md)).
...
- **Explanation‑model alignment:**
  - For games where ANM/FE had a material impact on the outcome (e.g. multiple FE sequences on the eventual loser), [`GameEndExplanation.primaryConceptId`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) SHOULD often be set to `'anm_fe_core'` even if the formal `outcomeType` is `ring_elimination` or `last_player_standing`.
...
- **Canonical telemetry contexts:**
  - `rulesContext = 'structural_stalemate'` (see [`UX_RULES_TELEMETRY_SPEC.md`](../UX_RULES_TELEMETRY_SPEC.md));
  - `weirdStateType = 'structural-stalemate'`.
- **Rationale for selection:**
  - Plateau endings resolved via territory → eliminated rings → markers → last actor are intrinsically non‑obvious and have historically been mis‑perceived as draws or bugs (see [`WEAKNESS_AND_HARDEST_PROBLEM_REPORT.md`](../../archive/assessments/WEAKNESS_AND_HARDEST_PROBLEM_REPORT.md)).
  - Iteration 0001 defined copy and teaching flows but did not yet standardise the structured tiebreak representation in [`GameEndExplanation.tiebreakSteps`](../UX_RULES_EXPLANATION_MODEL_SPEC.md).
...
- **Explanation‑model alignment:**
  - For any game that terminates via the canonical global stalemate ladder ([`RR‑CANON R173`](../../../RULES_CANONICAL_SPEC.md)), [`GameEndExplanation.outcomeType`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) SHOULD be `structural_stalemate`, `victoryReasonCode` SHOULD be `victory_structural_stalemate_tiebreak`, and `primaryConceptId` SHOULD be `'structural_stalemate'`.
...
- **Canonical telemetry contexts:**
  - `rulesContext = 'territory_mini_region'` (see [`UX_RULES_TELEMETRY_SPEC.md`](../UX_RULES_TELEMETRY_SPEC.md));
  - `weirdStateType = 'active-no-moves-territory'` when represented via `ANM_TERRITORY_NO_ACTIONS`.
- **Rationale for selection:**
  - Mini‑region scenarios (FAQ Q23) are a concentrated source of “why did I lose my own ring?” confusion, despite solid engine semantics and tests ([`RulesMatrix.Territory.MiniRegion.test.ts`](../../../tests/scenarios/RulesMatrix.Territory.MiniRegion.test.ts)).
...
- **Explanation‑model alignment:**
  - For endings where a mini‑region decision directly shifts the territory or elimination balance in the final scoring window, [`GameEndExplanation.primaryConceptId`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) SHOULD often be `'territory_mini_regions'`, even if the `outcomeType` is standard `territory_control` or `ring_elimination`.
...
For each targeted concept, [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) SHOULD be treated as the **single source of truth** for why the game ended and why a particular player won. HUD and VictoryModal MUST consume this payload rather than re‑deriving explanations from raw `GameResult` or ad‑hoc flags.

#### 4.1.1 Common requirements across concepts

- Introduce or standardise a shared builder such as [`buildGameEndExplanation()`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) that produces a complete [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) instance for every completed game (backend host and sandbox host).
- Ensure the builder:
  ...
  - Populates `rulesContextTags` from the same vocabulary as [`RulesUxContext`](../../../src/shared/telemetry/rulesUxEvents.ts);
  - Fills `weirdStateContext.reasonCodes` with the relevant weird‑state reason codes from [`UX_RULES_WEIRD_STATES_SPEC.md`](../UX_RULES_WEIRD_STATES_SPEC.md) when present;
...
- Expected copy‑key contracts (no literal copy specified):
  ...
  - VictoryModal keys under `uxCopy.detailedSummaryKey` SHOULD follow patterns like `game_end.ring_elimination.with_anm_fe.long` or reuse the FE‑focused explanation keys described in [`UX_RULES_EXPLANATION_MODEL_SPEC.md`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) and [`UX_RULES_WEIRD_STATES_SPEC.md`](../UX_RULES_WEIRD_STATES_SPEC.md).
- `debug` SHOULD include small, QA‑oriented fields such as:
  - counts of FE events per player in the last N turns;
  - whether any ANM/FE invariants from [`ACTIVE_NO_MOVES_BEHAVIOUR.md`](../../rules/ACTIVE_NO_MOVES_BEHAVIOUR.md) were involved.
...
- Expected copy‑key contracts:
  ...
  - VictoryModal explanation under `uxCopy.detailedSummaryKey` SHOULD use a key such as `game_end.structural_stalemate.tiebreak.detailed`, matching the narrative structure outlined in [`UX_RULES_WEIRD_STATES_SPEC.md`](../UX_RULES_WEIRD_STATES_SPEC.md).
- `debug` SHOULD expose the computed ladder inputs (territory counts, eliminated ring totals including rings in hand, marker counts, id of last real actor) for QA to compare against expectations from [`RULES_CANONICAL_SPEC.md`](../../../RULES_CANONICAL_SPEC.md).
...
- Expected copy‑key contracts:
  ...
  - VictoryModal explanation key under `uxCopy.detailedSummaryKey` SHOULD follow the pattern `game_end.territory.mini_region.detailed`, aligning with the worked example in [`UX_RULES_EXPLANATION_MODEL_SPEC.md`](../UX_RULES_EXPLANATION_MODEL_SPEC.md).
- `rulesReferences.rulesDocsLinks` SHOULD always include anchors to FAQ Q23 and the relevant canonical rules sections referenced in [`UX_RULES_CONCEPTS_INDEX.md`](../UX_RULES_CONCEPTS_INDEX.md).

### 4.2 Teaching surfaces (TeachingOverlay and flows)

When a game ends and a [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) is present, TeachingOverlay SHOULD use the explanation payload rather than ad‑hoc heuristics to decide which topics and flows to promote. For Iteration 0002:

- For `primaryConceptId = 'anm_fe_core'`:
  - `teaching.teachingTopics` SHOULD include an ANM/FE‑aligned topic id such as `anm_forced_elimination` (or equivalent, as defined in [`UX_RULES_TEACHING_SCENARIOS.md`](../UX_RULES_TEACHING_SCENARIOS.md)).
...
- For `primaryConceptId = 'structural_stalemate'`:
  ...
  - `teaching.recommendedFlows` SHOULD include `structural_stalemate_intro` (see [`UX_RULES_TEACHING_SCENARIOS.md`](../UX_RULES_TEACHING_SCENARIOS.md)).
...
- In sandbox or replay views where a completed game is shown, expose a developer‑only panel or toggle that renders the final [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) payload alongside the board, especially for curated scenarios covering:
  - ANM/FE loop archetypes for `anm_fe_core`;
  - plateau stalemate test cases for `structural_stalemate`;
  - mini‑region invariants from [`RulesMatrix.Territory.MiniRegion.test.ts`](../../../tests/scenarios/RulesMatrix.Territory.MiniRegion.test.ts) for `territory_mini_regions`.
...
- Whenever the corresponding concept is the **primary driver of the ending**, the final [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) for that game:
...
- For Square‑8 2‑player games, hotspot snapshots produced by [`scripts/analyze_rules_ux_telemetry.ts`](../../../scripts/analyze_rules_ux_telemetry.ts) include **non‑zero** entries for each targeted `rulesContext`.
- For representative post‑iteration windows, events associated with the targeted concepts (help opens, help reopens, resign‑after‑weird‑state) correctly reference the `rulesContext` values and, where applicable, weird‑state reason codes in line with the mappings in [`UX_RULES_TELEMETRY_SPEC.md`](../UX_RULES_TELEMETRY_SPEC.md) and [`UX_RULES_WEIRD_STATES_SPEC.md`](../UX_RULES_WEIRD_STATES_SPEC.md).
...
- For each concept, at least one **named test scenario** or existing test is explicitly documented as the canonical QA hook for validating the new UX behaviour, for example:
  - ANM/FE: invariants in [`test_active_no_moves_movement_forced_elimination_regression.py`](../../../ai-service/tests/invariants/test_active_no_moves_movement_forced_elimination_regression.py) and related ANM suites; any new game‑end UX tests that assert the produced [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) matches expectations in FE‑heavy endings.
  - Structural stalemate: stalemate/plateau scenarios used in engine tests and parity fixtures as listed in [`UX_RULES_CONCEPTS_INDEX.md`](../UX_RULES_CONCEPTS_INDEX.md); end‑to‑end tests that assert `tiebreakSteps` and HUD/VictoryModal explanations align.
  - Territory mini‑regions: [`RulesMatrix.Territory.MiniRegion.test.ts`](../../../tests/scenarios/RulesMatrix.Territory.MiniRegion.test.ts) and helper tests in [`territoryDecisionHelpers.shared.test.ts`](../../../tests/unit/territoryDecisionHelpers.shared.test.ts) extended or wrapped to assert correct `primaryConceptId` and `rulesContextTags` in `GameEndExplanation` for Q23‑style endings.
...
1. **Implement and wire [`buildGameEndExplanation()`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) for targeted concepts**
   - Introduce or finalise a shared builder that produces [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) instances for completed games, with explicit handling for `anm_fe_core`, `structural_stalemate`, and `territory_mini_regions` as defined in §3–§4.
...
2. **Update HUD and VictoryModal to consume [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) for the three concepts**
   - Refactor end‑of‑game UI in [`GameHUD.tsx`](../../../src/client/components/GameHUD.tsx) and [`VictoryModal.tsx`](../../../src/client/components/VictoryModal.tsx) so that:
...
3. **Extend teaching flows and wiring for `anm_fe_core`, `structural_stalemate`, and `territory_mini_regions`**
   - Ensure TeachingOverlay topic selection and recommended flows are driven by `GameEndExplanation.teaching` for these concepts, reusing existing flows (`fe_loop_intro`, `structural_stalemate_intro`, `mini_region_intro`) as defined in [`UX_RULES_TEACHING_SCENARIOS.md`](../UX_RULES_TEACHING_SCENARIOS.md).
...
4. **Align rules‑UX telemetry with `GameEndExplanation` for targeted endings**
   ...
   - Verify that snapshots consumed by [`scripts/analyze_rules_ux_telemetry.ts`](../../../scripts/analyze_rules_ux_telemetry.ts) can distinguish the three targeted contexts via `rulesContext` alone.
...
- **ANM/FE (`anm_fe_core` / `anm_forced_elimination`):**
  - Weird‑state reason codes and copy for ANM/FE were defined in [`UX_RULES_WEIRD_STATES_SPEC.md`](../UX_RULES_WEIRD_STATES_SPEC.md) (RWS‑001, RWS‑004) and wired into HUD banners and TeachingOverlay topics (`teaching.active_no_moves`, `teaching.forced_elimination`) as part of W1/W2.
  - Rules‑UX copy baselines for ANM/FE, including the distinction between **real moves** and forced elimination, were consolidated in [`UX_RULES_COPY_SPEC.md` §2–3, §10](../UX_RULES_COPY_SPEC.md).
  - ANM/FE teaching flows (`fe_loop_intro`) were specified in [`UX_RULES_TEACHING_SCENARIOS.md`](../UX_RULES_TEACHING_SCENARIOS.md) and integrated into TeachingOverlay and sandbox presets in W2/W3.

- **Structural stalemate (`structural_stalemate`):**
  - Canonical stalemate semantics and tiebreak ladder were clarified in [`../../rules/COMPLETE_RULES.md` §13.4](../../rules/COMPLETE_RULES.md) and [`RULES_CANONICAL_SPEC.md` R173](../../../RULES_CANONICAL_SPEC.md).
  - UX copy and weird‑state reason codes for structural stalemate (`STRUCTURAL_STALEMATE_TIEBREAK`) were defined in [`UX_RULES_WEIRD_STATES_SPEC.md` §3.1–3.2](../UX_RULES_WEIRD_STATES_SPEC.md) and wired into HUD/Victory surfaces in W2/W4.
  - Teaching flows for stalemate (`structural_stalemate_intro`) were added to [`UX_RULES_TEACHING_SCENARIOS.md`](../UX_RULES_TEACHING_SCENARIOS.md), with TeachingOverlay routing updated in W3/W4.
...
- **Territory mini‑regions (`territory_mini_regions` / `territory_mini_region`):**
  ...
  - The `territory_mini_region` concept row in the concordance table in [`RULES_DOCS_UX_AUDIT.md` §4](../../supplementary/RULES_DOCS_UX_AUDIT.md) now ties together rules docs, HUD copy, TeachingOverlay topics, and curated scenarios.
  - A dedicated `mini_region_intro` teaching flow was authored in [`UX_RULES_TEACHING_SCENARIOS.md`](../UX_RULES_TEACHING_SCENARIOS.md) and surfaced via TeachingOverlay and sandbox presets during W2/W3.

- **Game‑end explanations and structured model:**
  - The `GameEndExplanation` model and builder pipeline were designed and implemented in W4, as documented in [`UX_RULES_EXPLANATION_MODEL_SPEC.md`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) and validated via tests (`GameEndExplanation.*.test.ts` in `tests/unit`).
...
- **Telemetry and hotspot analysis:**
  - The rules‑UX telemetry envelope and metrics were formalised in [`UX_RULES_TELEMETRY_SPEC.md`](../UX_RULES_TELEMETRY_SPEC.md).
  - The hotspot analyzer CLI [`scripts/analyze_rules_ux_telemetry.ts`](../../../scripts/analyze_rules_ux_telemetry.ts) and its tests (`analyze_rules_ux_telemetry.test.ts`) were implemented as part of W3, providing the concrete mechanism that future runs of Iteration 0002 will use to confirm whether ANM/FE, structural stalemate, and territory mini‑regions remain top hotspots.
...
- the concordance table in [`RULES_DOCS_UX_AUDIT.md`](../../supplementary/RULES_DOCS_UX_AUDIT.md) as the single reference for ensuring any further copy tweaks remain aligned with canonical rules.
  - For each concept, add tests or extend existing ones so that they assert:
    - the produced [`GameEndExplanation`](../UX_RULES_EXPLANATION_MODEL_SPEC.md) matches this spec for the scenario (concept id, rules contexts, weird‑state reason codes, score/tiebreak structure);
...
All of the tasks above SHOULD be tracked as separate Code‑mode subtasks. This iteration spec [`UX_RULES_IMPROVEMENT_ITERATION_0002.md`](UX_RULES_IMPROVEMENT_ITERATION_0002.md) is the **single source of truth** for what “Rules UX Iteration 0002 – Hotspot‑Driven Game‑End Explanations” is meant to achieve; implementation details, scheduling, and test harness choices belong in Code and Orchestrator modes rather than here.
