...
# Optional PyTorch Geometric import for GNN refinement in v5
try:
...
class GNNRefinementLayer(nn.Module):
    """Optional GNN refinement layer for connectivity-aware features."""

    def __init__(self, in_features: int = 128, hidden_features: int = 128, num_layers: int = 2):
        super().__init__()
        if not HAS_PYG:
            self.enabled = False
    ...
    def forward(self, spatial_features: torch.Tensor, edge_index: torch.Tensor | None = None) -> torch.Tensor:
        ...
        num_nodes = H * W
        batch_edges = [edge_index + b * num_nodes for b in range(B)]
...
class RingRiftCNN_v5(nn.Module):
    """V5 architecture: V4 with optional GNN refinement layer."""

    ARCHITECTURE_VERSION = "v5.0.0"

    def __init__(
        self, board_size: int = 19, in_channels: int = 14, global_features: int = 20,
        num_res_blocks: int = 13, num_filters: int = 128, history_length: int = 3,
        policy_size: int | None = None, num_players: int = 4, num_attention_heads: int = 4,
        dropout: float = 0.08, initial_kernel_size: int = 5, use_gnn: bool = False,
    ) -> None:
        super().__init__()
        self.board_size = board_size
        self.num_filters = num_filters
        ...
        self.max_distance = MAX_DIST_SQUARE8 if board_size == 8 else MAX_DIST_SQUARE19
        self.total_in_channels = in_channels * (history_length + 1)
        self.in_channels = self.total_in_channels
        self.policy_size = policy_size or (POLICY_SIZE_8x8 if board_size == 8 else POLICY_SIZE_19x19)

        self.conv1 = nn.Conv2d(self.total_in_channels, num_filters, kernel_size=initial_kernel_size, padding=initial_kernel_size // 2)
        self.bn1 = nn.BatchNorm2d(num_filters)
        self.relu = nn.ReLU()
        self.res_blocks = nn.ModuleList([AttentionResidualBlock(num_filters, num_heads=num_attention_heads, dropout=dropout) for _ in range(num_res_blocks)])

        self.gnn_refinement: GNNRefinementLayer | None = None
        self._edge_index: torch.Tensor | None = None
        if self.use_gnn:
            self.gnn_refinement = GNNRefinementLayer(num_filters, num_filters, 2)
            self._edge_index = self._build_grid_edges(board_size)

        self.value_fc1 = nn.Linear(num_filters + global_features, 256)
        self.value_fc2 = nn.Linear(256, 256)
        self.value_fc3 = nn.Linear(256, num_players)
        self.tanh = nn.Tanh()
        self.dropout = nn.Dropout(dropout)
        self.rank_dist_fc1 = nn.Linear(num_filters + global_features, 256)
        self.rank_dist_fc2 = nn.Linear(256, 256)
        self.rank_dist_fc3 = nn.Linear(256, num_players * num_players)
        self.policy_fc = nn.Linear(num_filters + global_features, self.policy_size)

    def _build_grid_edges(self, board_size: int) -> torch.Tensor:
        edges = []
        for y in range(board_size):
            for x in range(board_size):
                node = y * board_size + x
                if x + 1 < board_size:
                    edges.extend([[node, y * board_size + x + 1], [y * board_size + x + 1, node]])
                if y + 1 < board_size:
                    edges.extend([[node, (y + 1) * board_size + x], [(y + 1) * board_size + x, node]])
        return torch.tensor(edges, dtype=torch.long).t().contiguous()

    def forward(self, x: torch.Tensor, globals_vec: torch.Tensor, return_features: bool = False):
        if x.shape[1] != self.in_channels:
            raise RuntimeError(f"Expected {self.in_channels} channels, got {x.shape[1]}")
        out = self.relu(self.bn1(self.conv1(x)))
        for block in self.res_blocks:
            out = block(out)
        if self.gnn_refinement is not None:
            edge_idx = self._edge_index.to(out.device) if self._edge_index is not None else None
            out = self.gnn_refinement(out, edge_idx)
        ...
        rank_dist = torch.softmax(rank_logits, dim=-1)

        policy = self.policy_fc(combined)
        return (value, policy, rank_dist, combined) if return_features else (value, policy, rank_dist)

    def forward_single(self, feature: np.ndarray, globals_vec: np.ndarray, player_idx: int = 0):
        self.eval()
        with torch.no_grad():
            x = torch.from_numpy(feature[None, ...]).float().to(next(self.parameters()).device)
            g = torch.from_numpy(globals_vec[None, ...]).float().to(next(self.parameters()).device)
