#!/usr/bin/env python
"""Debug chain capture after move 48."""
...
def main():
    ...
    # Replay CPU up to move 48, then check chain captures
    state = create_initial_state(BoardType.SQUARE8, num_players=2)

    for i, m in enumerate(game_dict['moves'][:49]):  # Through move 48
        ...
        if matched:
            if i == 48:
                print(f"Before move 48 (overtaking_capture):")
                print(f"  CPU phase: {state.current_phase.value}")
                print(f"  CPU player: {state.current_player}")
                print(f"  Move: {matched.type.value} from={matched.from_pos} to={matched.to}")
            ...
            if i == 48:
                ...
                # Check chain capture continuations
                if matched.to:
                    cont_info = get_chain_capture_continuation_info_py(
                        state, 2, matched.to
                    )
                    print(f"\n  Chain capture continuation from {matched.to}:")
                    print(f"    must_continue: {cont_info.must_continue}")
                    print(f"    available: {len(cont_info.available_continuations)}")
