...
class IdleDetectionConfig:
    ...
    idle_duration_threshold_seconds: float = 900.0  # 15 minutes
    check_interval_seconds: float = 60.0
    min_nodes_to_keep: int = 2  # Never flag last N nodes as idle


class IdleDetectionLoop(BaseLoop):
    """Background loop that detects idle nodes.

    Monitors GPU utilization and flags nodes that have been
    idle for too long. Used for cost optimization.
    """

    def __init__(
        self,
        get_node_metrics: Callable[[], dict[str, dict[str, Any]]],
        on_idle_detected: Callable[[str, float], Coroutine[Any, Any, None]],
        config: IdleDetectionConfig | None = None,
    ):
        """Initialize idle detection loop.

        Args:
            get_node_metrics: Callback returning node_id -> metrics dict
            on_idle_detected: Async callback when node is idle (node_id, idle_duration)
    ...
    async def _run_once(self) -> None:
        ...
        for node_id, node_metrics in metrics.items():
            gpu_util = node_metrics.get("gpu_utilization", 100)

            # Check if node is currently idle
            is_idle = gpu_util < self.config.gpu_idle_threshold_percent

            if is_idle:
                if node_id not in self._idle_since:
                    self._idle_since[node_id] = now
                    logger.debug(f"[IdleDetection] Node {node_id} became idle (GPU: {gpu_util}%)")
                ...
                if idle_duration >= self.config.idle_duration_threshold_seconds:
                    ...
                    if non_idle_count >= self.config.min_nodes_to_keep:
                        try:
                            await self._on_idle_detected(node_id, idle_duration)
                            self._detected_count += 1
                            logger.info(
                                f"[IdleDetection] Node {node_id} idle for {idle_duration:.0f}s"
                            )
                        except Exception as e:
