#!/usr/bin/env python
"""Debug seed 456 error at move 49."""
...
def main():
    ...
    for i, m in enumerate(game_dict['moves'][45:56], start=45):
        ...
        print(f"  {i:2}: [{m.get('phase','?'):20}] {m['type']:25} player={m['player']} from={from_str:10} to={to_str:10} {cap_str}")
    ...
    for i, m in enumerate(game_dict['moves']):
        ...
        if matched:
            ...
            if i >= 45:
                print(f"  {i:2}: FAIL {move_type_str:25} gpu_phase={gpu_phase:12} cpu_phase={state.current_phase.value:12} cpu_player={state.current_player}")
                print(f"        GPU wants from={from_pos} to={to_pos}")
                print(f"        CPU stacks:")
                for y in range(8):
                    for x in range(8):
