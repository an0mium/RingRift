...
def load_run(results_path: Path) -> Optional[RunSummary]:
  ...
  return RunSummary(
    ...
    descent_vs_mcts=float(cmp_.get("descent_vs_mcts", {}).get("descent_win_rate"))
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description="Summarize Descent vs MCTS experiment results."
  )
  parser.add_argument(
    "--root",
    type=str,
    ...
    required=True,
    help=(
      "Root directory to scan for run_*/results.json. Can be specified "
      "multiple times."
    ),
  )
  parser.add_argument(
