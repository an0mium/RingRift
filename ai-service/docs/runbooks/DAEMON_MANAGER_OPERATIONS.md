...
The `DaemonManager` coordinates 120 daemon types (78 active, 11 deprecated) in the RingRift training pipeline. It handles lifecycle management, health monitoring, and auto-restart with exponential backoff. For the full list, see `../DAEMON_REGISTRY.md`.
...
All 120 daemon types are defined in `app/coordination/daemon_registry.py`:
