#!/usr/bin/env python3
"""Unified AI Self-Improvement Loop (legacy entrypoint).

By default this script redirects to `scripts/master_loop.py`. Set
`RINGRIFT_UNIFIED_LOOP_LEGACY=1` to run the legacy unified loop.

This legacy daemon integrates all components of the AI improvement loop:
...
Usage:
    # Start the canonical loop
    python scripts/master_loop.py

    # Legacy loop (explicit opt-in)
    RINGRIFT_UNIFIED_LOOP_LEGACY=1 python scripts/unified_ai_loop.py --start

    # Run in foreground with verbose output (legacy)
    RINGRIFT_UNIFIED_LOOP_LEGACY=1 python scripts/unified_ai_loop.py --foreground --verbose

    # Check status (legacy)
    RINGRIFT_UNIFIED_LOOP_LEGACY=1 python scripts/unified_ai_loop.py --status

    # Stop gracefully (legacy)
    RINGRIFT_UNIFIED_LOOP_LEGACY=1 python scripts/unified_ai_loop.py --stop

    # Use custom config (legacy)
    RINGRIFT_UNIFIED_LOOP_LEGACY=1 python scripts/unified_ai_loop.py --config config/unified_loop.yaml
"""
...
if __name__ == "__main__":
    ...
    if os.environ.get("RINGRIFT_UNIFIED_LOOP_LEGACY", "").lower() not in {
        "1",
        "true",
        "yes",
        "on",
        ...
        print(
            "[UnifiedAILoop] Deprecated entrypoint. "
            "Redirecting to master_loop.py. "
            "Set RINGRIFT_UNIFIED_LOOP_LEGACY=1 to run legacy mode."
        )
        target = Path(__file__).with_name("master_loop.py")
...
try:
    from app.integration.unified_loop_extensions import (
        ...
        get_extension_config_from_yaml,
    ...
    get_extension_config_from_yaml = None
...
class LoopHealthStatus(Enum):
    ...
    Note: This is script-local for the unified AI loop.
...
class ComponentHealth:
    ...
    name: str
    status: LoopHealthStatus = LoopHealthStatus.UNKNOWN
    ...
    def record_success(self):
        ...
        self.status = LoopHealthStatus.HEALTHY
    ...
    def record_failure(self, error: str = ""):
        ...
        if self.consecutive_failures >= 3:
            self.status = LoopHealthStatus.UNHEALTHY
        else:
            self.status = LoopHealthStatus.DEGRADED

    def check_staleness(self) -> LoopHealthStatus:
        """Check if component is stale (no recent activity)."""
        if self.last_success == 0:
            return LoopHealthStatus.UNKNOWN
        age = time.time() - self.last_success
        if age > self.stale_threshold_seconds * 2:
            return LoopHealthStatus.UNHEALTHY
        elif age > self.stale_threshold_seconds:
            return LoopHealthStatus.DEGRADED
        return LoopHealthStatus.HEALTHY

    def get_status(self) -> LoopHealthStatus:
        """Get current health status (considers staleness)."""
        staleness = self.check_staleness()
        if self.status == LoopHealthStatus.UNHEALTHY or staleness == LoopHealthStatus.UNHEALTHY:
            return LoopHealthStatus.UNHEALTHY
        if self.status == LoopHealthStatus.DEGRADED or staleness == LoopHealthStatus.DEGRADED:
            return LoopHealthStatus.DEGRADED
        if self.status == LoopHealthStatus.HEALTHY and staleness == LoopHealthStatus.HEALTHY:
            return LoopHealthStatus.HEALTHY
        return LoopHealthStatus.UNKNOWN
...
class HealthTracker:
    ...
    def _update_metrics(self, component: str):
        ...
        COMPONENT_HEALTH.labels(component=component).set(
            1.0 if status == LoopHealthStatus.HEALTHY else
            0.5 if status == LoopHealthStatus.DEGRADED else 0.0
        )
    ...
    def get_overall_status(self) -> LoopHealthStatus:
        """Get overall system health status."""
        statuses = [c.get_status() for c in self.components.values()]
        if any(s == LoopHealthStatus.UNHEALTHY for s in statuses):
            return LoopHealthStatus.UNHEALTHY
        if any(s == LoopHealthStatus.DEGRADED for s in statuses):
            return LoopHealthStatus.DEGRADED
        if all(s == LoopHealthStatus.HEALTHY for s in statuses):
            return LoopHealthStatus.HEALTHY
        return LoopHealthStatus.UNKNOWN
    ...
    def get_health_summary(self) -> dict[str, Any]:
        ...
        if HAS_PROMETHEUS:
            OVERALL_HEALTH.set(
                1.0 if overall == LoopHealthStatus.HEALTHY else
                0.5 if overall == LoopHealthStatus.DEGRADED else 0.0
            )
        return {
...
class UnifiedAILoop:
    ...
    def __init__(self, config: UnifiedLoopConfig, config_path: Path | None = None):
        self.config = config
        ...
        if HAS_LOOP_EXTENSIONS and integrate_extensions is not None:
            try:
                ...
                if get_extension_config_from_yaml is not None:
                    config_path = self._config_path or (AI_SERVICE_ROOT / "config" / "unified_loop.yaml")
                    extensions_config = get_extension_config_from_yaml(Path(config_path))
                self.extensions = integrate_extensions(self, extensions_config)
    ...
    def _update_metrics(self):
        ...
        # Update selfplay queue size metric for pipeline monitoring
    ...
    async def _spawn_selfplay_on_idle_hosts(self) -> int:
        ...
        async def check_host(name: str, ssh_target: str, cpus: int) -> tuple[str, str, int, int]:
            ...
            except (asyncio.TimeoutError, subprocess.SubprocessError, ValueError, UnicodeDecodeError):
                    ...
                    if synced_dir.exists():
                        for db_path in synced_dir.rglob("*.db"):
                            ...
                            try:
                                ...
                                conn.close()
                                current_count += count
                            except (sqlite3.Error, FileNotFoundError, TypeError):
                                pass
        ...
        while self._running:
            try:
                ...
                if result:
                    ...
                    # Record pipeline iteration for throughput tracking
...
def main():
    ...
    if args.start or args.foreground:
        ...
        loop = UnifiedAILoop(config, config_path=config_path)
