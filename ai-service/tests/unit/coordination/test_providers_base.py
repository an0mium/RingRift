...
class TestGPUType:
    ...
    def test_from_string_h100(self):
        ...
        assert GPUType.from_string("NVIDIA H100") == GPUType.H100_80GB
    ...
    def test_from_string_a100_80gb(self):
        ...
        assert GPUType.from_string("A100_80GB") == GPUType.A100_80GB
    ...
    def test_from_string_a100_40gb(self):
        ...
        assert GPUType.from_string("A100") == GPUType.A100_40GB
        assert GPUType.from_string("a100-40gb") == GPUType.A100_40GB
    ...
    def test_from_string_rtx_3090(self):
        ...
        assert GPUType.from_string("GeForce RTX 3090") == GPUType.RTX_3090
    ...
    def test_from_string_case_insensitive(self):
        ...
        assert GPUType.from_string("RTX4090") == GPUType.from_string("rtx4090")
...
class TestInstance:
    ...
    def test_instance_defaults(self):
        ...
        assert instance.cost_per_hour == 0.0
        assert instance.region == ""
        assert instance.tags == {}
        assert instance.raw_data == {}

    def test_instance_full_creation(self):
        ...
        instance = Instance(
            ...
            region="us-east-1",
            tags={"env": "prod"},
            raw_data={"provider_id": "abc123"},
        )
        assert instance.gpu_count == 2
        assert instance.gpu_memory_gb == 160.0
        ...
        assert instance.cost_per_hour == 3.50
        assert instance.region == "us-east-1"
        assert instance.tags == {"env": "prod"}
        assert instance.raw_data["provider_id"] == "abc123"
    ...
    def test_is_running_false_not_running(self):
        ...
        for status in [InstanceStatus.PENDING, InstanceStatus.STOPPED, InstanceStatus.TERMINATED]:
            instance = Instance(
                id="inst-123",
                provider=ProviderType.VAST,
                name="test",
    ...
    def test_ssh_host_default_user(self):
        """Test ssh_host with default root user."""
        instance = Instance(
            id="inst-123",
            provider=ProviderType.VAST,
            name="test",
        ...
        assert instance.ssh_host == "root@10.0.0.1"
...
class TestCloudProvider:
    ...
    async def test_get_instance_status(self, provider_with_instances: MockCloudProvider):
        ...
        status = await provider_with_instances.get_instance_status("inst-3")
    ...
    async def test_scale_up(self, provider: MockCloudProvider):
        ...
        assert all(i.status == InstanceStatus.PENDING for i in instances)

        # Verify they're in provider's instance list
        all_instances = await provider.list_instances()
        assert len(all_instances) == 2

    @pytest.mark.asyncio
    async def test_scale_up_with_prefix(self, provider: MockCloudProvider):
        """Test scale_up with custom name prefix."""
        instances = await provider.scale_up(
            GPUType.H100_80GB,
            count=1,
            name_prefix="custom",
        )
        assert instances[0].name.startswith("custom")

    @pytest.mark.asyncio
    async def test_scale_down(self, provider_with_instances: MockCloudProvider):
        """Test scale_down terminates instances."""
        results = await provider_with_instances.scale_down(["inst-1", "inst-2"])
        assert results["inst-1"] is True
        assert results["inst-2"] is True

        # Verify status changed
        inst = await provider_with_instances.get_instance("inst-1")
        assert inst.status == InstanceStatus.TERMINATED

    @pytest.mark.asyncio
    async def test_scale_down_nonexistent(self, provider_with_instances: MockCloudProvider):
        """Test scale_down with nonexistent instance."""
        results = await provider_with_instances.scale_down(["nonexistent"])
    ...
    def test_get_cost_per_hour(self, provider: MockCloudProvider):
        ...
        assert provider.get_cost_per_hour(GPUType.H100_80GB) == 3.00
    ...
    async def test_get_available_gpus(self, provider_with_instances: MockCloudProvider):
        ...
        assert gpus[GPUType.H100_80GB] == 2  # 2 GPUs on inst-2
        assert GPUType.RTX_3090 not in gpus  # inst-3 is stopped

    @pytest.mark.asyncio
    async def test_get_available_gpus_empty(self, provider: MockCloudProvider):
        """Test get_available_gpus with no instances."""
    ...
    async def test_get_total_cost_per_hour(self, provider_with_instances: MockCloudProvider):
        ...
        # inst-1: 0.80, inst-2: 6.00, inst-3: stopped (doesn't count)
        assert total == 6.80

    @pytest.mark.asyncio
    async def test_get_total_cost_per_hour_empty(self, provider: MockCloudProvider):
        """Test get_total_cost_per_hour with no instances."""
        total = await provider.get_total_cost_per_hour()
        assert total == 0.0
    ...
    @pytest.mark.asyncio
    async def test_health_check_ssh_failure(self, provider_with_instances: MockCloudProvider):
        """Test health_check handles SSH failures."""
        instance = await provider_with_instances.get_instance("inst-1")
    ...
    @pytest.mark.asyncio
    async def test_health_check_ssh_success(self, provider_with_instances: MockCloudProvider):
        """Test health_check succeeds with SSH."""
        instance = await provider_with_instances.get_instance("inst-1")
