...
# territoryFeEdgeRegionForPlayer1 from tests/fixtures/territoryFeEdgeFixture.ts.
...
class TestTerritoryFeEdgeFixture:
    ...
    def test_self_elimination_prerequisite_depends_on_stacks_outside_region(
        ...
        # The fixture must have at least one P1 stack outside the region for the
        # self-elimination prerequisite to be meaningful.
        assert outside_keys, (
            "Expected at least one P1 stack outside the curated region"
        )

        # With the original board, the region should satisfy the self-elimination
        # prerequisite.
    ...
    def test_internal_eliminations_and_credit_on_k89_edge_fixture(
        ...
        for pos in region.spaces:
            ...
            assert owner == 1, f"Expected collapsed owner 1 at {key}, got {owner!r}"
        ...
        assert after_p1_elims == before_p1_elims + internal_height, (
            "Expected all internal rings from (6,1) to be credited to player 1; "
        ...
        assert after_total_elims == before_total_elims + internal_height, (
            "Expected total_rings_eliminated to increase by the internal stack "
            f"height; before={before_total_elims}, internal_height={internal_height}, "
...
if __name__ == "__main__":  # pragma: no cover - manual debug helper
    ...
    #   cd ai-service && python -m pytest tests/test_territory_fe_edge_fixture.py -q
