...
> - Not a rules or lifecycle single source of truth. Rules semantics SSoT is [`RULES_CANONICAL_SPEC.md`](../../RULES_CANONICAL_SPEC.md), with the shared TS engine under `src/shared/engine/**` serving as the primary executable implementation of that spec as catalogued in [`CANONICAL_ENGINE_API.md`](../../docs/architecture/CANONICAL_ENGINE_API.md) and [`MODULE_RESPONSIBILITIES.md`](../../docs/architecture/MODULE_RESPONSIBILITIES.md). Lifecycle and WebSocket contracts are owned by shared types and schemas under `src/shared/types/**` and `src/shared/validation/**`.
> - Orchestrator rollout, SLOs, and incident posture are owned by [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../../docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md) and [`STRICT_INVARIANT_SOAKS.md`](../../docs/testing/STRICT_INVARIANT_SOAKS.md).
...
- [`GamePage.tsx`](./pages/GamePage.tsx) – routing shell that selects the correct host.
- [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx) – online and spectate games (backend WebSocket‑driven).
- [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx) – local sandbox games (client‑side engine).
- Context providers:
  - [`GameContext.tsx`](./contexts/GameContext.tsx)
  - [`SandboxContext.tsx`](./contexts/SandboxContext.tsx)
...
- Bridge between domain models and view models:
  - Translate `GameState`, connection state, and diagnostics into `HUDViewModel`, `BoardViewModel`, `EventLogViewModel`, and `VictoryViewModel` via [`gameViewModels.ts`](./adapters/gameViewModels.ts).
...
- [`BoardView.tsx`](./components/BoardView.tsx)
- [`GameHUD.tsx`](./components/GameHUD.tsx)
- [`GameEventLog.tsx`](./components/GameEventLog.tsx)
- [`VictoryModal.tsx`](./components/VictoryModal.tsx)
- [`AIDebugView.tsx`](./components/AIDebugView.tsx)
- [`GameHistoryPanel.tsx`](./components/GameHistoryPanel.tsx)
...
- View‑model adapters in [`gameViewModels.ts`](./adapters/gameViewModels.ts):
  - `toHUDViewModel`, `toBoardViewModel`, `toEventLogViewModel`, `toVictoryViewModel`.
- Game hooks:
  - [`useGameState.ts`](./hooks/useGameState.ts) – read‑only access to game state and view models.
  - [`useGameConnection.ts`](./hooks/useGameConnection.ts) – connection status and heartbeat metadata.
  - [`useGameActions.ts`](./hooks/useGameActions.ts) – `submitMove`, `submitChoice`, `sendChatMessage`, and `validMoves`.
- Sandbox hooks:
  - [`useSandboxInteractions.ts`](./hooks/useSandboxInteractions.ts) – interprets board input events and forwards them to `ClientSandboxEngine` via `SandboxContext`.
...
| ----------------------------------------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx)        | Host           | Online/spectate host, backend connection lifecycle, mapping backend game state to view models, wiring board and HUD interactions to `submitMove` and `submitChoice`                      | Direct engine/orchestrator imports; rules logic; manual WebSocket usage                                               |
| [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx)        | Host           | Sandbox configuration, `ClientSandboxEngine` lifecycle via `SandboxContext`, sandbox diagnostics, mapping sandbox state to view models and sandbox‑specific HUD                          | Local rules logic; calling shared engine helpers directly; bypassing `ClientSandboxEngine`                            |
| [`BoardView.tsx`](./components/BoardView.tsx)               | Presentational | Render board from `BoardViewModel` and low‑level props; handle click/keyboard events; call host callbacks                                                                                | Talking to backend or sandbox; computing legal moves or outcomes                                                      |
| [`GameHUD.tsx`](./components/GameHUD.tsx)                   | Presentational | Render HUD from `HUDViewModel` (or legacy props); show phase/turn, connection and spectator status, per‑player ring/territory stats, AI profile info, timers, and decision‑phase banners | Inspecting or mutating `GameState` outside view models; owning connection or engine lifecycle                         |
| [`GameEventLog.tsx`](./components/GameEventLog.tsx)         | Presentational | Render event log from `EventLogViewModel`; optional replay callbacks                                                                                                                     | Parsing engine history directly; querying backend                                                                     |
| [`AIDebugView.tsx`](./components/AIDebugView.tsx)           | Presentational | Render AI diagnostics given derived props from hosts                                                                                                                                     | Direct AI service calls; orchestrator or engine imports                                                               |
| [`GameHistoryPanel.tsx`](./components/GameHistoryPanel.tsx) | Presentational | Render per‑turn summaries, allow navigation via host callbacks                                                                                                                           | Owning game/session lifecycle; direct context access                                                                  |
| [`ReplayPanel`](./components/ReplayPanel/ReplayPanel.tsx)   | Presentational | Browse stored games via replay service hooks, control playback (step/play/speed), surface current move info, and expose fork/close callbacks to hosts                                    | Talking directly to WebSockets or orchestrator; mutating live `GameState`; owning sandbox or backend engine lifecycle |

**Allowed dependencies by category:**

- Hosts:
  - May depend on contexts and hooks (`useGame`, `useSandbox`, `useAuth`), routing, and domain services such as [`GameAPI`](./domain/GameAPI.ts).
  - May import view‑model adapters and shared types.
- Presentational components:
  - May depend on props, view‑model types, and UI primitives under [`components/ui`](./components/ui/Button.tsx).
...
- Backend host stack:
  - WebSocket API → backend `GameEngine` → [`TurnEngineAdapter.ts`](../server/game/turn/TurnEngineAdapter.ts) → shared orchestrator and aggregates.
- Sandbox host stack:
  - [`ClientSandboxEngine.ts`](./sandbox/ClientSandboxEngine.ts) → [`SandboxOrchestratorAdapter.ts`](./sandbox/SandboxOrchestratorAdapter.ts) → shared orchestrator and aggregates.

The canonical orchestrator API and decision model are documented in [`CANONICAL_ENGINE_API.md`](../../docs/architecture/CANONICAL_ENGINE_API.md). Orchestrator rollout phases, SLOs, and rollback levers are defined in [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../../docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md) and [`STRICT_INVARIANT_SOAKS.md`](../../docs/testing/STRICT_INVARIANT_SOAKS.md).

### 3.1 Backend game flow (online and spectate)

The typical lifecycle for a user action in a backend game is:

1. The user clicks or taps on the board in [`BoardView.tsx`](./components/BoardView.tsx).
2. `BoardView` calls an `onCellClick` or keyboard handler provided by [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx).
3. `BackendGameHost` uses `validMoves` and other data from `useGameActions()` / `useGameState()` to:
   ...
   - Call `submitMove` or `submitChoice` from [`useGameActions.ts`](./hooks/useGameActions.ts).
4. [`GameContext.tsx`](./contexts/GameContext.tsx) serializes the move into a WebSocket `player_move` / `player_move_by_id` message using shared `Move` and WebSocket types.
5. The backend `GameEngine` receives the message and delegates to [`TurnEngineAdapter.ts`](../server/game/turn/TurnEngineAdapter.ts), which calls the shared orchestrator (`processTurnAsync`) and aggregates.
...
8. `BackendGameHost` recomputes view models via [`gameViewModels.ts`](./adapters/gameViewModels.ts) and re‑renders `BoardView`, `GameHUD`, `GameEventLog`, and `VictoryModal`.
...
2. Event handlers provided by [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx) are implemented using [`useSandboxInteractions.ts`](./hooks/useSandboxInteractions.ts).
3. `useSandboxInteractions` reads `sandboxEngine` and sandbox configuration from [`SandboxContext.tsx`](./contexts/SandboxContext.tsx) and:
...
4. [`ClientSandboxEngine.ts`](./sandbox/ClientSandboxEngine.ts) uses [`SandboxOrchestratorAdapter.ts`](./sandbox/SandboxOrchestratorAdapter.ts) (when `useOrchestratorAdapter` is enabled) to:
...
Because orchestrator behaviour and invariants are guarded by CI and soak SLOs (see [`ORCHESTRATOR_ROLLOUT_PLAN.md`](../../docs/architecture/ORCHESTRATOR_ROLLOUT_PLAN.md) and [`STRICT_INVARIANT_SOAKS.md`](../../docs/testing/STRICT_INVARIANT_SOAKS.md)), frontend code must adhere to the following constraints:
...
### 4.1 [`GamePage.tsx`](./pages/GamePage.tsx)
...
### 4.2 [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx)
...
- **Mapping backend state into view models:**
  - Use adapters in [`gameViewModels.ts`](./adapters/gameViewModels.ts) to compute:
...
- **Driving spectate and player UX:**
  - Respect current user identity from [`AuthContext.tsx`](./contexts/AuthContext.tsx) to determine if the user is a player or spectator.
...
### 4.3 [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx)

**Allowed responsibilities:**

- **Sandbox configuration and lifecycle:**
  - Manage sandbox configuration state (board type, player count, AI/human seats).
  - Attempt to create real backend sandbox games via [`GameAPI`](./domain/GameAPI.ts) when desired, and gracefully fall back to local sandbox.
  - Use [`SandboxContext.tsx`](./contexts/SandboxContext.tsx) to initialise and reset `ClientSandboxEngine`.
...
- **Board and HUD interaction orchestration:**
  - Use [`useSandboxInteractions.ts`](./hooks/useSandboxInteractions.ts) to wire `BoardView` input events into sandbox engine actions.
...
### 4.4 [`GameContext.tsx`](./contexts/GameContext.tsx)
...
### 4.5 [`SandboxContext.tsx`](./contexts/SandboxContext.tsx)
...
### 4.6 [`AuthContext.tsx`](./contexts/AuthContext.tsx)
...
2. Extend **hosts and adapters**, not presentational components, with:
   ...
   - New view‑model fields in [`gameViewModels.ts`](./adapters/gameViewModels.ts) when presentation needs additional derived data.
...
1. In [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx):
...
- Remaining legacy or diagnostics‑only rules helpers are tracked and fenced by:
  - [`WEAKNESS_ASSESSMENT_REPORT.md`](../../docs/archive/plans/WEAKNESS_ASSESSMENT_REPORT.md)
  - [`PASS16_ASSESSMENT_REPORT.md`](../../docs/archive/assessments/PASS16_ASSESSMENT_REPORT.md)
...
- **View models** are defined and built in [`gameViewModels.ts`](./adapters/gameViewModels.ts), transforming `GameState` into UI‑friendly shapes.
- **Custom hooks** in [`src/client/hooks`](./hooks/index.ts) provide focused access to game state, connection state, and actions.
- **Contexts** under [`src/client/contexts`](./contexts/GameContext.tsx) own connections and engine instances.
- **Domain services** such as `ClientSandboxEngine` and `GameAPI` live under [`src/client/sandbox`](./sandbox/ClientSandboxEngine.ts) and [`src/client/domain`](./domain/GameAPI.ts) respectively.
...
- Keep the shared engine and orchestrator (for example [`turnOrchestrator.ts`](../shared/engine/orchestration/turnOrchestrator.ts)) as the **only rules SSOT**, consistent with [`RULES_SSOT_MAP.md`](../../docs/rules/RULES_SSOT_MAP.md).
...
- Provide a migration target for existing hosts such as [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx) and [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx) without changing behaviour.
...
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RouteShell**          | [`GamePage.tsx`](./pages/GamePage.tsx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Route params, simple layout flags                                                                                                                                          | React Router `useParams`, `gameId` and mode (backend vs sandbox)                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Game or sandbox lifecycle, engine calls, WebSockets, orchestrator helpers                                                                                                                                                                                                         |
| **ConnectionShell**     | Backend: connection portion of [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx) using [`useGameConnection.ts`](./hooks/useGameConnection.ts). Sandbox: setup and session wiring in [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx) using [`SandboxContext.tsx`](./contexts/SandboxContext.tsx).                                                                                                                                                                                                                           | Connection status banners, loading shells, backend error strings                                                                                                           | `useGameConnection()` and `useConnectionStatus()` from [`useGameConnection.ts`](./hooks/useGameConnection.ts), backend `connectToGame` and `disconnect` from [`GameContext.tsx`](./contexts/GameContext.tsx); sandbox config and engine lifecycle (`initLocalSandboxEngine`, `resetSandboxEngine`) from [`SandboxContext.tsx`](./contexts/SandboxContext.tsx); backend game creation via [`GameAPI`](./domain/GameAPI.ts)                                                                                                    | Inspecting or mutating board or rules state; calling [`ClientSandboxEngine`](./sandbox/ClientSandboxEngine.ts) methods directly; importing orchestrator helpers such as [`turnOrchestrator.ts`](../shared/engine/orchestration/turnOrchestrator.ts)                               |
| **GameStateController** | Backend: controller logic in [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx) (selection, `validMoves` interpretation, view‑model construction) plus focused hooks such as [`useGameState.ts`](./hooks/useGameState.ts) and [`useGameActions.ts`](./hooks/useGameActions.ts). Sandbox: controller logic in [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx) together with [`useSandboxInteractions.ts`](./hooks/useSandboxInteractions.ts).                                                                                | Board selection and `validTargets`, per‑host overlays (controls help, diagnostics toggles), local countdowns and timers, chat input buffer (where not owned by DecisionUI) | Read‑only game data via [`useGameState()`](./hooks/useGameState.ts); `validMoves` and helper APIs via [`useValidMoves()`](./hooks/useGameActions.ts); actions via [`useGameActions()`](./hooks/useGameActions.ts); sandbox state via [`useSandbox()`](./contexts/SandboxContext.tsx) and interaction handlers from [`useSandboxInteractions()`](./hooks/useSandboxInteractions.ts); view‑model adapters in [`gameViewModels.ts`](./adapters/gameViewModels.ts); identity via [`AuthContext.tsx`](./contexts/AuthContext.tsx) | Direct imports of shared engine or orchestrator modules; custom legality logic (movement, capture, territory, victory); direct WebSocket usage; direct imports of sandbox helpers like [`sandboxMovement.ts`](./sandbox/sandboxMovement.ts) from React code                       |
| **DecisionUI**          | Backend: choice dialog, instructions, victory modal, and chat wiring in [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx). Sandbox: `ChoiceDialog` and capture‑direction wiring in [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx) plus decision portions of [`useSandboxInteractions.ts`](./hooks/useSandboxInteractions.ts).                                                                                                                                                                                             | Choice dialog visibility and timers, victory modal dismissal, ephemeral UI state around chat input                                                                         | `pendingChoice` and `choiceDeadline` via [`usePendingChoice()`](./hooks/useGameActions.ts) or host‑level sandbox choice state; `respondToChoice` and `sendChatMessage` via [`useGameActions()`](./hooks/useGameActions.ts); victory and HUD view models via [`gameViewModels.ts`](./adapters/gameViewModels.ts); sandbox choices and capture targets via [`SandboxContext.tsx`](./contexts/SandboxContext.tsx)                                                                                                               | Subscribing directly to connection lifecycle; owning engine instances; constructing or mutating `GameState` directly; computing `validMoves`                                                                                                                                      |
| **BoardAndHUDLayout**   | Composition sections at the bottom of [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx) and [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx); layout wrappers around [`BoardView.tsx`](./components/BoardView.tsx), [`GameHUD.tsx`](./components/GameHUD.tsx), [`GameEventLog.tsx`](./components/GameEventLog.tsx), [`VictoryModal.tsx`](./components/VictoryModal.tsx), [`SandboxTouchControlsPanel.tsx`](./components/SandboxTouchControlsPanel.tsx), [`BoardControlsOverlay.tsx`](./components/BoardControlsOverlay.tsx) | Pure UI state only (tab selection, collapsed sections), no domain state                                                                                                    | Typed props and view models (`BoardViewModel`, `HUDViewModel`, `EventLogViewModel`, `VictoryViewModel`) from [`gameViewModels.ts`](./adapters/gameViewModels.ts); callbacks provided by GameStateController and DecisionUI                                                                                                                                                                                                                                                                                                   | Accessing React contexts such as `useGame`, `useSandbox`, or `useAuth`; importing [`ClientSandboxEngine`](./sandbox/ClientSandboxEngine.ts), [`SandboxOrchestratorAdapter`](./sandbox/SandboxOrchestratorAdapter.ts), or backend `GameEngine`; computing legal moves or decisions |
| **DiagnosticsPanel**    | Backend: event log and connection‑status watcher inside [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx). Sandbox: stall watchdog and AI trace controls in [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx) and diagnostics fields in [`SandboxContext.tsx`](./contexts/SandboxContext.tsx). Presentational panels such as [`AIDebugView.tsx`](./components/AIDebugView.tsx).                                                                                                                                              | Diagnostics event log, toggles for “moves only” vs “moves + system”, stall warnings, flags for trace export or advanced panels                                             | Host‑exposed diagnostics from [`GameContext.tsx`](./contexts/GameContext.tsx) (connection status, errors, last heartbeat); sandbox diagnostics from [`SandboxContext.tsx`](./contexts/SandboxContext.tsx) (stall watcher, trace buffer handle); read‑only helpers that operate on cloned boards, such as functions from [`sandboxCaptureSearch.ts`](./sandbox/sandboxCaptureSearch.ts) invoked in non‑UI tooling                                                                                                             | Driving canonical legality or victory decisions; mutating live `GameState` outside of `ClientSandboxEngine`; calling orchestrator or aggregates directly; being treated as a rules SSOT                                                                                           |
...
- Only **ConnectionShell** and **GameStateController** roles are allowed to talk to host engines and contexts:
  - Backend: via [`GameContext.tsx`](./contexts/GameContext.tsx) and hooks such as [`useGameState.ts`](./hooks/useGameState.ts), [`useGameConnection.ts`](./hooks/useGameConnection.ts), and [`useGameActions.ts`](./hooks/useGameActions.ts).
  - Sandbox: via [`SandboxContext.tsx`](./contexts/SandboxContext.tsx), [`ClientSandboxEngine.ts`](./sandbox/ClientSandboxEngine.ts), and [`SandboxOrchestratorAdapter.ts`](./sandbox/SandboxOrchestratorAdapter.ts) (indirectly).
...
This section refines the SSOT rules from [`RULES_SSOT_MAP.md`](../../docs/rules/RULES_SSOT_MAP.md) for the frontend.

**Rules and orchestrator layer**

- All rules semantics are owned by:
  - Shared engine helpers and aggregates under [`src/shared/engine`](../shared/engine/types.ts).
  - Orchestrator entry points such as [`processTurn()`](../shared/engine/orchestration/turnOrchestrator.ts), [`processTurnAsync()`](../shared/engine/orchestration/turnOrchestrator.ts), [`validateMove()`](../shared/engine/orchestration/turnOrchestrator.ts), [`getValidMoves()`](../shared/engine/orchestration/turnOrchestrator.ts), and [`hasValidMoves()`](../shared/engine/orchestration/turnOrchestrator.ts).
- Frontend React code **must not** import orchestrator or aggregate modules directly. The only allowed orchestrator usage is via:
  - Backend adapter [`TurnEngineAdapter.ts`](../server/game/turn/TurnEngineAdapter.ts) driven by `GameEngine`.
  - Sandbox adapter [`SandboxOrchestratorAdapter.ts`](./sandbox/SandboxOrchestratorAdapter.ts) used by [`ClientSandboxEngine.ts`](./sandbox/ClientSandboxEngine.ts).

**Host orchestration layer**

- Backend host stack:
  - WebSocket transport and message handling are encapsulated by [`GameContext.tsx`](./contexts/GameContext.tsx) and [`SocketGameConnection`](./services/GameConnection.ts).
  - Backend rules execution goes through the adapter path documented in [`RULES_SSOT_MAP.md` §3.1](../../docs/rules/RULES_SSOT_MAP.md).
- Sandbox host stack:
  - [`ClientSandboxEngine`](./sandbox/ClientSandboxEngine.ts) owns local `GameState` and calls the orchestrator via [`SandboxOrchestratorAdapter`](./sandbox/SandboxOrchestratorAdapter.ts) when `useOrchestratorAdapter` is enabled.
  - [`SandboxContext.tsx`](./contexts/SandboxContext.tsx) owns engine lifecycle and exposes `getSandboxGameState()` and diagnostics.

**Host / UI boundary rules**

- Only **ConnectionShell** and **GameStateController** roles may:
  - Call `connectToGame()` or `disconnect()` on the backend via [`useGameConnection()`](./hooks/useGameConnection.ts).
  - Call `initLocalSandboxEngine()` and `resetSandboxEngine()` from [`SandboxContext.tsx`](./contexts/SandboxContext.tsx).
  - Access `sandboxEngine` and call methods such as `getGameState()`, `getVictoryResult()`, or `maybeRunAITurn()` on [`ClientSandboxEngine.ts`](./sandbox/ClientSandboxEngine.ts).
- No React component outside host controllers may:
  - Import any helper from `src/shared/engine/**`.
  - Import sandbox helpers such as [`sandboxMovement.ts`](./sandbox/sandboxMovement.ts), [`sandboxCaptures.ts`](./sandbox/sandboxCaptures.ts), or [`sandboxTerritory.ts`](./sandbox/sandboxTerritory.ts) directly.
  - Re‑implement legality logic (`getValidMoves`, capture chains, territory disconnection) or victory checks. These must always come from:
    - Backend `validMoves` and `GameResult` exposed via [`GameContext.tsx`](./contexts/GameContext.tsx).
    - Sandbox `getValidMoves()` and victory helpers inside [`ClientSandboxEngine.ts`](./sandbox/ClientSandboxEngine.ts) and [`sandboxVictory.ts`](./sandbox/sandboxVictory.ts) when used via the engine.
...
These rules extend the anti‑patterns in §6 to the finer‑grained host roles and align with SSOT enforcement tooling under [`scripts/ssot`](../../scripts/ssot/rules-ssot-check.ts).
...
| -------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`sandboxMovement.ts`](./sandbox/sandboxMovement.ts)           | `UX / diagnostics‑only adapter` over shared movement helpers           | May be used inside [`ClientSandboxEngine.ts`](./sandbox/ClientSandboxEngine.ts) and test utilities to enumerate simple movement landings and apply marker‑path effects for highlighting and AI heuristics; may be used in diagnostics tools to visualise reachable cells on a cloned `BoardState` | Must not be used by React components or hooks to compute canonical `validMoves`; must not mutate live `GameState` or `BoardState` originating from contexts                         |
| [`sandboxCaptures.ts`](./sandbox/sandboxCaptures.ts)           | `UX / diagnostics‑only adapter` over shared capture helpers            | May be used by [`ClientSandboxEngine.ts`](./sandbox/ClientSandboxEngine.ts) and offline tools to enumerate capture segments or apply diagnostic capture segments on cloned boards; may support capture‑visualisation overlays in diagnostics panels                                               | Must not define an alternative capture pipeline for live games; must not be imported from components, contexts, or [`useSandboxInteractions.ts`](./hooks/useSandboxInteractions.ts) |
| [`sandboxTerritory.ts`](./sandbox/sandboxTerritory.ts)         | `adapter over canonical territory engine; candidate for consolidation` | Delegates territory detection and processing to shared helpers (for example `applyTerritoryRegion`); may be used by sandbox engines and tests to compute disconnected regions and border markers; may power diagnostics views that visualise regions on snapshots                                 | New semantics must be added in shared territory modules, not here; React code must not call it directly, only via `ClientSandboxEngine` or specialised test utilities               |
| [`sandboxLines.ts`](./sandbox/sandboxLines.ts)                 | `adapter over canonical line detection; candidate for consolidation`   | Uses shared `findAllLines` to expose marker lines for sandbox engines and diagnostics; safe for line visualisation on cloned boards                                                                                                                                                               | Must not introduce bespoke line‑finding rules; React components must not import it directly                                                                                         |
| [`sandboxElimination.ts`](./sandbox/sandboxElimination.ts)     | `adapter with invariants; candidate for consolidation`                 | Provides helpers such as `forceEliminateCapOnBoard()` for engines and tests, enforcing board invariants; may be used by [`ClientSandboxEngine.ts`](./sandbox/ClientSandboxEngine.ts) and parity suites                                                                                            | Must not be called from UI code; elimination semantics must continue to be validated against backend tests and orchestrator contract vectors                                        |
| [`sandboxVictory.ts`](./sandbox/sandboxVictory.ts)             | `adapter over canonical victory helper; candidate for consolidation`   | Delegates victory detection to shared `evaluateVictory` and builds UI‑facing `GameResult` objects for sandbox; suitable for diagnostics dashboards that replay sandbox games                                                                                                                      | Must not diverge from orchestrator victory reasons; any new victory logic belongs in shared engine and orchestrator                                                                 |
| [`sandboxCaptureSearch.ts`](./sandbox/sandboxCaptureSearch.ts) | `diagnostics‑only (analysis tool)`                                     | May be used in unit tests, parity tools, or debug scripts to explore maximal capture chains on cloned boards; may be invoked from CLI or devtools UIs that are clearly marked “Diagnostics”                                                                                                       | Must never be imported into production React components, contexts, or host controllers; must not drive AI policies or any `getValidMoves` surface                                   |

**Key constraint:** all sandbox helpers operate only **on cloned board snapshots** for diagnostics, or as thin adapters inside [`ClientSandboxEngine.ts`](./sandbox/ClientSandboxEngine.ts). They must not be used to define or override canonical behaviour for live games.

### 8.5 Migration Plan from Current Hosts to Target Roles

This section maps current responsibilities in [`GamePage.tsx`](./pages/GamePage.tsx), [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx), [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx), and related hooks/contexts to the target roles.

#### 8.5.1 `GamePage` → RouteShell

Current state:

- [`GamePage.tsx`](./pages/GamePage.tsx) already behaves as a minimal routing shell: it reads `gameId` from the route and chooses between `BackendGameHost` and `SandboxGameHost`.
...
Current responsibilities in [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx) include:
...
- Decision handling:
  - Rendering [`ChoiceDialog`](./components/ChoiceDialog.tsx) and wiring `respondToChoice`.
  - Counting down `choiceTimeRemainingMs` with a timer.
- View‑model computation:
  - `toBoardViewModel`, `toHUDViewModel`, `toEventLogViewModel`, `toVictoryViewModel` from [`gameViewModels.ts`](./adapters/gameViewModels.ts).
...
- **ConnectionShell (backend)**:
  - Extract connection‑focused logic into a small component or hook (for example `useBackendConnectionShell`) that uses [`useGameConnection()`](./hooks/useGameConnection.ts) instead of raw `useGame()`.
  - Responsibilities:
...
- **GameStateController (backend)**:
  - Introduce a controller hook, for example `useBackendGameStateController`, that:
    - Reads `gameState`, `validMoves`, `pendingChoice`, `victoryState` via [`useGameState()`](./hooks/useGameState.ts) and [`useValidMoves()`](./hooks/useGameActions.ts).
    - Owns board selection and `validTargets` state (currently the `selected` and `validTargets` fields).
    - Builds all view models via [`gameViewModels.ts`](./adapters/gameViewModels.ts).
    - Computes interaction copy (`boardInteractionMessage`, `getInstruction`) from phase and identity (via [`AuthContext.tsx`](./contexts/AuthContext.tsx)).
    - Exposes strongly typed callbacks (`onCellClick`, `onCellDoubleClick`, `onCellContextMenu`) that are pure functions of `gameState` and `validMoves` and delegate to [`useGameActions()`](./hooks/useGameActions.ts).
...
- **DecisionUI (backend)**:
  - Move choice timer and `ChoiceDialog` wiring into a focused hook (for example `useBackendDecisionUI`) or a small component nested under the host:
    - Uses `pendingChoice` and `choiceDeadline` from [`usePendingChoice()`](./hooks/useGameActions.ts).
    - Owns `choiceTimeRemainingMs` and the timer ref.
    - Renders [`ChoiceDialog`](./components/ChoiceDialog.tsx) and calls `respondToChoice`.
...
- **BoardAndHUDLayout (backend)**:
  - Factor out a pure layout component (for example `BackendGameLayout`) that:
    ...
    - Composes [`BoardView.tsx`](./components/BoardView.tsx), [`GameHUD.tsx`](./components/GameHUD.tsx), [`GameEventLog.tsx`](./components/GameEventLog.tsx), [`VictoryModal.tsx`](./components/VictoryModal.tsx), chat panel, and [`BoardControlsOverlay.tsx`](./components/BoardControlsOverlay.tsx).
...
- **DiagnosticsPanel (backend)**:
  - Consolidate phase/currentPlayer/choice and connection status logging into a dedicated hook (for example `useGameDiagnosticsLog`) that:
    ...
    - Produces a list of diagnostic strings suitable for [`GameEventLog.tsx`](./components/GameEventLog.tsx).
  - The host passes this diagnostic log into `toEventLogViewModel` and does not mutate it elsewhere.

**Migration steps (backend)**:

1. Introduce `useBackendConnectionShell` and refactor [`BackendGameHost.tsx`](./pages/BackendGameHost.tsx) to consume it instead of using raw connection fields from `useGame()`.
2. Extract board interaction logic into `useBackendBoardInteractions` that implements `onCellClick`, `onCellDoubleClick`, and `onCellContextMenu` using [`useValidMoves()`](./hooks/useGameActions.ts) and [`useGameActions()`](./hooks/useGameActions.ts).
3. Extract decision handling and timers into `useBackendDecisionUI` and update `BackendGameHost` to render [`ChoiceDialog`](./components/ChoiceDialog.tsx) via this hook.
...
5. Move diagnostics logging into `useGameDiagnosticsLog` and pass its output to [`GameEventLog`](./components/GameEventLog.tsx) via `toEventLogViewModel`.

#### 8.5.3 Sandbox path: `SandboxGameHost` decomposition

Current responsibilities in [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx) include:

- Pre‑game setup:
  - Local config (`LocalConfig`) for `boardType`, `numPlayers`, `playerTypes`, and per-seat `aiDifficulties`.
  - Attempted backend sandbox game creation via [`gameApi.createGame`](./services/api.ts) and fallback to local `ClientSandboxEngine`.
- Sandbox engine lifecycle:
  - Calls `initLocalSandboxEngine()` and `resetSandboxEngine()` from [`SandboxContext.tsx`](./contexts/SandboxContext.tsx).
  - Reads `sandboxEngine.getGameState()` and `sandboxEngine.getVictoryResult()`.
- Board interaction orchestration:
  - Delegated to [`useSandboxInteractions.ts`](./hooks/useSandboxInteractions.ts), with `selected` and `validTargets` held in the host.
- Diagnostics:
  - Surfaces `sandboxStallWarning` and `sandboxLastProgressAt` from [`SandboxContext.tsx`](./contexts/SandboxContext.tsx).
  - Provides “Copy AI trace” functionality backed by the `__RINGRIFT_SANDBOX_TRACE__` buffer.
- Layout and HUD:
  - Renders [`BoardView.tsx`](./components/BoardView.tsx), [`SandboxTouchControlsPanel.tsx`](./components/SandboxTouchControlsPanel.tsx), [`GameEventLog.tsx`](./components/GameEventLog.tsx), phase guide, and sandbox notes.
...
- **ConnectionShell (sandbox)**:
  - Treat the pre‑game setup and engine initialisation as the sandbox ConnectionShell:
    ...
    - Call `initLocalSandboxEngine()` with a [`SandboxInteractionHandler`](./sandbox/ClientSandboxEngine.ts) derived from `playerTypes` and `aiDifficulties`.
...
- **GameStateController (sandbox)**:
  - Introduce `useSandboxGameStateController` that:
    - Reads `sandboxGameState` and `sandboxVictoryResult` via [`useSandbox()`](./contexts/SandboxContext.tsx) and `sandboxEngine.getGameState()`.
    - Owns `selected`, `validTargets`, `showMovementGrid`, `showValidTargetsOverlay`, and board controls overlay flags.
    - Uses [`toBoardViewModel`](./adapters/gameViewModels.ts) and [`toVictoryViewModel`](./adapters/gameViewModels.ts) to create view models.
    - Merges capture targets from `sandboxCaptureTargets` with standard `validTargets` for highlighting.

- **DecisionUI (sandbox)**:
  - Continue to use [`ChoiceDialog`](./components/ChoiceDialog.tsx) and [`SandboxTouchControlsPanel.tsx`](./components/SandboxTouchControlsPanel.tsx), but treat them as part of DecisionUI:
...
- **BoardAndHUDLayout (sandbox)**:
  - Factor a `SandboxGameLayout` component that:
    - Receives board and HUD view models, selection state, and all board interaction callbacks (from [`useSandboxInteractions.ts`](./hooks/useSandboxInteractions.ts)).
    - Renders the board, player list, phase guide, diagnostics panels, and overlays.
    - Does not import [`useSandbox`](./contexts/SandboxContext.tsx) or [`ClientSandboxEngine`](./sandbox/ClientSandboxEngine.ts).

- **DiagnosticsPanel (sandbox)**:
  - Move stall watchdog rendering and “Copy AI trace” controls into a dedicated diagnostics component, driven by `sandboxStallWarning`, `sandboxDiagnosticsEnabled`, and `sandboxLastProgressAt` from [`SandboxContext.tsx`](./contexts/SandboxContext.tsx).
  - Any future diagnostics that rely on helpers like [`sandboxCaptureSearch.ts`](./sandbox/sandboxCaptureSearch.ts) must operate on snapshots obtained via `getSandboxGameState()` only and may not influence move legality or turn progression.

**Migration steps (sandbox)**:

1. Encapsulate setup and engine initialisation logic in a `useSandboxConnectionShell` hook that exposes `startSandboxGame`, `resetSandboxGame`, and configuration state, and refactor [`SandboxGameHost.tsx`](./pages/SandboxGameHost.tsx) to use it.
2. Introduce `useSandboxGameStateController` to own selection, view models, and phase copy derived from `sandboxGameState`, and to compose data for [`SandboxTouchControlsPanel.tsx`](./components/SandboxTouchControlsPanel.tsx).
3. Move stall warnings and AI trace export into a `SandboxDiagnosticsPanel` component that consumes diagnostics props from the controller or [`SandboxContext`](./contexts/SandboxContext.tsx).
...
- Prefer **narrow hooks** over `useGame()`:
  - Replace direct `useGame()` usage in hosts with [`useGameState()`](./hooks/useGameState.ts), [`useGameConnection()`](./hooks/useGameConnection.ts), [`useGameActions()`](./hooks/useGameActions.ts), and [`useValidMoves()`](./hooks/useGameActions.ts) wherever practical.
  - Keep [`GameContext`](./contexts/GameContext.tsx) as a non‑visual owner of backend game state and WebSocket lifecycle; do not move host‑local UI state (selection, overlays) into it.
- Consider a thin `useSandboxState()` wrapper for [`SandboxContext`](./contexts/SandboxContext.tsx) that:
  - Returns `sandboxGameState`, `sandboxVictoryResult`, and key diagnostics fields.
  - Helps keep React components out of direct engine calls except via well‑defined hooks such as [`useSandboxInteractions.ts`](./hooks/useSandboxInteractions.ts).
...
- Unit test [`useGameConnection()`](./hooks/useGameConnection.ts) and any `useBackendConnectionShell` or `useSandboxConnectionShell` wrappers by:
  - Providing a mocked [`GameContext`](./contexts/GameContext.tsx) or [`SandboxContext`](./contexts/SandboxContext.tsx).
...
- For backend controllers:
  - Mount components under a test `GameProvider` with a fake [`GameConnection`](./domain/GameAPI.ts) or stubbed [`SocketGameConnection`](./services/GameConnection.ts) that emits scripted `game_state` and `game_over` messages.
...
- For sandbox controllers:
  - Provide a lightweight fake [`ClientSandboxEngine`](./sandbox/ClientSandboxEngine.ts) that returns canned `GameState` snapshots and accepts no‑op methods for moves.
  - Verify that [`useSandboxInteractions()`](./hooks/useSandboxInteractions.ts) correctly interprets clicks, double‑clicks, and AI turns given those snapshots.

**DecisionUI tests**

- Use React Testing Library to test decision flows:
  - Simulate `pendingChoice` values and ensure [`ChoiceDialog`](./components/ChoiceDialog.tsx) calls `respondToChoice` with the right payloads.
  - Exercise countdown logic in isolation by faking `choiceDeadline`.
- Chat flows can be tested via [`useChatMessages`](./hooks/useGameActions.ts) with a fake [`GameContext`](./contexts/GameContext.tsx).

**BoardAndHUDLayout tests**

- Treat layout components as **pure presentation**:
  - Snapshot or DOM tests using static view models from [`gameViewModels.ts`](./adapters/gameViewModels.ts).
  - No engine, context, or orchestrator mocking is required.

**DiagnosticsPanel tests**

- For backend diagnostics:
  - Feed synthetic sequences of `GameState` and `ConnectionStatus` into a `useGameDiagnosticsLog` hook and assert on log entries rendered by [`GameEventLog.tsx`](./components/GameEventLog.tsx).
- For sandbox diagnostics:
  ...
  - Any tests that use [`sandboxCaptureSearch.ts`](./sandbox/sandboxCaptureSearch.ts) must operate on cloned boards and never on live engine instances.
...
With these roles, boundaries, and testing hooks, the frontend host layer can be refactored incrementally while strictly preserving the orchestrator‑as‑SSOT model described in [`RULES_SSOT_MAP.md`](../../docs/rules/RULES_SSOT_MAP.md).
