...
describe.skip('Python vs TS self-play replay parity (DB fixtures)', () => {
  ...
  // For now, exercise just the first fixture as a focused repro harness.
  const firstFixturePath = path.join(fixturesDir, fixtureFiles[0]);

  test(`replays canonical move for ${path.basename(firstFixturePath)} and compares TS vs Python summaries`, async () => {
    const raw = readFileSync(firstFixturePath, 'utf-8');
    ...
    for (let i = 0; i <= targetIndex && i < recordedMoves.length; i += 1) {
      await engine.applyCanonicalMoveForReplay(recordedMoves[i]);
    }
