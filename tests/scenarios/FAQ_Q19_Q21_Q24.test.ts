...
describe('FAQ Q19-Q21, Q24: Player Counts, Thresholds & Forced Elimination', () => {
  ...
  describe('FAQ Q19: Player Count Variations', () => {
    describe('2-Player Games', () => {
      ...
      it('should use correct thresholds for 2-player square19 (>36 rings)', () => {
        ...
        // Total rings: 2 × 36 = 72
        // Victory threshold: >36 (>50% of 72)
        expect(gameState.victoryThreshold).toBe(36);
    ...
    describe('3-Player Games (Recommended)', () => {
      it('should use correct thresholds for 3-player square8 (>27 rings)', () => {
        ...
        // Total rings: 3 × 18 = 54
        // Victory threshold: >27 (>50% of 54)
        expect(gameState.victoryThreshold).toBe(27);
      ...
      it('should use correct thresholds for 3-player square19 (>54 rings)', () => {
        ...
        // Victory threshold: >54 (>50% of 108)
        expect(gameState.victoryThreshold).toBe(54);
    ...
    describe('4-Player Games', () => {
      it('should use correct thresholds for 4-player square19 (>72 rings)', () => {
        ...
        // Victory threshold: >72 (>50% of 144)
  ...
  describe('FAQ Q21: Victory Thresholds Always >50%', () => {
    it('should guarantee no simultaneous victories with >50% rule', () => {
      ...
      const testCases = [
        { total: 36, threshold: 18 },   // 2p square8
        { total: 54, threshold: 27 },   // 3p square8
        { total: 72, threshold: 36 },   // 2p square19/hex
        { total: 108, threshold: 54 },  // 3p square19/hex
        { total: 144, threshold: 72 }   // 4p square19/hex
      ];
    ...
    it('should use territory threshold >50% of board spaces', () => {
      ...
      const configs = [
        { boardType: 'square8', spaces: 64, threshold: 32 },
        { boardType: 'square19', spaces: 361, threshold: 180 },
        { boardType: 'hexagonal', spaces: 331, threshold: 165 }
      ];
  ...
  describe('FAQ Q24: Forced Elimination When Blocked', () => {
    it('should force cap elimination when no moves available but stacks exist', async () => {
      ...
      const engine = new GameEngine('faq-q24-backend', 'square8', [
      ...
      // Should have forced_elimination move available
      const moves = engine.getValidMoves(1);
      const forcedElimMoves = moves.filter((m: any) => m.type === 'forced_elimination');
      
      expect(forcedElimMoves.length).toBeGreaterThan(0);
      
      // Execute forced elimination
      const result = await engine.makeMove(forcedElimMoves[0]);
    ...
    it('should count force-eliminated rings toward victory total', async () => {
      ...
      const engine = new GameEngine('faq-q24-victory-backend', 'square8', [
    ...
    it('should continue game after forced elimination if under threshold', async () => {
      ...
      const engine = new GameEngine('faq-q24-continue-backend', 'square8', [
      ...
      await engine.makeMove(forcedElimMoves[0]);
  ...
  describe('FAQ Q11 & Q21: Stalemate with Rings in Hand', () => {
    it('should convert rings in hand to eliminated rings on stalemate', async () => {
      ...
      const engine = new GameEngine('faq-q11-backend', 'square8', [
  ...
  describe('Territory Victory Thresholds', () => {
    it('should use >50% of board spaces for all board types', () => {
      ...
      const configs = [
        { boardType: 'square8', totalSpaces: 64, threshold: 32 },
        { boardType: 'square19', totalSpaces: 361, threshold: 180 },
        { boardType: 'hexagonal', totalSpaces: 331, threshold: 165 }
      ];
      ...
      configs.forEach(config => {
        ...
        // Verify exactly >50%
        expect(config.threshold).toBe(Math.floor(config.totalSpaces / 2));
      });
    });
  });
});