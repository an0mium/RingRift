...
describe('FAQ Q22-Q23: Graduated Line Rewards & Territory Prerequisites', () => {
  ...
  describe('FAQ Q22: Graduated Line Rewards Strategic Choices', () => {
    describe('Option 1: Maximize Territory (Collapse All + Eliminate Ring)', () => {
      it('should collapse entire overlength line and eliminate ring with Option 1', async () => {
        ...
        const engine = new GameEngine('faq-q22-option1-backend', 'square8', createBasePlayers(), timeControl, false);
        ...
        const lineProcessMoves = moves.filter((m: any) => 
          m.type === 'process_line' || m.type === 'choose_line_reward'
        );
        
        expect(lineProcessMoves.length).toBeGreaterThan(0);
        
        // If explicit choice is available, select Option 1
        const option1Move = lineProcessMoves.find((m: any) => 
          m.type === 'choose_line_reward' && m.option === 'collapse_all'
        );
    ...
    describe('Option 2: Preserve Rings (Collapse Minimum + No Elimination)', () => {
      it('should collapse only required markers without elimination with Option 2', async () => {
        ...
        const engine = new GameEngine('faq-q22-option2-backend', 'square8', createBasePlayers(), timeControl, false);
        const engineAny: any = engine;
        const gameState = engineAny.gameState;
    ...
    describe('Strategic Scenarios', () => {
      it('should use Option 1 when territory is strategically valuable', async () => {
        ...
        const engine = new GameEngine('faq-q22-strategic-backend', 'square8', createBasePlayers(), timeControl, false);
        const engineAny: any = engine;
        const gameState = engineAny.gameState;
  ...
  describe('FAQ Q23: Territory Self-Elimination Prerequisite', () => {
    describe('Cannot Process Region Without Outside Stack', () => {
      it('should NOT process disconnected region when player has no outside stack', async () => {
        ...
        const engine = new GameEngine('faq-q23-negative-backend', 'square19', [
        ...
        // Get valid moves - should NOT include processing this region
        const moves = engine.getValidMoves(1);
        const territoryMoves = moves.filter((m: any) => m.type === 'process_territory_region');
        
        // Cannot process because Blue has no stack outside the region
    ...
    describe('Can Process Region With Outside Stack', () => {
      it('should process disconnected region when player has outside stack', async () => {
        ...
        const engine = new GameEngine('faq-q23-positive-backend', 'square19', [
          createTestPlayer(1, { ringsInHand: 36, eliminatedRings: 0 }),
          createTestPlayer(2, { ringsInHand: 36, eliminatedRings: 0 }),
          createTestPlayer(3, { ringsInHand: 36, eliminatedRings: 0 })
        ], timeControl, false);
        ...
        // Verify:
        // - All rings inside region eliminated (Blue 1 + Red 2 = 3)
        // - Blue's outside stack cap eliminated (2 more)
        // - Total: 5 rings eliminated for Blue
        expect(gameState.players[0].eliminatedRings).toBe(initialEliminated + 3); // 1 inside + 2 from outside cap
        
        // Region should be collapsed
        const regionCollapsed = gameState.board.collapsedSpaces.get('6,6');
        expect(regionCollapsed).toBe(1); // Blue's color
      });
    });
    ...
    describe('Multiple Regions With Limited Stacks', () => {
      it('should allow processing only as many regions as outside stacks available', async () => {
        ...
        const engine = new GameEngine('faq-q23-multiple-backend', 'square8', createBasePlayers(), timeControl, false);
        const engineAny: any = engine;
        const gameState = engineAny.gameState;
  ...
  describe('Hexagonal Board Variants', () => {
    it('should apply same prerequisites on hexagonal boards', async () => {
      ...
      const engine = new GameEngine('faq-q23-hex-backend', 'hexagonal', [
        createTestPlayer(1, { ringsInHand: 36, eliminatedRings: 0 }),
        createTestPlayer(2, { ringsInHand: 36, eliminatedRings: 0 }),
        createTestPlayer(3, { ringsInHand: 36, eliminatedRings: 0 })
      ], timeControl, false);
