...
describe('FAQ Q7-Q8: Line Formation & Collapse', () => {
  ...
  describe('FAQ Q7: Multiple Lines of Markers (Version-Specific Length)', () => {
    describe('8×8 Version - 4+ markers required', () => {
      it('should process exact-length line (4 markers) with mandatory elimination', async () => {
        // FAQ Q7: Exact-length line on 8×8 requires collapsing all 4 markers
        // and eliminating one ring/cap
        
        const engine = new GameEngine('faq-q7-exact-backend', 'square8', createBasePlayers(), timeControl, false);
        const engineAny: any = engine;
        const gameState = engineAny.gameState;
        
        // Set up 4 consecutive Blue markers in a row
      ...
      it('should process overlength line (5+ markers) with two options', async () => {
        // FAQ Q7: Overlength line allows choice between:
        // Option 1: Collapse all + eliminate ring
        // Option 2: Collapse only 4 + no elimination
        
        const engine = new GameEngine('faq-q7-overlength-backend', 'square8', createBasePlayers(), timeControl, false);
        const engineAny: any = engine;
        const gameState = engineAny.gameState;
        
        // Set up 5 consecutive Blue markers
        gameState.board.markers.clear();
        gameState.board.collapsedSpaces.clear();
        ...
        // With Option 2: only 4 markers collapsed, no elimination
        expect(gameState.players[0].eliminatedRings).toBe(initialEliminated);
        expect(gameState.players[0].territorySpaces).toBe(4);
      });
    });

    describe('19×19 Version - 5+ markers required', () => {
      it('should process exact-length line (5 markers) with mandatory elimination', async () => {
        // FAQ Q7: Exact-length line on 19×19 requires 5 markers
        
        const engine = new GameEngine('faq-q7-square19-backend', 'square19', [
        ...
        // All 5 markers collapsed, entire cap (3 rings) eliminated
        expect(gameState.players[0].eliminatedRings).toBe(initialEliminated + 3);
        expect(gameState.players[0].territorySpaces).toBe(5);
      });
    });
  });
  ...
  describe('FAQ Q8: No Rings/Stacks to Remove When Required', () => {
    it('should prevent exact-length line processing when no rings to eliminate', async () => {
      ...
      const engine = new GameEngine('faq-q8-backend', 'square8', createBasePlayers(), timeControl, false);
      const engineAny: any = engine;
      const gameState = engineAny.gameState;
      
      // Set up 4-marker line for Blue (exact length for square8)
      gameState.board.markers.clear();
    ...
    it('should allow overlength line with Option 2 when no elimination rings available', async () => {
      ...
      const engine = new GameEngine('faq-q8-option2-backend', 'square8', createBasePlayers(), timeControl, false);
      const engineAny: any = engine;
      const gameState = engineAny.gameState;
      
      // Set up 5-marker line (overlength for square8)
      gameState.board.markers.clear();
      gameState.board.stacks.clear();
      ...
      // Only 4 markers collapsed (Option 2), no elimination
      expect(gameState.players[0].eliminatedRings).toBe(0);
      expect(gameState.players[0].territorySpaces).toBe(4);
    });
  });
  ...
  describe('Multiple Intersecting Lines', () => {
    it('should process lines one at a time, invalidating intersecting lines', async () => {
      ...
      const engine = new GameEngine('faq-q7-intersecting-backend', 'square8', createBasePlayers(), timeControl, false);
      const engineAny: any = engine;
      const gameState = engineAny.gameState;
