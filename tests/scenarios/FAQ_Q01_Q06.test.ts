...
describe('FAQ Q1-Q6: Basic Mechanics', () => {
  ...
  describe('FAQ Q1: Cannot Split or Rearrange Stack Order', () => {
    it('should maintain ring order in stacks - cannot rearrange', () => {
      ...
      const engine = new GameEngine('faq-q1-backend', 'square8', createBasePlayers(), timeControl, false);
    ...
    it('should add captured rings to bottom of capturing stack', async () => {
      ...
      const engine = new GameEngine('faq-q1-capture-backend', 'square8', createBasePlayers(), timeControl, false);
      const engineAny: any = engine;
      const gameState = engineAny.gameState;
  ...
  describe('FAQ Q2: Minimum Jump Requirements', () => {
    describe('Stack Height = Distance Requirement', () => {
      it('should require height-1 stack to move ≥1 space', () => {
        const engine = new GameEngine('faq-q2-h1-backend', 'square8', createBasePlayers(), timeControl, false);
        const engineAny: any = engine;
        const gameState = engineAny.gameState;
      ...
      it('should require height-4 stack to move ≥4 spaces', () => {
        const engine = new GameEngine('faq-q2-h4-backend', 'square8', createBasePlayers(), timeControl, false);
        const engineAny: any = engine;
        const gameState = engineAny.gameState;
    ...
    describe('Markers Count Toward Distance', () => {
      it('should count markers when calculating minimum distance', async () => {
        ...
        const engine = new GameEngine('faq-q2-markers-backend', 'square8', createBasePlayers(), timeControl, false);
        const engineAny: any = engine;
        const gameState = engineAny.gameState;
        ...
        const moveTo55 = moves.find((m: any) => 
          m.to?.x === 5 && m.to?.y === 5
        );
  ...
  describe('FAQ Q3: Landing Distance Flexibility During Captures', () => {
    it('should allow landing on any valid space beyond target (not just first)', async () => {
      ...
      const engine = new GameEngine('faq-q3-backend', 'square19', [
  ...
  describe('FAQ Q4: Rings Under Captured Top Ring', () => {
    it('should leave remaining rings when only top is captured', async () => {
      // FAQ Q4: Only top ring is overtaken; rest stay in place
      
      const engine = new GameEngine('faq-q4-backend', 'square8', createBasePlayers(), timeControl, false);
      const engineAny: any = engine;
      const gameState = engineAny.gameState;
      ...
      // Capturing stack should have the captured ring
      const capturingStack = gameState.board.stacks.get('4,4');
      expect(capturingStack).toBeDefined();
      expect(capturingStack!.stackHeight).toBe(2); // 1 Blue + 1 Red captured
      expect(capturingStack!.rings).toEqual([1, 2]); // Blue, Red - captured at bottom
    });
  });

  describe('FAQ Q5: Multiple Ring Capture from One Stack', () => {
    it('should capture only one ring per jump segment', async () => {
      // FAQ Q5: Single jump = single top ring captured
      
      const engine = new GameEngine('faq-q5-single-backend', 'square8', createBasePlayers(), timeControl, false);
      const engineAny: any = engine;
      const gameState = engineAny.gameState;
      ...
      // Target should still have 2 rings
      const target = gameState.board.stacks.get('3,3');
      expect(target!.stackHeight).toBe(2);
      
      // Capturing stack should have gained only 1 ring
      const capturer = gameState.board.stacks.get('4,4');
      expect(capturer!.stackHeight).toBe(2); // 1 + 1 captured
    });

    it('should allow multiple captures from same stack via reversal/cycles', async () => {
      // FAQ Q5: Can capture multiple rings through chain (180° or cyclic)
      
      const engine = new GameEngine('faq-q5-multiple-backend', 'square8', createBasePlayers(), timeControl, false);
      const engineAny: any = engine;
      const gameState = engineAny.gameState;
      ...
      // Chain should continue with 180° reversal possible
      ...
      // Target should have fewer rings (captured multiple times)
      const target = gameState.board.stacks.get('2,3');
      if (target) {
        // Some rings captured via reversal
  ...
  describe('FAQ Q6: Overtaking vs Elimination Distinction', () => {
    it('should keep overtaken rings in play (not count toward victory)', async () => {
      ...
      const engine = new GameEngine('faq-q6-overtake-backend', 'square8', createBasePlayers(), timeControl, false);
      const engineAny: any = engine;
      const gameState = engineAny.gameState;
    ...
    it('should count eliminated rings toward victory threshold', async () => {
      ...
      const engine = new GameEngine('faq-q6-elim-backend', 'square8', createBasePlayers(), timeControl, false);
      const engineAny: any = engine;
      const gameState = engineAny.gameState;
  ...
  describe('Unified Movement Landing Rule', () => {
    it('should allow landing on any valid space beyond markers (not just first)', async () => {
      ...
      const engine = new GameEngine('faq-q2-q3-unified-backend', 'square19', [
        createTestPlayer(1, { ringsInHand: 36 }),
        createTestPlayer(2, { ringsInHand: 36 })
      ], timeControl, false);
      const engineAny: any = engine;
