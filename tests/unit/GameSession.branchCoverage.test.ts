...
describe('GameSession Branch Coverage - handlePlayerMove runtime branches', () => {
  ...
  it('throws when move destination is missing', async () => {
    ...
    const moveData = {
      moveType: 'place_ring',
      position: JSON.stringify({ from: { x: 0, y: 0 } }), // No 'to' field
    } as const;
...
// ============================================================================
// NEW TESTS FOR maybePerformAITurn FULL EXECUTION
// ============================================================================

describe('GameSession Branch Coverage - maybePerformAITurn full execution', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('executes AI turn when current player is AI', async () => {
    ...
    // Mock AIEngine to return a move
    const { globalAIEngine } = require('../../src/server/game/ai/AIEngine');
    globalAIEngine.getAIConfig.mockReturnValue({ difficulty: 5 });
    globalAIEngine.getAIMove.mockResolvedValue({
      id: 'ai-move-1',
      type: 'ring_placement',
      player: 1,
      to: { x: 0, y: 0 },
    ...
    // AI is player 1, should trigger AI turn on init
    const state = session.getGameState();
  ...
  it('handles decision phase moves for AI player', async () => {
    ...
    const { globalAIEngine } = require('../../src/server/game/ai/AIEngine');
    globalAIEngine.getAIConfig.mockReturnValue({ difficulty: 5 });
    globalAIEngine.chooseLocalMoveFromCandidates.mockReturnValue({
      id: 'decision-move-1',
      type: 'process_line',
      player: 1,
      to: { x: 0, y: 0 },
    ...
    // Session created with AI player
    expect(session.getGameState().players[0].type).toBe('ai');
  });
});

// ============================================================================
// NEW TESTS FOR LOCAL FALLBACK BRANCHES
// ============================================================================

describe('GameSession Branch Coverage - AI fallback branches', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('uses local fallback when getAIMove returns null', async () => {
    ...
    const { globalAIEngine } = require('../../src/server/game/ai/AIEngine');
    globalAIEngine.getAIConfig.mockReturnValue({ difficulty: 5 });
    globalAIEngine.getAIMove.mockResolvedValue(null);
    globalAIEngine.getLocalFallbackMove.mockReturnValue({
      type: 'ring_placement',
      player: 1,
      to: { x: 0, y: 0 },
      thinkTime: 0,
    ...
    // AI player should be configured
    const state = session.getGameState();
    expect(state.players[0].type).toBe('ai');
  });
});
