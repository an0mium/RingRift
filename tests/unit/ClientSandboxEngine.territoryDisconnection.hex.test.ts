 ...
 *
 * This mirrors BoardManager.territoryDisconnection.hex.test.ts but uses the
 * ClientSandboxEngine + sandboxTerritory helpers instead of the backend
 * BoardManager. We stub hex territory detection and line
...
(skipWithOrchestrator ? describe.skip : describe)(
  'ClientSandboxEngine territory + line processing (hexagonal)',
  () => {
    ...
    test('line + territory consequences combine correctly on hex board (stubbed detection)', async () => {
      ...
      jest.spyOn(sharedLineDetection, 'findAllLines').mockImplementation(() => [lineInfo]);
      ...
      const stacksInRegion = Array.from(finalBoard.stacks.keys()).filter((k) =>
        interiorKeys.has(k)
      );
