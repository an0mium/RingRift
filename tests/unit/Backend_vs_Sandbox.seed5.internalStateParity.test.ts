...
function normaliseBackendInternalState(engine: any): InternalStateSnapshot {
  ...
  const rawMask: Map<number, boolean> | undefined = engine.lpsCurrentRoundActorMask;
  ...
  return {
    ...
    lpsRoundIndex:
      typeof engine.lpsRoundIndex === 'number' && Number.isFinite(engine.lpsRoundIndex)
        ? engine.lpsRoundIndex
        : null,
    lpsCurrentRoundFirstPlayer:
      typeof engine.lpsCurrentRoundFirstPlayer === 'number'
        ? engine.lpsCurrentRoundFirstPlayer
        : null,
    lpsExclusivePlayerForCompletedRound:
      typeof engine.lpsExclusivePlayerForCompletedRound === 'number'
        ? engine.lpsExclusivePlayerForCompletedRound
        : null,
...
function normaliseSandboxInternalState(engine: any): InternalStateSnapshot {
  const mask: Array<{ playerNumber: number; hadRealAction: boolean }> = [];
  const rawMask: Map<number, boolean> | undefined = engine._lpsCurrentRoundActorMask;
  ...
  return {
    ...
    lpsRoundIndex:
      typeof engine._lpsRoundIndex === 'number' && Number.isFinite(engine._lpsRoundIndex)
        ? engine._lpsRoundIndex
        : null,
    lpsCurrentRoundFirstPlayer:
      typeof engine._lpsCurrentRoundFirstPlayer === 'number'
        ? engine._lpsCurrentRoundFirstPlayer
        : null,
    lpsExclusivePlayerForCompletedRound:
      typeof engine._lpsExclusivePlayerForCompletedRound === 'number'
        ? engine._lpsExclusivePlayerForCompletedRound
        : null,
    lpsCurrentRoundActorMask: mask,
  };
}

function getBackendSemanticLpsSnapshot(
  engine: any,
  state: GameState
): SemanticLpsSnapshot {
  const perPlayer = state.players
    ...
    .map((p) => {
      ...
      const hasMaterial = typeof engine.playerHasMaterial === 'function'
        ? engine.playerHasMaterial(playerNumber) === true
        : false;
      const hasRealAction = typeof engine.hasAnyRealActionForPlayer === 'function'
  ...
  const exclusiveCandidate =
    typeof engine.lpsExclusivePlayerForCompletedRound === 'number'
      ? (engine.lpsExclusivePlayerForCompletedRound as number)
      : null;
...
function getSandboxSemanticLpsSnapshot(engine: any): SemanticLpsSnapshot {
  ...
  const exclusiveCandidate =
    typeof engine._lpsExclusivePlayerForCompletedRound === 'number'
      ? (engine._lpsExclusivePlayerForCompletedRound as number)
      : null;
...
describe('Backend vs Sandbox internal-state parity diagnostics (square8 / 2p / seed=5)', () => {
  ...
  test('logs earliest divergence in GameState and internal metadata across full seed-5 trace', async () => {
    ...
    for (let i = 0; i < moves.length; i++) {
      ...
      const sandboxSemanticLps = getSandboxSemanticLpsSnapshot(
        sandboxEngine as any
      );

      const gameStatesEqual = snapshotsEqual(backendSnap, sandboxSnap);
      const internalsEqual =
        JSON.stringify(backendInternal) === JSON.stringify(sandboxInternal);
