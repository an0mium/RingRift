...
(skipWithOrchestrator ? describe.skip : describe)(
  'Square19 two-region territory + elimination parity (GameEngine vs ClientSandboxEngine)',
  () => {
    ...
    test('backend and sandbox stay in parity through two-region processing (B then A) and self-elimination', async () => {
      const { initialState, regionA, regionB, outsideStackPositions } =
        createSquare19TwoRegionTerritoryScenario('square19-two-region-territory-region-then-elim');
      ...
      const backendEngine = new GameEngine(
        'square19-two-region-territory-region-then-elim-backend',
        boardType,
        backendPlayers,
        backendTimeControl,
        true
      ...
      if (!snapshotsEqual(backendSnapAfterRegionB, sandboxSnapAfterRegionB)) {
        console.error('[TerritoryDecisions.Square19TwoRegionThenElim] mismatch after Region B', {
      ...
      if (!snapshotsEqual(backendSnapAfterRegionA, sandboxSnapAfterRegionA)) {
        console.error('[TerritoryDecisions.Square19TwoRegionThenElim] mismatch after Region A', {
      ...
      if (!snapshotsEqual(backendFinalSnap, sandboxFinalSnap)) {
        console.error('[TerritoryDecisions.Square19TwoRegionThenElim] final snapshot mismatch', {
