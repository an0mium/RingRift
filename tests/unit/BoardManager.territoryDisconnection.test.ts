...
describe('BoardManager territory disconnection (square19, Von Neumann)', () => {
  ...
  it('Q20_square8_von_neumann_territory_disconnection_backend', () => {
    ...
    // FAQ Q20 emphasises that territory connectivity on both 8×8 and
    // 19×19 uses Von Neumann (4-direction) adjacency, even though
    // movement and lines use Moore (8-direction). This scenario mirrors
    // the canonical 3×3 disconnection example from the 19×19 rules on
    // a smaller 8×8 board.
    //
    // Three players: 1 (A), 2 (B), 3 (C).
    ...
    // A marker border around the 3×3 block using Von Neumann
    // adjacency, sized for the 8×8 board.
    const borderCoords: Array<[number, number]> = [];
    for (let x = 1; x <= 5; x++) {
