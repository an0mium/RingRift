...
describe('Seed5 move63 line-detection snapshot invariants (square8 / 2p / seed=5)', () => {
  const boardType: BoardType = 'square8';
  const numPlayers = 2;
  const seed = 5;
  const MAX_STEPS = 64;

  function canonicalLineKeys(lines: { positions: { x: number; y: number; z?: number }[] }[]): string[] {
    return lines
      .map((line) =>
        line.positions
