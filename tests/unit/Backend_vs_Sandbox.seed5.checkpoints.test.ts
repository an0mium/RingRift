 ...
 */
describe('Backend vs Sandbox seed=5 checkpoint diagnostics around move index 43', () => {
  ...
  test('collects backend vs sandbox checkpoints for seed=5 move index 43', async () => {
    ...
    // --- Apply target move index 43 with checkpoints enabled ---
    {
      ...
      const backendValidMoves = backendEngine.getValidMoves(
        backendStateBefore.currentPlayer
      );
      const matching = findMatchingBackendMove(targetMove, backendValidMoves);

      if (!matching) {
        // eslint-disable-next-line no-console
        console.error('[Backend_vs_Sandbox.seed5.checkpoints] No matching backend move at index 43', {
          sandboxMove: {
