...
(skipWithOrchestrator ? describe.skip : describe)(
  'Hex two-region territory + elimination parity (GameEngine vs ClientSandboxEngine)',
  () => {
    ...
    test('backend and sandbox stay in parity through two-region processing (B then A) and self-elimination', async () => {
      const { initialState, regionA, regionB, outsideStackPositions } =
        createHexTwoRegionTerritoryScenario('hex-two-region-territory-region-then-elim');
      ...
      const backendEngine = new GameEngine(
        'hex-two-region-territory-region-then-elim-backend',
        boardType,
        backendPlayers,
        backendTimeControl,
        true
      ...
      if (!snapshotsEqual(backendSnapAfterRegionB, sandboxSnapAfterRegionB)) {
        console.error('[TerritoryDecisions.HexTwoRegionThenElim] mismatch after Region B', {
      ...
      if (!snapshotsEqual(backendSnapAfterRegionA, sandboxSnapAfterRegionA)) {
        console.error('[TerritoryDecisions.HexTwoRegionThenElim] mismatch after Region A', {
      ...
      if (!snapshotsEqual(backendFinalSnap, sandboxFinalSnap)) {
        console.error('[TerritoryDecisions.HexTwoRegionThenElim] final snapshot mismatch', {
