...
test('ClientSandboxEngine single-seed debug: square8 with 2 AI players, seed=18 terminates or logs diagnostics', async () => {
  ...
  try {
    ...
    if (finalState.gameStatus === 'active') {
      ...
      const movementViewForHypothetical = (hypothetical: GameState['board']): MovementBoardView => {
        return {
      ...
      const adaptersForHypothetical = (hypothetical: GameState['board']): CaptureBoardAdapters => {
        return {
          isValidPosition: (pos: Position) => isValidPositionLocal(pos),
