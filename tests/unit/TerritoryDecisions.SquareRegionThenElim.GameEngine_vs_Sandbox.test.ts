...
(skipWithOrchestrator ? describe.skip : describe)(
  'Square8 territory region + elimination parity (GameEngine vs ClientSandboxEngine)',
  () => {
    function clonePlayers(players: Player[]): Player[] {
      return players.map((p) => ({ ...p }));
    }

    test('backend and sandbox stay in parity through square8 region processing and self-elimination', async () => {
      const { initialState, region, outsideStackPosition } = createSquareTerritoryRegionScenario(
        'square-territory-region-then-elim'
      );
      ...
      const backendEngine = new GameEngine(
        'square-territory-region-then-elim-backend',
        boardType,
        backendPlayers,
        backendTimeControl,
        true
      ...
      const backendAfterRegionSnap = snapshotFromGameState(
        'backend-square-after-region',
        backendAfterRegion
      );
      const sandboxAfterRegionSnap = snapshotFromGameState(
        'sandbox-square-after-region',
        sandboxAfterRegion
      );

      if (!snapshotsEqual(backendAfterRegionSnap, sandboxAfterRegionSnap)) {
        console.error(
          '[SquareTerritoryParity] mismatch after process_territory_region',
      ...
      const backendFinalSnap = snapshotFromGameState('backend-square-final', backendFinal);
      const sandboxFinalSnap = snapshotFromGameState('sandbox-square-final', sandboxFinal);

      if (!snapshotsEqual(backendFinalSnap, sandboxFinalSnap)) {
        console.error(
          '[SquareTerritoryParity] mismatch after eliminate_rings_from_stack',
